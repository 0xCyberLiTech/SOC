...
Restauration de tous les blocs applicatifs sur la VM test (IP `<VM-TEST-IP>`),
...
> **[ERRR] nginx en Phase A** : attendu — la config bind sur `<SRV-NGINX-IP>` (IP prod).
> La VM test ayant `<VM-TEST-IP>`, nginx ne peut pas démarrer. Tous les fichiers sont en place.
...
ssh -i ~/.ssh/id_proxmox -p <SSH-PORT> root@<PROXMOX-IP> "qm shutdown 108 && qm wait 108"

# 2. Appliquer le bloc réseau sur la VM test
ssh root@<VM-TEST-IP> "bash restore-soc.sh soc-config-2026-04-28_2100.tar.gz --step network"
# → VM test prend <SRV-NGINX-IP>, SSH bascule sur port <SSH-PORT>

# 3. Reconnecter sur la VM test (maintenant à l'IP prod)
ssh-keygen -R "[<SRV-NGINX-IP>]:<SSH-PORT>"   # effacer l'ancienne clé host
...
curl -sI http://<SRV-NGINX-IP>:8080/
...
```bash
# Éteindre la VM test (libère <SRV-NGINX-IP>)
ssh -i ~/.ssh/id_proxmox -p <SSH-PORT> root@<PROXMOX-IP> "qm shutdown 101 && qm wait 101"
# Rallumer la prod
ssh -i ~/.ssh/id_proxmox -p <SSH-PORT> root@<PROXMOX-IP> "qm start 108"
