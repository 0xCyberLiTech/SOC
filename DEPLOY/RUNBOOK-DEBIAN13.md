...
if step_active "ufw"; then
  ...
  if ufw status | grep -q "Status: active"; then
    ...
    run "ufw allow out to ${CLT_IP} port ${SSH_PORT} proto tcp comment 'SSH sync clt'"
    run "ufw allow out to ${PA85_IP} port ${SSH_PORT} proto tcp comment 'SSH monitoring pa85'"
...
echo "  3. Configurer les vhosts nginx (clt, pa85, monitoring)"
