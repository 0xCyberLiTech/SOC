...
function _kcSetBtnOk(btn,via){if(!btn)return;btn.textContent='\u2714 BANNI 24h \u2014 '+via;btn.classList.add('kc-btn-ok');}
function _kcSetBtnErr(btn,msg){if(!btn)return;btn.textContent='\u26a0 '+(msg||'').slice(0,30);btn.classList.add('kc-btn-err');btn.disabled=false;}
function _kcBanViaCrowdSec(btn,ipStr){
  if(btn) btn.textContent='\u29d7 BAN\u2026 (CrowdSec direct)';
  var csKey='';try{csKey=localStorage.getItem(_LS_KEYS.CS_KEY)||'';}catch(e){}
  if(!csKey){
    if(!btn){return;}
    _kcSetBtnErr(btn,'Clé CS absente');
    return;
  }
  var payload=[{duration:'24h',ip:ipStr,reason:'soc-dashboard-ban',scenario:'soc/kill-chain-autoban',scope:'Ip',type:'ban',value:ipStr}];
  fetch('http://'+SOC_INFRA.SRV_NGIX+':8080/v1/decisions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Key '+csKey},body:JSON.stringify(payload),signal:AbortSignal.timeout(_CS_TIMEOUT_MS)})
...
// 7 maillons (2026-05-16) : narrative cyber complète frappe → recon → ... → ban
// PROBE = paquets bruts droppés par UFW (avant nginx) — visibilité "qui sonne à la porte"
// WAF   = blocages ModSecurity inline sur les backends Apache CLT/PA85
var KCK_STAGES = [
  {id:'PROBE',   label:'PROBE',     desc:'Paquets bloqués',color:KC_PALETTE.PROBE.color,   rgb:KC_PALETTE.PROBE.rgb,   mitre:'UFW',  icon:'◍'},
  {id:'RECON',   label:'RECON',     desc:'Sondage',       color:KC_PALETTE.RECON.color,   rgb:KC_PALETTE.RECON.rgb,   mitre:'T1595',icon:'◎'},
  {id:'SCAN',    label:'SCAN',      desc:'Énumération',   color:KC_PALETTE.SCAN.color,    rgb:KC_PALETTE.SCAN.rgb,    mitre:'T1046',icon:'⌖'},
  {id:'EXPLOIT', label:'EXPLOIT',   desc:'Exploitation',  color:KC_PALETTE.EXPLOIT.color, rgb:KC_PALETTE.EXPLOIT.rgb, mitre:'T1190',icon:'◈'},
  {id:'WAF',     label:'WAF',       desc:'ModSec block',  color:KC_PALETTE.WAF.color,     rgb:KC_PALETTE.WAF.rgb,     mitre:'T1190',icon:'◇'},
  {id:'BRUTE',   label:'BRUTE',     desc:'Intrusion',     color:KC_PALETTE.BRUTE.color,   rgb:KC_PALETTE.BRUTE.rgb,   mitre:'T1110',icon:'⊗'},
  {id:'BLOCKED', label:'NEUTRALISÉ',desc:'Défense active',color:KC_PALETTE.BLOCKED.color, rgb:KC_PALETTE.BLOCKED.rgb, mitre:'DEF',  icon:'⬡'},
];
...
window._kcCritThresh={PROBE:50,RECON:10,SCAN:5,EXPLOIT:3,WAF:5,BRUTE:1};
...
function _kcDrawBackground(ctx, W, H){
  ...
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.035)'; ctx.lineWidth=1;
...
function _kcDrawBeacon(ctx, node, nodeR, phase, cfg){
  var rgb=cfg.rgb, xA=cfg.xA, xB=cfg.xB, nY=node.y;
  var bX=cfg.rtl?xB:xA, cnt=node.cnt;
  var pulse1=0.45+0.55*Math.sin(phase*Math.PI*4+cfg.p1);
  var pulse2=0.5+0.5*Math.sin(phase*Math.PI*4+cfg.p2);
  var nA=cnt>0?0.92:0.35, midX=Math.round((xA+xB)/2);
  ctx.save(); ctx.fillStyle='rgba('+rgb+','+(nA*pulse1*0.08)+')';
  ctx.beginPath(); ctx.arc(bX,nY,Math.round(nodeR*1.1),0,Math.PI*2); ctx.fill(); ctx.restore();
  ctx.save(); ctx.strokeStyle='rgba('+rgb+','+(nA*pulse1)+')'; ctx.lineWidth=3; ctx.lineCap='round';
  ctx.shadowColor='rgba('+rgb+','+nA+')'; ctx.shadowBlur=cnt>0?20:7;
  ctx.beginPath(); ctx.moveTo(bX,nY-Math.round(nodeR*1.15)); ctx.lineTo(bX,nY+Math.round(nodeR*1.15)); ctx.stroke(); ctx.restore();
  ctx.save(); ctx.strokeStyle='rgba('+rgb+','+(nA*pulse2*0.35)+')'; ctx.lineWidth=7; ctx.lineCap='round';
  ctx.beginPath(); ctx.moveTo(bX,nY-Math.round(nodeR*1.1)); ctx.lineTo(bX,nY+Math.round(nodeR*1.1)); ctx.stroke(); ctx.restore();
  ctx.save(); ctx.fillStyle='rgba('+rgb+','+(cnt>0?nA*0.80:0.28)+')';
  ...
  ctx.fillText(cfg.labelTop,midX,nY-nodeR-4); ctx.restore();
  if(xB>xA+8){
    var lA=cfg.rtl?xA:xA+4, lB=cfg.rtl?xB-4:xB;
    var tg=ctx.createLinearGradient(xA,0,xB,0);
    tg.addColorStop(cfg.rtl?1:0,'rgba('+rgb+','+(cnt>0?0.60:0.12)+')');
    tg.addColorStop(cfg.rtl?0:1,'rgba('+cfg.rgb2+','+(cnt>0?0.22:0.04)+')');
    ctx.strokeStyle=tg; ctx.lineWidth=cnt>0?5:2; ctx.lineCap='butt';
    ctx.beginPath(); ctx.moveTo(lA,nY); ctx.lineTo(lB,nY); ctx.stroke();
    if(cnt>0){
      ctx.save(); ctx.strokeStyle='rgba('+rgb+',0.40)'; ctx.lineWidth=1;
      ctx.shadowColor='rgba('+rgb+',0.4)'; ctx.shadowBlur=4;
      ctx.beginPath(); ctx.moveTo(lA,nY-3); ctx.lineTo(lB,nY-3); ctx.stroke(); ctx.restore();
    }
    var d=cfg.rtl?1:-1;
    for(var fi=0;fi<3;fi++){
      var ft=((phase*3.5+fi/3)%1);
      var fx=cfg.rtl?xB-6-ft*(xB-xA-14):xA+6+ft*(xB-xA-14);
      var fa=Math.sin(ft*Math.PI)*(cnt>0?nA*0.98:0.22);
      if(fa<0.03) continue;
      ctx.save(); ctx.fillStyle='rgba('+rgb+','+fa+')';
      if(cnt>0){ctx.shadowColor='rgba('+rgb+','+fa+')';ctx.shadowBlur=6;}
      ctx.beginPath();
      ctx.moveTo(fx+d*6,nY-4.5); ctx.lineTo(fx,nY); ctx.lineTo(fx+d*6,nY+4.5); ctx.lineTo(fx+d*3,nY);
      ctx.closePath(); ctx.fill(); ctx.restore();
    }
  }
  ctx.save(); ctx.fillStyle='rgba('+rgb+','+(cnt>0?nA*0.72:0.25)+')';
  ...
  ctx.fillText(cfg.labelBot,midX,nY+nodeR+10); ctx.restore();
}
...
function _kcDrawEscalation(ctx, nodes, nodeR, phase){
  var _flDur=_ESCALATION_FADE_MS, _flNow=Date.now();
  Object.keys(window._kcEscalated||{}).forEach(function(si){
    ...
    if(flA<0.02) return;
    var fn1=nodes[parseInt(si,10)], fn2=nodes[parseInt(si,10)+1];
    if(!fn1||!fn2) return; /* guard couvre NaN si si invalide */
    ctx.save(); ctx.strokeStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+flA+')'; ctx.lineWidth=9; ctx.lineCap='butt';
    ctx.shadowColor='rgba('+KC_PALETTE.BRUTE.rgb+',0.95)'; ctx.shadowBlur=18;
    ...
    if(decay>0.4){
      ...
      ctx.save(); ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+(decay*0.85)+')';
      ctx.font='bold 8px "Courier New",monospace'; ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.shadowColor='rgba('+KC_PALETTE.BRUTE.rgb+',0.7)'; ctx.shadowBlur=6;
      ctx.fillText('\u26a1 '+ev.from+'\u2192'+ev.to,mx,my); ctx.restore();
    }
  });
}

function _kcDrawParticles(ctx, nodes, nodeR, particles){
...
function _kcDrawNodeLabel(ctx,node,nodeR,counts,topCC){
  var cnt=node.cnt, rgb=node.s.rgb;
...
// Option B (2026-05-16) : compteur bans 24h par stage AU-DESSUS de l'hexagone
// (au-dessus des chevrons de progression) — corrige l'illusion de calme quand
// le compteur 15min est à 0 mais que des IPs ont été bannies sur 24h.
// 4 stages attaque uniquement (pas sur le hexagone BLOCKED qui agrège déjà).
// Uniformisation 7 maillons (2026-05-16) : structure identique sur TOUS les hexagones.
// Chaque maillon affiche : 24h:N (haut) + barre proportion + ligne pays (bas).
// BLOCKED inclus : 24h:N utilise counts._alerts24h (alertes CrowdSec cr\u00e9\u00e9es sur 24h).
var _bans24=counts._bans24||{}, _b24=_bans24[node.s.id]||0;
if(node.s.id==='BLOCKED'){
  // NEUTRALISÉ 24h:N = somme des bans 24h tous stages d'attaque (sémantique cohérente
  // avec PROBE/RECON/.../BRUTE qui affichent chacun leurs bans 24h).
  // Représente "défense effective 24h" — agrège ce qui a été neutralisé par stage.
  _b24=(_bans24.PROBE||0)+(_bans24.RECON||0)+(_bans24.SCAN||0)+
       (_bans24.EXPLOIT||0)+(_bans24.WAF||0)+(_bans24.BRUTE||0);
}
// Rehausse pour éviter chevauchement avec halo pulsant qui peut atteindre +15px
var ban24Y=node.y-nodeR-Math.max(28,Math.round(nodeR*0.70));
ctx.save();
ctx.fillStyle=_b24>0?'rgba('+rgb+',0.72)':'rgba(122,154,184,0.28)';
ctx.font='bold '+Math.max(8,Math.round(nodeR*0.26))+'px "Courier New",monospace';
ctx.textAlign='center'; ctx.textBaseline='bottom';
if(_b24>0){ctx.shadowColor='rgba('+rgb+',0.42)'; ctx.shadowBlur=5;}
ctx.fillText('24h: '+_b24,node.x,ban24Y);
ctx.restore();
...
// Ligne pays TOUJOURS affich\u00e9e (uniformit\u00e9 visuelle) \u2014 '\u2014' (\u2014) si vide, en gris p\u00e2le
var ctries=(topCC[node.s.id])||[];
var paysTxt=ctries.length?ctries.join(' \u00b7 '):'\u2014';
ctx.save();
ctx.fillStyle=ctries.length?'rgba('+rgb+',0.52)':'rgba(122,154,184,0.22)';
ctx.font='bold '+Math.max(8,Math.round(nodeR*0.24))+'px "Courier New",monospace';
ctx.textAlign='center'; ctx.textBaseline='top';
if(ctries.length){ctx.shadowColor='rgba('+rgb+',0.25)'; ctx.shadowBlur=3;}
ctx.fillText(paysTxt,node.x,by+bH+5); ctx.restore();
}

function _kcDrawNodes(ctx, nodes, nodeR, maxC, counts, phase, topCC){
  nodes.forEach(function(node,i){
    var cnt=node.cnt;
    // Intensité visuelle (halo/glow/bg) basée sur events (hits cumulés) pas IPs uniques
    var _intMap=counts._intensity||{}, _maxI=counts._maxI||maxC;
    var intCur=_intMap[node.s.id]||cnt;
    var inten=intCur>0?Math.min(1,intCur/(_maxI*0.6)):0;
    ...
    ctx.stroke(); ctx.restore();
    // Icône + compteur (icône lue depuis node.s.icon — KC 7 maillons 2026-05-16)
    var stageIcon=(node.s.icon)||'◯';
    ...
    ctx.fillText(stageIcon,node.x,node.y-Math.round(nodeR*0.30)); ctx.restore();
    ...
    _kcDrawNodeLabel(ctx,node,nodeR,counts,topCC);
...
function _kcDrawMetrics(ctx, W, kcData, counts){
  ...
  if(_kcKillRate>0){
    var krRgb=_kcKillRate>70?KC_PALETTE.BLOCKED.rgb:_kcKillRate>40?KC_PALETTE.EXPLOIT.rgb:KC_PALETTE.BRUTE.rgb;
    ctx.fillStyle='rgba('+krRgb+',0.6)'; ctx.fillText('KILL RATE '+_kcKillRate+'%',W-10,20);
  }
  if(_kcActive>0){ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.5)'; ctx.fillText(_kcActive+' actif'+(1!==_kcActive?'s':''),W-10,32);}
  if(_kcCs>0){ctx.fillStyle='rgba('+KC_PALETTE.SCAN.rgb+',0.48)'; ctx.fillText('\u229b CS '+_kcCs,W-10,44);}
  ctx.restore();
}

// ── Orchestrateur principal ──────────────────────────────────────────────────
function _drawKillChain(canvas, kcData, f2bBanned, phase, particles){
  if(!canvas) return;
  ...
  // Option B (2026-05-16) : bans 24h par stage transmis sous chaque hexagone
  counts._bans24=kcData.stage_bans_24h||{};
  // Uniformisation 7 maillons : 24h:N sur BLOCKED = alertes CrowdSec créées sur 24h
  counts._alerts24h=((window._lastData||{}).crowdsec||{}).alerts_24h||0;
  // Intensité animation (2026-05-16) : compteur events (hits cumulés) ≠ IPs uniques.
  // Restaure les pulses/halos/shadows proportionnels au volume réel d'attaque.
  var _intMap={};
  (kcData.active_ips||[]).forEach(function(ip){
    var sid=(ip.stage||'').toUpperCase();
    _intMap[sid]=(_intMap[sid]||0)+(ip.count||1);
  });
  (kcData.probe_ips_15min||[]).forEach(function(e){
    _intMap['PROBE']=(_intMap['PROBE']||0)+(e.count||1);
  });
  (kcData.waf_ips_15min||[]).forEach(function(e){
    _intMap['WAF']=(_intMap['WAF']||0)+(e.count||1);
  });
  _intMap['BLOCKED']=counts['BLOCKED']||0;
  counts._intensity=_intMap;
  var maxC=Math.max(1, KCK_STAGES.reduce(function(m,s){return Math.max(m,counts[s.id]||0);},0));
  // maxI = max d'intensité (events) — utilisé par _kcDrawNodes pour halos/glows
  counts._maxI=Math.max(1, KCK_STAGES.reduce(function(m,s){return Math.max(m,(counts._intensity||{})[s.id]||0);},0));
  ...
  // Top countries par stage — agrège active_ips (RECON/SCAN/EXPLOIT/BRUTE) +
  // probe_ips_15min (PROBE) + waf_ips_15min (WAF) pour cohérence visuelle 7 maillons
  var _kcCC={};
  (kcData.active_ips||[]).forEach(function(ip){
    var sid=(ip.stage||'').toUpperCase(); if(!_kcCC[sid]) _kcCC[sid]={};
    var cc=ip.country||'-'; if(cc!=='-') _kcCC[sid][cc]=(_kcCC[sid][cc]||0)+1;
  });
  (kcData.probe_ips_15min||[]).forEach(function(ip){
    var cc=ip.country||'-'; if(cc==='-'||!cc) return;
    if(!_kcCC['PROBE']) _kcCC['PROBE']={};
    _kcCC['PROBE'][cc]=(_kcCC['PROBE'][cc]||0)+1;
  });
  (kcData.waf_ips_15min||[]).forEach(function(ip){
    var cc=ip.country||'-'; if(cc==='-'||!cc) return;
    if(!_kcCC['WAF']) _kcCC['WAF']={};
    _kcCC['WAF'][cc]=(_kcCC['WAF'][cc]||0)+1;
  });
  var topCC={};
  KCK_STAGES.forEach(function(s){
    var byC=_kcCC[s.id]||{};
  ...
  _kcDrawBeacon(ctx, nodes[0], nodeR, phase,
    {rgb:'255,59,92', xA:7, xB:nodes[0].x-nodeR-3, rgb2:nodes[0].s.rgb, p1:0, p2:1.2, rtl:false, labelTop:'◀ MENACES', labelBot:'INTERNET'});
  _kcDrawBeacon(ctx, nodes[nodes.length-1], nodeR, phase,
    {rgb:'0,255,136', xA:nodes[nodes.length-1].x+nodeR+3, xB:W-7, rgb2:'0,255,136', p1:0.8, p2:2.0, rtl:true, labelTop:'DÉFENSE ►', labelBot:'BOUCLIER'});
  _kcDrawTubes(ctx, nodes, nodeR, maxC);
  _kcDrawEscalation(ctx, nodes, nodeR, phase);
  _kcDrawParticles(ctx, nodes, nodeR, particles||[]);
...
function _kcInitParticles(kcData, f2bBanned){
  ...
  // Intensité animation = nombre d'EVENTS (hits cumulés) par stage, pas IPs uniques.
  // counts[stage] = IPs uniques (pour compteur affiché), intensity = events (pour animation).
  // Fix 2026-05-16 : restaure les particules visuelles que le dedup IPs avait calmées.
  var intensity={};
  (kcData.active_ips||[]).forEach(function(ip){
    var sid=(ip.stage||'').toUpperCase();
    intensity[sid]=(intensity[sid]||0)+(ip.count||1);
  });
  (kcData.probe_ips_15min||[]).forEach(function(e){
    intensity['PROBE']=(intensity['PROBE']||0)+(e.count||1);
  });
  (kcData.waf_ips_15min||[]).forEach(function(e){
    intensity['WAF']=(intensity['WAF']||0)+(e.count||1);
  });
  intensity['BLOCKED']=counts['BLOCKED']||0;
  KCK_STAGES.forEach(function(s,i){
    ...
    var intCur=intensity[s.id]||0;
    var intNext=intensity[KCK_STAGES[i+1].id]||0;
    ...
    // np basé sur INTENSITY (events) pas IPs uniques → animation reflète volume d'attaque
    var np=Math.min(10,Math.ceil(Math.max(intCur,intNext)/2));
    var stageCC=_kcIpsForCC[s.id]||[];
    for(var p=0;p<np;p++){
      _kcParticles.push({seg:i, t:Math.random(), speed:0.003+Math.random()*0.004, r:1.6+Math.random()*1.6, ghost:false, cc:stageCC.length?stageCC[p%stageCC.length]:''});
    }
  });
  return _kcParticles;
}
...
function animateKillChain(canvas, kcData, f2bData, csData){
  ...
  var f2bBanned=_f2bTotalBanned(f2bData);
  ...
  var _kcParticles=_kcInitParticles(kcData, totalBlocked);
  var phase=0;
  function frame(){
    if(!canvas.isConnected){_kcAnimFrame=null;return;}
    phase=(phase+0.004)%1;
    _kcParticles.forEach(function(p){p.t+=p.speed; if(p.t>1)p.t=0;});
    _drawKillChain(canvas,kcData,totalBlocked,phase,_kcParticles);
...
function _kcOpenStageModal(stageId, kcData){
  ...
  surCrit.forEach(function(a){
    ...
    var m=(a.signature||'').match(_CVE_RE);
  ...
  var stageRowsHtml=stageIps.map(function(ip){
    ...
    return '<div style="display:flex;align-items:center;gap:.4rem;padding:.28rem .42rem;margin-bottom:.22rem;background:rgba('+rgb+',0.05);border:1px solid rgba('+rgb+',0.14);border-radius:4px;cursor:pointer" data-kc-ip="'+esc(ip.ip)+'">'
      +'<span style="display:inline-block;width:1.8rem;text-align:center;flex-shrink:0;font-size:var(--fs-xs);color:rgba(200,230,255,.55)">'+(ip.country&&ip.country!=='-'?esc(ip.country):'')+'</span>'
      +'<span style="min-width:9.5rem;color:'+stageCol+';font-size:var(--fs-xs);font-weight:700;font-family:\'Courier New\',monospace">'+esc(ip.ip)+'</span>'
      ...
      +(ip.sur_count?'<span style="font-size:var(--fs-xs);background:rgba(255,96,48,0.2);color:'+_COL_SUR_IDS_HEX+';padding:.02rem .2rem;border-radius:2px">\u25c8IDS</span>':'')
      ...
      +((sub24[pfx]||0)>=2?'<span style="font-size:var(--fs-xs);background:rgba(255,215,0,.12);color:var(--yellow);border:1px solid rgba(255,215,0,.3);padding:.02rem .2rem;border-radius:2px">\u26a1/24</span>':'')
  ...
  var mh='<div style="font-family:\'Courier New\',monospace;font-size:var(--fs-xs)">'
   +'<div style="font-size:var(--fs-xs);color:var(--c-label);letter-spacing:.8px;margin-bottom:.5rem">'
  ...
  // Bindings CSP-safe — data-kc-ip · guard évite accumulation de listeners
  var b=document.getElementById('modal-body');
  if(b&&!b._kcClickBound){
    b._kcClickBound=true;
...
function _kcStageRgb(stage){
  var p=KC_PALETTE[(stage||'').toUpperCase()];
  return p?p.rgb:'0,217,255';
}
function _kcStageCol(stage){
  var p=KC_PALETTE[(stage||'').toUpperCase()];
  return p?p.color:_COL_CYAN_HEX;
}
...
function _kcBanIP(ipStr, btn){
  ...
  fetch(JV_URL+'/api/soc/ban-ip',{method:'POST',
    ...
    signal:AbortSignal.timeout(_PING_TMO_MS)})
...
function _kcCopyIP(ipStr,btn){
  ...
  var done=function(){ btn.textContent='✔ COPIÉ'; setTimeout(function(){btn.textContent=t;},_KC_COPY_RESET_MS); };
  if(navigator.clipboard&&window.isSecureContext){
    navigator.clipboard.writeText(ipStr).then(done).catch(function(){_kcCopyFallback(ipStr,done);});
  } else {
    _kcCopyFallback(ipStr,done);
  }
}
function _kcCopyFallback(ipStr,done){
  var ta=document.createElement('textarea');
  ta.value=ipStr; ta.style.position='fixed'; ta.style.opacity='0';
  document.body.appendChild(ta); ta.focus(); ta.select();
  try{ document.execCommand('copy'); done(); }catch(e){ /* execCommand deprecated — échec silencieux, fallback textarea */ }
  document.body.removeChild(ta);
}
function _kciCollectIp(ipStr,d){
  var kc=d.kill_chain||{};
  ...
  ((d.fail2ban||{}).jails||[]).forEach(function(j){
    (j.banned_ips||[]).forEach(function(e){if(e.ip)f2bSet[e.ip]=true;});
  });
  var activeIpObj=(kc.active_ips||[]).filter(function(x){return x.ip===ipStr;})[0]||null;
  var ipObj=activeIpObj||
    (csDetail[ipStr]?{ip:ipStr,stage:'BLOCKED',count:0,country:csDetail[ipStr].country||'-',cs_decision:true,sur_count:0}:null)||
    (f2bSet[ipStr]?{ip:ipStr,stage:'BLOCKED',count:0,country:'-',cs_decision:false,sur_count:0}:null)||
    {ip:ipStr,stage:'INCONNU',count:0,country:'-',cs_decision:false,sur_count:0};
  ipObj.banned=!!f2bSet[ipStr];
  var xhIp=((d.xhosts||{}).kill_chain_ips||{})[ipStr]||{};
  // router_seen / router_hits / router_out retirés 2026-05-17 — 
  if((!ipObj.f2b_vms||!ipObj.f2b_vms.length)&&xhIp.f2b_vms&&xhIp.f2b_vms.length)ipObj.f2b_vms=xhIp.f2b_vms;
  return {ipObj:ipObj,surInfo:surInfo,csInfo:csDetail[ipStr]||{},kc:kc,activeIpObj:activeIpObj};
}
function _kciScoreData(ipObj,activeIpObj,surInfo,ipStr,kc){
  var conf=0,confSrcs=[];
  if(activeIpObj){
  ...
  if(ipObj.sur_count){
    conf++;confSrcs.push({n:'SURICATA IDS ◈',v:surInfo.cat?_kcFmtCat(surInfo.cat):'×'+ipObj.sur_count+' alertes IDS',on:true,col:_COL_SUR_IDS_HEX});
  } else {
    confSrcs.push({n:'SURICATA IDS ◈',v:'Aucune alerte IDS',on:false,col:_COL_SUR_IDS_HEX});
  }
  if(ipObj.banned){
    conf++;confSrcs.push({n:'FAIL2BAN ⛔',v:'Banni — jail actif (srv-nginx)',on:true,col:'var(--red)'});
  } else {
    confSrcs.push({n:'FAIL2BAN ⛔',v:'Non banni sur srv-nginx',on:false,col:'var(--red)'});
  }
  // ROUTEUR retiré 2026-05-17
  if(ipObj.f2b_vms&&ipObj.f2b_vms.length){
    conf++;confSrcs.push({n:'FAIL2BAN VMs ⊛',v:'Banni sur '+(ipObj.f2b_vms||[]).join(' + '),on:true,col:_COL_AMBER_HEX});
  } else {
    confSrcs.push({n:'FAIL2BAN VMs ⊛',v:'Non banni sur VMs (clt/pa85)',on:false,col:_COL_AMBER_HEX});
  }
  // Source ROUTEUR retirée 2026-05-17 — seuils confiance ajustés (max 4 au lieu de 5)
  ...
  if(isBlocked){
    ...
    recTxt='⚡ Ban automatique déclenché — '+conf+'/5 sources (NGINX + Suricata IDS)';recCol='rgba(255,140,50,0.95)';
  } else {
    recTxt='◎ Confiance faible (1/5 source) — surveiller évolution';recCol='var(--cyan)';
  }
  return {conf:conf,confSrcs:confSrcs,confCol:confCol,confLbl:confLbl,subPeers:subPeers,subPfx:subPfx,recTxt:recTxt,recCol:recCol};
}
function _kciHtmlFragments(sc,surInfo,csInfo,ipStr){
  // 5 pips depuis retrait ROUTEUR 2026-05-17 ()
  var pipsHtml=[0,1,2,3,4].map(function(_pi){
    return '<div class="kci-conf-pip" style="background:'+(_pi<sc.conf?sc.confCol:'rgba(255,255,255,0.08)')+'"></div>';
  }).join('');
  var srcsHtml=sc.confSrcs.map(function(src){
  ...
  var scenarioHtml=csInfo.scenario
    ?'<div style="margin:.28rem 0;padding:.23rem .4rem;background:rgba(255,107,53,0.07);border-left:2px solid rgba(255,107,53,0.38);border-radius:0 3px 3px 0">'
      ...
      +'<span style="font-size:var(--fs-xs);color:var(--orange)">'+esc(fmtScenario(csInfo.scenario))+'</span>'
      +'</div>'
    :'';
  var surCatHtml=surInfo.cat
    ?'<div style="margin:.22rem 0;padding:.23rem .4rem;background:rgba(255,96,48,0.07);border-left:2px solid rgba(255,96,48,0.32);border-radius:0 3px 3px 0">'
      ...
      +'<span style="font-size:var(--fs-xs);color:'+_COL_SUR_IDS_HEX+'">'+esc(surInfo.cat)+'</span>'
      +'</div>'
    :'';
  var clusterHtml=sc.subPeers.length
    ?'<div class="kci-cluster-alert">'
      +'⚡ CAMPAGNE COORDONNÉE — réseau /24 → '+esc(sc.subPfx)+'.x — '
      +sc.subPeers.length+' autre'+(sc.subPeers.length>1?'s':'')+' IP'+(sc.subPeers.length>1?'s':'')+' actives : '
      +sc.subPeers.slice(0,5).map(function(x){return esc(x.ip);}).join(' · ')
      +(sc.subPeers.length>5?'  +'+(sc.subPeers.length-5)+' …':'')
      +'</div>'
    :'';
  return {pipsHtml:pipsHtml,srcsHtml:srcsHtml,cvesHtml:cvesHtml,scenarioHtml:scenarioHtml,surCatHtml:surCatHtml,clusterHtml:clusterHtml};
}
function _kcInvestigateIP(ipStr){
  var d=window._lastData||{};
  var col=_kciCollectIp(ipStr,d);
  var ipObj=col.ipObj,surInfo=col.surInfo,csInfo=col.csInfo,kc=col.kc,activeIpObj=col.activeIpObj;
  var stageCol=_kcStageCol(ipObj.stage);
  var rgb=_kcStageRgb(ipObj.stage);
  var sc=_kciScoreData(ipObj,activeIpObj,surInfo,ipStr,kc);
  var frags=_kciHtmlFragments(sc,surInfo,csInfo,ipStr);
  ...
  var mh='<div style="font-family:\'Courier New\',monospace;font-size:var(--fs-xs);line-height:1.55">'
   ...
   +'<div style="display:flex;align-items:center;gap:.55rem;flex-wrap:wrap;padding:.4rem .58rem;background:rgba('+rgb+',0.08);border:1px solid rgba('+rgb+',0.3);border-radius:5px;margin-bottom:.5rem">'
    ...
    +(activeIpObj&&activeIpObj.spoofed_bot?'<span style="background:rgba(255,0,80,0.22);color:'+_COL_C2_HEX+';padding:.07rem .26rem;border-radius:3px;font-size:var(--fs-xs);font-weight:700" title="User-Agent usurpé : prétend être '+esc(activeIpObj.spoofed_bot)+' mais FCrDNS échoue — signal hostile">⚠ FAUX '+esc(activeIpObj.spoofed_bot)+'</span>':'')
   ...
   +'<div style="margin-bottom:.48rem">'
    ...
    +'<span style="font-size:var(--fs-xs);font-weight:700;color:'+sc.confCol+'">'+sc.confLbl+' — '+sc.conf+'/5 sources</span>'
    +'</div>'
    +'<div class="kci-conf-bar">'+frags.pipsHtml+'</div></div>'
   +'<div class="kci-grid">'+frags.srcsHtml+'</div>'
   +frags.cvesHtml+frags.scenarioHtml+frags.surCatHtml+frags.clusterHtml
   +'<div style="font-size:var(--fs-xs);color:'+sc.recCol+';padding:.26rem .4rem;background:rgba(255,255,255,0.022);border-radius:3px;margin:.36rem 0;border:1px solid rgba(255,255,255,0.05)">'+sc.recTxt+'</div>'
   ...
   +'<div class="kci-actions">'
    ...
    +'<button class="kci-btn kci-btn-deep" data-kci-deep="1">⊙ APPROFONDI</button>'
    +'</div>'
   +'</div>';
  ...
  (function(ipS){ var b=document.getElementById('modal-body'); if(!b) return;
    ...
    var deepBtn=b.querySelector('[data-kci-deep]');
    if(deepBtn) deepBtn.addEventListener('click',function(){
      closeModal();
      setTimeout(function(){ if(window.openIpDeepModal) openIpDeepModal(ipS); },260);
    });
  })(ipStr);
