...
function _kcBanViaCrowdSec(btn,ipStr){
  ...
  if(!csKey){
    if(!btn){if(location.hostname==='localhost')console.warn('[SOC auto-ban] '+ipStr+' — CrowdSec LAPI key absente. Définir via localStorage.setItem("'+_LS_KEYS.CS_KEY+'","<clé>")');return;}
    _kcSetBtnErr(btn,'Clé CS absente — voir console');
    console.warn('[SOC] Clé CrowdSec LAPI manquante. Définir via : localStorage.setItem("'+_LS_KEYS.CS_KEY+'","<votre-clé>")');
    return;
  }
...
var KCK_STAGES = [
  {id:'RECON',   label:'RECON',     desc:'Sondage',       color:KC_PALETTE.RECON.color,   rgb:KC_PALETTE.RECON.rgb,   mitre:'T1595'},
  {id:'SCAN',    label:'SCAN',      desc:'Énumération',   color:KC_PALETTE.SCAN.color,    rgb:KC_PALETTE.SCAN.rgb,    mitre:'T1046'},
  {id:'EXPLOIT', label:'EXPLOIT',   desc:'Exploitation',  color:KC_PALETTE.EXPLOIT.color, rgb:KC_PALETTE.EXPLOIT.rgb, mitre:'T1190'},
  {id:'BRUTE',   label:'BRUTE',     desc:'Intrusion',     color:KC_PALETTE.BRUTE.color,   rgb:KC_PALETTE.BRUTE.rgb,   mitre:'T1110'},
  {id:'BLOCKED', label:'NEUTRALISÉ',desc:'Défense active',color:KC_PALETTE.BLOCKED.color, rgb:KC_PALETTE.BLOCKED.rgb, mitre:'DEF'},
];
...
function _kcInvestigateIP(ipStr){
  ...
  // Enrichir avec données rsyslog cross-host (router_seen/f2b_vms) pour IPs non-actives
  var _xhIp=((d.xhosts||{}).kill_chain_ips||{})[ipStr]||{};
  if(!ipObj.router_seen&&_xhIp.router_seen){ipObj.router_seen=_xhIp.router_seen;ipObj.router_hits=_xhIp.router_hits||0;ipObj.router_out=_xhIp.router_out||0;}
  if((!ipObj.f2b_vms||!ipObj.f2b_vms.length)&&_xhIp.f2b_vms&&_xhIp.f2b_vms.length)ipObj.f2b_vms=_xhIp.f2b_vms;
  ...
  if(ipObj.banned){
    conf++;confSrcs.push({n:'FAIL2BAN \u26d4',v:'Banni — jail actif (srv-ngix)',on:true,col:'var(--red)'});
  } else {
  ...
  if(isBlocked){
    ...
    recTxt='\u26a1 Ban automatique d\u00e9clench\u00e9 \u2014 '+conf+'/6 sources (NGINX + Suricata IDS)';recCol='rgba(255,140,50,0.95)';
  } else {
    recTxt='\u25ce Confiance faible (1/4 source)\u00a0— surveiller \u00e9volution';recCol='var(--cyan)';
  }
  // Build modal HTML — pre-vars
  var pipsHtml2=[0,1,2,3,4,5].map(function(_pi){
  ...
  var mh='<div style="font-family:\'Courier New\',monospace;font-size:var(--fs-xs);line-height:1.55">'
   ...
   +'<div style="margin-bottom:.48rem">'
    ...
    +'<span style="font-size:var(--fs-xs);font-weight:700;color:'+confCol+'">'+confLbl+' — '+conf+'/6 sources</span>'
    +'</div>'
  ...
  document.getElementById('modal-body').innerHTML=mh;
  // Binding CSP-safe — data-kci-copy + data-kci-deep
