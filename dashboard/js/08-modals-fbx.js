...
// Icône de règle auto-ban : lue directement depuis monitoring.json (champ rule_icon,
// source unique = AUTOBAN_RULES dans monitoring_gen.py). Fallback ◈ pour les anciens
// bans loggés avant 2026-05-14 (sans rule_icon).
...
function _fbxSecWanHtml(wm,wmSt2,stC2,stLbl2,fbx,hist,incidents){
  ...
  var incidentsListHtml=!incidents.length
    ...
    :incidents.slice().reverse().map(function(inc){
      ...
      var dur='';try{var ms3=new Date(inc.end).getTime()-new Date(inc.start).getTime();var mn=Math.round(ms3/60000);dur=mn>0?mn+' min':'<1 min';}catch(ee){}
...
function _fbxAuthBodyHtml(errDesc,isInvalid){
  return '<div style="font-family:\'Courier New\',monospace">'
   ...
   +'</div>'
   +(isInvalid
     ?'<div style="font-size:var(--fs-xs);color:rgba(180,200,220,0.6);margin-bottom:.9rem;line-height:1.6">La Freebox a perdu la référence de l\'application SOC Dashboard (réinitialisation, mise à jour firmware, ou expiration). Il faut re-générer le token d\'accès en lançant le script de pairing depuis srv-nginx.</div>'
   ...
   +(isInvalid?''
    ...
    +'<div><div style="font-size:var(--fs-xs);font-weight:700;color:rgba(180,200,220,0.85);margin-bottom:.25rem">Connexion SSH sur srv-nginx</div>'
...
function _fbxSecChartHtml(sl,hist,cuts24,cuts7d){
  if(!sl||!sl.chart_24h)return'';
  return '<div id="fbx-sec-chart" style="display:none">'
    +(hist.length>1
...
function openFbxModal(){
  ...
  var secChart=_fbxSecChartHtml(sl,hist,cuts24,cuts7d);
