...
function _fbxSecWanHtml(wm,wmSt2,stC2,stLbl2,fbx,hist,incidents){
  ...
  var incidentsListHtml=!incidents.length
    ...
    :incidents.slice().reverse().map(function(inc){
      ...
      return `<div style="display:flex;gap:.6rem;align-items:center;padding:.3rem 0;border-bottom:1px solid rgba(255,255,255,0.05);font-size:var(--fs-xs)"><span style="color:${ic};font-weight:700">${inc.status==='ONGOING'?'🔴 EN COURS':'⚠ RÉSOLU'}</span><span style="color:rgba(180,200,220,0.5)">${inc.start.replace('T',' ').slice(0,16)} UTC</span>${dur?`<span style="color:var(--cyan);margin-left:auto">${dur}</span>`:''}</div>`;
    }).join('');
  return `<div id="fbx-sec-wan"><div style="display:flex;gap:1rem;align-items:center;padding:.6rem .8rem;background:rgba(0,0,0,0.25);border:1px solid ${stC2}22;border-radius:4px;margin-bottom:.8rem"><span style="font-size:var(--fs-2xl);font-weight:700;color:${stC2}">${stLbl2[wmSt2]||wmSt2}</span><span style="font-size:var(--fs-md);color:var(--cyan);font-family:'Courier New',monospace">${fbx.ipv4||'—'}</span>${wm.uptime_24h!=null?`<span style="margin-left:auto;font-size:var(--fs-xs);font-weight:700;color:var(--green)">uptime 24h : ${wm.uptime_24h}%</span>`:''}</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.6rem;margin-bottom:.8rem">${probeGridHtml}</div>${latencyHtml}<div style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem;margin-bottom:.8rem">${sondesHtml}</div><div style="font-size:var(--fs-xs);color:var(--muted);text-transform:uppercase;letter-spacing:.7px;margin-bottom:.35rem">⚠ Incidents (${incidents.length})</div>${incidentsListHtml}<div style="margin-top:.7rem;padding:.5rem;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.06);border-radius:4px;font-size:var(--fs-xs);color:var(--muted)">💡 Fibre Free Delta — Les Sables d'Olonne · Panne : <span style="color:var(--cyan)">downdetector.fr/statut/free</span> · Assistance : <span style="color:var(--cyan)">3244</span></div></div>`;
}
...
function _fbxTempsHtml(temps,fans){
  ...
  if(fans.length){
    var fanCardsHtml=fans.map(function(f){
      ...
      return `<div style="padding:.35rem .45rem;background:rgba(0,0,0,0.3);border:1px solid ${fc}22;border-radius:4px"><div style="font-size:var(--fs-xs);color:var(--muted);margin-bottom:.12rem">${f.name||'?'}</div><div style="display:flex;align-items:center;gap:.5rem"><span style="font-size:var(--fs-sm);font-weight:700;color:${fc};line-height:1">${rpm}<span style="font-size:var(--fs-xs)"> RPM</span></span><span style="margin-left:auto">${barsHtml}</span></div>${rpm===0?'<div style="font-size:var(--fs-xs);color:var(--red);margin-top:.1rem">▲ ARRÊTÉ</div>':''}</div>`;
    }).join('');
...
function _fbxAuthBodyHtml(errDesc,isInvalid){
  return '<div style="font-family:\'Courier New\',monospace">'
   ...
   +(isInvalid?''
    ...
    +'<div style="padding:.35rem .5rem;background:rgba(0,0,0,0.35);border:1px solid rgba(255,255,255,0.08);border-radius:3px;font-size:var(--fs-xs);color:var(--cyan)">bash /opt/site01/setup-freebox-token.sh</div></div>'
    +'</div>'
    +'<div style="display:flex;gap:.7rem;align-items:flex-start;margin-bottom:.6rem">'
...
// NDT-47 : section SPIKES de _fbxTab
function _fbxFmtMbps(v){return(v||0).toFixed(2)+' Mbps';}
function _fbxSpikesHtml(spikes){
  var rows=spikes.slice().reverse().map(function(s){
    var col=s.tx_mbps>2.5||s.rx_mbps>2.5?'var(--red)':'var(--orange)';
    return `<tr style="border-bottom:1px solid rgba(255,255,255,0.05)"><td style="padding:.3rem .5rem;color:rgba(180,200,220,0.6);font-size:var(--fs-xs)">${_fbxFmtTs(s.ts)}</td><td style="padding:.3rem .5rem;color:var(--cyan);font-size:var(--fs-xs)">${_fbxFmtMbps(s.tx_mbps)}</td><td style="padding:.3rem .5rem;color:var(--green);font-size:var(--fs-xs)">${_fbxFmtMbps(s.rx_mbps)}</td><td style="padding:.3rem .5rem;color:rgba(180,200,220,0.4);font-size:var(--fs-xs)">moy TX:${_fbxFmtMbps(s.avg_tx_mbps)} / RX:${_fbxFmtMbps(s.avg_rx_mbps)}</td><td style="padding:.3rem .5rem"><span style="color:${col};font-size:var(--fs-xs)">⚡</span></td></tr>`;
  }).join('');
  return `<div style="padding:.4rem 0"><div style="font-size:var(--fs-xs);color:rgba(255,149,0,0.7);margin-bottom:.6rem;letter-spacing:.5px">PICS DE BANDE PASSANTE DÉTECTÉS — seuil 1.5 Mbps &amp; ×2 moyenne</div>${spikes.length===0?'<div style="color:rgba(180,200,220,0.35);font-size:var(--fs-xs);text-align:center;padding:1.5rem">Aucun pic détecté</div>':`<table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:.2rem .5rem;font-size:var(--fs-xs);color:rgba(180,200,220,0.4);font-weight:400;letter-spacing:.5px">HORODATAGE</th><th style="text-align:left;padding:.2rem .5rem;font-size:var(--fs-xs);color:rgba(0,217,255,0.5);font-weight:400;letter-spacing:.5px">TX ↓</th><th style="text-align:left;padding:.2rem .5rem;font-size:var(--fs-xs);color:rgba(0,200,100,0.5);font-weight:400;letter-spacing:.5px">RX ↑</th><th style="text-align:left;padding:.2rem .5rem;font-size:var(--fs-xs);color:rgba(180,200,220,0.3);font-weight:400;letter-spacing:.5px">BASELINE</th><th style="padding:.2rem .5rem"></th></tr></thead><tbody>${rows}</tbody></table>`}<div style="margin-top:.6rem;font-size:var(--fs-xs);color:rgba(180,200,220,0.3)">Rétention 7 jours — max 100 entrées — cycle détection 5 min</div></div>`;
}
...
function _fbxTab(name){
  ['wan','box','chart','spikes'].forEach(function(n){
  ...
  if(name==='spikes'){
    var spikes=(window._lastData&&window._lastData.net_spikes)||[];
    var sec=document.getElementById('fbx-sec-spikes');
    if(!sec)return;
    sec.innerHTML=_fbxSpikesHtml(spikes);
  }
  if(name==='chart'){
...
function openFbxAuthModal(){
  ...
  (function(){ var b=document.getElementById('modal-body'); if(!b) return;
    b.addEventListener('click',function(e){
      ...
      if(a==='copy-cmd') navigator.clipboard&&navigator.clipboard.writeText('bash /opt/site01/setup-freebox-token.sh');
...
function openFbxModal(){
  ...
  var secSpikes='<div id="fbx-sec-spikes" style="display:none"></div>';
  var body='<div style="font-family:\'Courier New\',monospace;padding:.2rem 0">'+secWan+secBox+secChart+secSpikes+'</div>';
  var mc2=document.getElementById('modal-card');
  ...
  if(_ht)_ht.innerHTML='<span>⬡ FREEBOX DELTA — BOX &amp; WAN</span>'
    ...
    +'<button id="fbx-btn-spikes" class="fbx-tab-btn" data-tab="spikes">⚡ ALERTES TRAFIC</button>'
    +'</div>';
