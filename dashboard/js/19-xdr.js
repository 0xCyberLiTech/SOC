// 19-xdr.js — XDR Modal : logigramme animé + timeline — v3.92.1 — 2026-04-15
...
function _css(){
  ...
  s.textContent=[
    ...
    '.xdr-lg{display:flex;align-items:stretch;justify-content:center;gap:6px;',
    ...
    /* ── Source nodes ── */
    '.xdr-lg-srcs{display:flex;flex-direction:column;justify-content:space-around;gap:5px;flex-shrink:0;width:180px;position:relative;z-index:1}',
    '.xdr-lg-src{display:flex;flex-direction:column;gap:2px;padding:5px 10px 4px;border-radius:4px;',
    ...
    '.xdr-pipe-col.collect{width:168px}',
    '.xdr-pipe-col.normalize{width:132px;justify-content:space-evenly}',
    '.xdr-pipe-col.correlate{width:158px;align-items:center;justify-content:center}',
    '.xdr-pipe-col.respond{width:132px;justify-content:space-evenly}',
...
var _SRCS = [
  ...
  {id:'msc', fam:'modsec',   lbl:'MODSEC/SITE01',  ico:'⚔', col:'#f77', bc:'rgba(255,100,100,.25)',bg:'rgba(255,100,100,.06)',wc:'msc',
   proc:'WAF Layer-7 · OWASP CRS', tags:'SQLi · XSS · LFI · RCE · CVE patch'},
  {id:'msp', fam:'modsec',   lbl:'MODSEC/SITE02', ico:'⚔', col:'#f77', bc:'rgba(255,100,100,.25)',bg:'rgba(255,100,100,.06)',wc:'msp',
   proc:'WAF Layer-7 · OWASP CRS', tags:'SQLi · XSS · LFI · RCE · CVE patch'},
...
var _FAM_SRC = {'fail2ban':'f2b','ufw':'ufw','apparmor':'aar','modsec_clt':'msc','modsec_pa85':'msp','suricata':'sur','autoban':'abn','nginx_drop':'ngx'};
var _FAM_C   = {fail2ban:'#f90',ufw:'#4af',apparmor:'#c7f',modsec:'#f77',autoban:'#0f8',suricata:'#ff4',nginx_drop:'#f0a'};
...
function _tsH(ts){var m=ts&&ts.match(/T(\d{2}:\d{2}:\d{2})/);return m?m[1]:ts?ts.slice(0,10):'—';}
...
function _jarvisOk(d){
  ...
  try{return localStorage.getItem('soc_ai')==='on';}catch(e2){return false;}
}
...
function _lgHtml(ev, ipMap, d){
  var cnt={f2b:0,ufw:0,aar:0,msc:0,msp:0,sur:0,abn:0,ngx:0};
  ...
  var collectHtml=_SRCS.map(function(s){
    ...
    if(active&&srcTypes[s.id]){
      ...
      topTypeHtml='<div class="xdr-lg-src-top" style="color:'+s.col+'">▸ '+types[0]+' ×'+srcTypes[s.id][types[0]]+(types.length>1?' +':'')+'</div>';
    }
  ...
  function _normNode(ico,lbl,sub,val,badge,col,bcol){
    // col = couleur texte/accent, bcol = couleur badge
    ...
    return '<div class="xdr-norm" style="border-top:2px solid '+cTop+';border-left:2px solid '+cLeft+';box-shadow:0 0 8px '+cGlow+'">'
      ...
      +(badge?'<span class="xdr-norm-bdg" style="color:'+bc+';background:'+bc.replace(/[\d.]+\)$/,'.1)')+';border:1px solid '+bc.replace(/[\d.]+\)$/,'.25)')+'">'+badge+'</span>':'')+'</div>'
  ...
  var topCorrHtml=msIps.slice(0,2).map(function(x){
    return '<div style="font:600 7px \'Courier New\';color:rgba(255,130,60,.55);margin-top:1px;white-space:nowrap">▸ '+x.ip.slice(-11)+' ×'+x.srcs.length+' src</div>';
  }).join('');
  ...
  var engHtml='<div class="xdr-lg-eng">'
    ...
    +'</div>'
    +(topSrc?'<div class="xdr-lg-eng-row"><span class="xdr-lg-eng-stat" style="color:'+topSrc.col+';opacity:.8">▲ '+topSrc.lbl+' <b>'+cnt[topSrc.id]+'</b></span></div>':'')
  ...
  if(jOk){
    jarvisRespHtml='<div class="xdr-resp on jarvis-on">'
      ...
      +'<div class="xdr-resp-val">'+jModel+' · '+jStats.bans_24h+' bans</div>'
      +'</div>';
  } else {
    jarvisRespHtml='<div class="xdr-joff-wrap" id="xdr-joff-wrap" data-xdr-joff-wrap>'
      ...
      +'<span class="xdr-jpop-txt">JARVIS tourne sur <b style="color:rgba(0,220,255,.8)">localhost:5000</b> (machine Windows).</span></div>'
      +'<div class="xdr-jpop-row"><span class="xdr-jpop-ico">🌐</span>'
      +'<span class="xdr-jpop-txt">Consulté depuis <b style="color:rgba(0,220,255,.8)">192.168.x.x</b> — navigateur ne peut pas atteindre localhost Windows.</span></div>'
      +'<div class="xdr-jpop-row"><span class="xdr-jpop-ico">✅</span>'
      +'<span class="xdr-jpop-txt">Ouvrir le dashboard depuis la machine Windows pour activer JARVIS.</span></div>'
      +'<a class="xdr-jpop-link" href="http://localhost:5000" target="_blank" data-xdr-jplink>→ Ouvrir JARVIS localhost:5000</a>'
      +'</div></div>';
  }
...
function _filter(){
  ...
  var vis=_xEv.filter(function(e){
    ...
    if(q&&(e.ip.indexOf(q)===-1)&&(e.detail.toLowerCase().indexOf(q)===-1)&&(e.src.indexOf(q)===-1))return false;
    return true;
  });
  ...
  for(var i=0;i<vis.length;i++){
    ...
    if(hr&&hr!==lastHr){lastHr=hr;html+='<div class="xdr-tsep">── '+hr+' ──────────────────────────</div>';}
    html+=_rowHtml(vis[i],_xIpMap,_xHl);
  }
  tl.innerHTML=html;
...
function _xdrOpenModal(d){
  ...
  var FAMS=['fail2ban','ufw','apparmor','modsec','autoban','suricata','nginx_drop'];
  ...
  // Open modal — bypass _isOpen guard en réinitialisant proprement
  if(typeof closeModal==='function'&&window._isOpen)closeModal();
  // Ouvrir directement l'overlay (contenu déjà injecté dans #modal-body)
  var _ov=document.getElementById('overlay');
  var _mc2=document.getElementById('modal-card');
  if(_ov&&_mc2){
    if(_mc2)_mc2.classList.add('modal-wide');
  ...
  (function(){
    ...
    fetch('http://localhost:5000/api/status',{signal:ctrl.signal})
