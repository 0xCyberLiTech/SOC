...
(function(){
'use strict';
var _CLICK_GUARD_MS = 10;   // garde anti-fermeture immédiate du popup au clic
var _XDR_ABORT_MS   = 2500; // abort fetch JARVIS status à l'ouverture modale

/* ══════════════════════════════════════════════════════════════════════════
   CSS
══════════════════════════════════════════════════════════════════════════ */
// ── CSS + données (avant fonction pour NDT-LONG) ────────────────────────────
var _XDR_CSS_ALL=[
    /* ── Modal body wrapper ── */
    ...
    '.xdr-lg-wire.f2b.active::after{color:'+XDR_PALETTE.fail2ban+'}',
    '.xdr-lg-wire.ufw.active::after{color:'+XDR_PALETTE.ufw+'}',
    '.xdr-lg-wire.aar.active::after{color:'+XDR_PALETTE.apparmor+'}',
    '.xdr-lg-wire.msc.active::after,.xdr-lg-wire.msp.active::after{color:'+XDR_PALETTE.modsec+'}',
    '.xdr-lg-wire.sur.active::after{color:'+XDR_PALETTE.suricata+'}',
    '.xdr-lg-wire.abn.active::after{color:'+XDR_PALETTE.autoban+'}',
    '.xdr-lg-wire.ngx.active::after{color:'+XDR_PALETTE.nginx_drop+'}',
    '.xdr-lg-wire.aid.active::after{color:'+XDR_PALETTE.aid+'}',
    '.xdr-lg-wire.vlm.active::after{color:'+XDR_PALETTE.vmf2b+'}',
    ...
    '.xdr-lg-eng-tot{font:700 26px "Courier New";color:'+XDR_PALETTE.aid+';line-height:1.1;',
    ...
    '.xdr-chip[data-f="fail2ban"]{color:'+XDR_PALETTE.fail2ban+';border-color:rgba(255,153,0,.3);background:rgba(255,153,0,.07)}',
    '.xdr-chip[data-f="ufw"]{color:'+XDR_PALETTE.ufw+';border-color:rgba(64,170,255,.3);background:rgba(64,170,255,.07)}',
    '.xdr-chip[data-f="apparmor"]{color:'+XDR_PALETTE.apparmor+';border-color:rgba(200,120,255,.3);background:rgba(200,120,255,.07)}',
    '.xdr-chip[data-f="modsec"]{color:'+XDR_PALETTE.modsec+';border-color:rgba(255,100,100,.3);background:rgba(255,100,100,.07)}',
    '.xdr-chip[data-f="autoban"]{color:'+XDR_PALETTE.autoban+';border-color:rgba(0,255,136,.3);background:rgba(0,255,136,.07)}',
    '.xdr-chip[data-f="suricata"]{color:'+XDR_PALETTE.suricata+';border-color:rgba(255,240,0,.3);background:rgba(255,240,0,.07)}',
    '.xdr-chip[data-f="nginx_drop"]{color:'+XDR_PALETTE.nginx_drop+';border-color:rgba(255,0,170,.3);background:rgba(255,0,170,.07)}',
    '.xdr-chip[data-f="aid"]{color:'+XDR_PALETTE.aid+';border-color:rgba(0,200,255,.3);background:rgba(0,200,255,.07)}',
    ...
    '.xdr-clr:hover{border-color:rgba(255,80,80,.5);color:'+_COL_XDR_CLR_HEX+';background:rgba(255,40,40,.05)}',
...
var _SRCS = [
  /* paire 1 — fail2ban */
  {id:'f2b', fam:'fail2ban', lbl:'FAIL2BAN',   ico:'🔒', col:XDR_PALETTE.fail2ban, bc:'rgba(255,153,0,.25)', bg:'rgba(255,153,0,.06)', wc:'f2b',
   proc:'Bans actifs · jail multi-services', tags:'ssh · http-auth · botsearch · nginx-cve'},
  {id:'vlm', fam:'vmf2b',     lbl:'F2B VMs',    ico:'⊛', col:_COL_AMBER_HEX, bc:'rgba(255,179,0,.25)', bg:'rgba(255,179,0,.06)', wc:'vlm',
   proc:'Fail2ban clt · fail2ban pa85', tags:'bans · ssh · apache · bruteforce'},
  /* paire 2 — WAF */
  {id:'msc', fam:'modsec',   lbl:'MODSEC/CLT',  ico:'⚔', col:XDR_PALETTE.modsec, bc:'rgba(255,100,100,.25)',bg:'rgba(255,100,100,.06)',wc:'msc',
   proc:'WAF Layer-7 · OWASP CRS', tags:'SQLi · XSS · LFI · RCE · CVE patch'},
  {id:'msp', fam:'modsec',   lbl:'MODSEC/PA85', ico:'⚔', col:XDR_PALETTE.modsec, bc:'rgba(255,100,100,.25)',bg:'rgba(255,100,100,.06)',wc:'msp',
   proc:'WAF Layer-7 · OWASP CRS', tags:'SQLi · XSS · LFI · RCE · CVE patch'},
  /* paire 3 — réseau\/système */
  {id:'ufw', fam:'ufw',      lbl:'UFW',         ico:'🛡', col:XDR_PALETTE.ufw, bc:'rgba(64,170,255,.25)',bg:'rgba(64,170,255,.06)',wc:'ufw',
   proc:'Firewall L4 · drop stateless', tags:'TCP · UDP · port scan · flood'},
  {id:'aar', fam:'apparmor', lbl:'APPARMOR',    ico:'⬡', col:XDR_PALETTE.apparmor, bc:'rgba(200,120,255,.25)',bg:'rgba(200,120,255,.06)',wc:'aar',
   proc:'MAC confinement · DENIED ops', tags:'file · network · exec · capability'},
  /* paire 4 — détection */
  {id:'sur', fam:'suricata', lbl:'SURICATA IDS', ico:'◈', col:XDR_PALETTE.suricata, bc:'rgba(255,240,0,.25)', bg:'rgba(255,240,0,.06)', wc:'sur',
   proc:'IDS réseau · 90k signatures', tags:'Sev1-CRIT · Sev2-HIGH · C2 · exploit'},
  {id:'ngx', fam:'nginx_drop', lbl:'NGX DROP',  ico:'⛔', col:XDR_PALETTE.nginx_drop, bc:'rgba(255,0,170,.25)', bg:'rgba(255,0,170,.06)', wc:'ngx',
   proc:'nginx return 444 · TCP drop silent', tags:'CVE-2024-4577 · PHP-RCE · RFI · path-traversal'},
  /* paire 5 — réponse autonome */
  {id:'abn', fam:'autoban',  lbl:'AUTOBAN',      ico:'⬢', col:XDR_PALETTE.autoban, bc:'rgba(0,255,136,.25)', bg:'rgba(0,255,136,.06)', wc:'abn',
   proc:'Réponse auto cron/5 min', tags:'R1-EXPLOIT · R2-FREQ · R3-HONEY · R4-BRUTE'},
  {id:'aid', fam:'aid',        lbl:'AID HIDS',   ico:'◉', col:XDR_PALETTE.aid, bc:'rgba(0,200,255,.25)', bg:'rgba(0,200,255,.06)', wc:'aid',
   proc:'Intégrité fichiers · diff HIDS', tags:'modified · added · removed · baseline'},
  /* paire 6 — cross-host rsyslog (rtr ROUTEUR retiré 2026-05-17 — ) */
  {id:'ap', fam:'apache', lbl:'APACHE VMs', ico:'⊚', col:_COL_APACHE_HEX, bc:'rgba(128,222,234,.25)', bg:'rgba(128,222,234,.06)', wc:'ap',
   proc:'Apache clt · Apache pa85 · rsyslog', tags:'errors ≥5 · 4xx · SCAN · bruteforce'},
];
var _FAM_SRC = {'fail2ban':'f2b','ufw':'ufw','apparmor':'aar','modsec_clt':'msc','modsec_pa85':'msp','suricata':'sur','autoban':'abn','nginx_drop':'ngx','aide':'aid','vmf2b':'vlm','apache':'ap'};
var _FAM_C   = XDR_PALETTE;
...
function _css(){
  ...
  s.textContent=_XDR_CSS_ALL.join('');
  document.head.appendChild(s);
}
function _ss(src){
  var m={
    fail2ban:   {c:XDR_PALETTE.fail2ban,  bc:'rgba(255,153,0,.3)', bg:'rgba(255,153,0,.08)',  lbl:'F2B'},
    ufw:        {c:XDR_PALETTE.ufw,  bc:'rgba(64,170,255,.3)',bg:'rgba(64,170,255,.08)', lbl:'UFW'},
    apparmor:   {c:XDR_PALETTE.apparmor,  bc:'rgba(200,120,255,.3)',bg:'rgba(200,120,255,.08)',lbl:'AAR'},
    modsec_clt: {c:XDR_PALETTE.modsec,  bc:'rgba(255,100,100,.3)',bg:'rgba(255,100,100,.08)',lbl:'MODSEC'},
    modsec_pa85:{c:XDR_PALETTE.modsec,  bc:'rgba(255,100,100,.3)',bg:'rgba(255,100,100,.08)',lbl:'MODSEC'},
    autoban:    {c:XDR_PALETTE.autoban,  bc:'rgba(0,255,136,.3)', bg:'rgba(0,255,136,.08)', lbl:'AUTOBAN'},
    suricata:   {c:XDR_PALETTE.suricata,  bc:'rgba(255,240,0,.3)', bg:'rgba(255,240,0,.08)', lbl:'SURICATA'},
    nginx_drop: {c:XDR_PALETTE.nginx_drop,  bc:'rgba(255,0,170,.3)', bg:'rgba(255,0,170,.08)', lbl:'NGX DROP'},
    aid:        {c:XDR_PALETTE.aid,  bc:'rgba(0,200,255,.3)', bg:'rgba(0,200,255,.08)', lbl:'AID'},
    vmf2b:      {c:XDR_PALETTE.vmf2b,  bc:'rgba(255,179,0,.3)', bg:'rgba(255,179,0,.08)', lbl:'F2B-VM'},
    apache:     {c:_COL_APACHE_HEX,bc:'rgba(128,222,234,.3)',bg:'rgba(128,222,234,.08)',lbl:'APACHE'},
  };
...
window._xdrToggleJarvisPop=function(evt){
  ...
  if(!isOpen){
    ...
    setTimeout(function(){document.addEventListener('click',_close);},_CLICK_GUARD_MS);
  } else {
    pop.classList.remove('open');
  }
};
...
// ── _lgHtml stage helpers ────────────────────────────────────────────────────
function _lgCollectHtml(cnt, srcTypes, maxCnt){
  return _SRCS.map(function(s){
    ...
    if(active&&srcTypes[s.id]){
      ...
      topTypeHtml='<div class="xdr-lg-src-top" style="color:'+s.col+'">&#x25b8; '+esc(types[0])+' \xd7'+srcTypes[s.id][types[0]]+(types.length>1?' +':'')+'</div>';
    }
...
function _lgNormalizeHtml(total, d, ev){
  ...
  var mitreTop=mitreKeys.slice(0,2).join(' \xb7 ')||'T1190 \xb7 T1595';
  return _normNode('📥','LOG PARSER','nginx \xb7 auth \xb7 syslog → JSON',total+' evt','OK','rgba(0,200,255,.75)','rgba(0,255,136,.85)')
    +_normNode('🌍','GEO / IP','country \xb7 ASN \xb7 risque',geoBlocks+' geo-bloqu\xe9s',(topCtry||'GEO'),'rgba(100,200,255,.7)','rgba(0,200,255,.8)')
    +_normNode('🔍','IOC / CTI','CrowdSec CTI \xb7 threat feeds',csDecis+' actifs',(csScen?csScen+' sc\xe9n':'CTI'),'rgba(180,120,255,.75)','rgba(180,120,255,.85)')
    +_normNode('🎯','MITRE ATT&CK','kill-chain \xb7 technique map',mitreTop,mitreKeys.length?mitreKeys.length+' tech':'MAP','rgba(255,200,60,.7)','rgba(255,200,60,.85)');
}
function _lgCorrelateHtml(msCount,msIps,total,ipCount,activeSrcs,sevC,tRange,topSrc,cnt,threatLvl,threatCol,threatBg,threatBc){
  var topCorrHtml=msIps.slice(0,2).map(function(x){
    return '<div style="font:600 7px \'Courier New\';color:rgba(255,130,60,.55);margin-top:1px;white-space:nowrap">▸ '+esc(x.ip.slice(-11))+' \xd7'+x.srcs.length+' src</div>';
  }).join('');
  var corrHtml=msCount>0
    ?'<div class="xdr-lg-eng-corr">⚡ \xd7'+msCount+' IP MULTI-SOURCE</div>'
    :'<div class="xdr-lg-eng-ok">✓ PAS DE CORRÉLATION</div>';
  return '<div class="xdr-lg-eng">'
    +'<div class="xdr-lg-eng-radar"><span class="xdr-lg-eng-cross">◈</span></div>'
    +'<div class="xdr-lg-eng-badge">X \xb7 D \xb7 R</div>'
    +'<div class="xdr-lg-eng-title">CORRELATION</div>'
    +'<div class="xdr-lg-eng-tot">'+total+'</div>'
    +'<div class="xdr-lg-eng-sub">'+ipCount+' IPs \xb7 '+activeSrcs.length+' SRCS \xb7 '+tRange+'</div>'
...
function _lgRespondHtml(d){
  ...
  if(jOk){
    jarvisRespHtml='<div class="xdr-resp on jarvis-on">'
      ...
      +'<div class="xdr-resp-sub">LLM \xb7 auto-ban \xb7 restart \xb7 TTS</div>'
      +'<div class="xdr-resp-val">'+esc(jModel)+' \xb7 '+jStats.bans_24h+' bans</div>'
      +'</div>';
  } else {
    jarvisRespHtml='<div class="xdr-joff-wrap" id="xdr-joff-wrap" data-xdr-joff-wrap>'
      ...
      +'<div class="xdr-jpop-row"><span class="xdr-jpop-ico">🌐</span>'
      +'<span class="xdr-jpop-txt">Consult\xe9 depuis <b style="color:rgba(0,220,255,.8)">'+SOC_INFRA.SRV_NGIX+'</b> — navigateur ne peut pas atteindre localhost Windows.</span></div>'
      +'<div class="xdr-jpop-row"><span class="xdr-jpop-ico">✅</span>'
  ...
  return _respNode('🛡','CROWDSEC BAN','auto-d\xe9cisions \xb7 bouncer nginx',csDecis+' IPs bannies',csBanState)
    +_respNode('🔒','FAIL2BAN','jail \xb7 iptables/nftables',f2bBans+' IPs \xb7 multi-h\xf4tes',f2bBans>0?'warn':'on')
    +jarvisRespHtml
    +_respNode('🔊','TTS / ALERT','vocal \xb7 seuil CRIT/HIGH \xb7 SOC engine',ttsState==='on'?'SOC engine actif':'inactif',ttsState);
}
function _lgHtml(ev, ipMap, d){
  ...
  var collectHtml  = _lgCollectHtml(cnt, srcTypes, maxCnt);
  var normHtml     = _lgNormalizeHtml(total, d, ev);
  var engHtml      = _lgCorrelateHtml(msCount,msIps,total,ipCount,activeSrcs,sevC,tRange,topSrc,cnt,threatLvl,threatCol,threatBg,threatBc);
  var respondHtml  = _lgRespondHtml(d);
...
/* ══════════════════════════════════════════════════════════════════════════
   OPEN MODAL
══════════════════════════════════════════════════════════════════════════ */
// ── _xdrOpenModal helpers ────────────────────────────────────────────────────
function _xdrModalChipsHtml(FAMS, sevC){
  ...
  return {chips:chipsHtml, sevs:sevHtml};
}
function _xdrModalBodyHtml(chipsHtml, sevHtml){
  return '<div class="xdr-bar">'
...
function _xdrJarvisRefresh(body){
  var ctrl=new AbortController();
  var tid=setTimeout(function(){ctrl.abort();},_XDR_ABORT_MS);
...
function _xdrOpenModal(d){
  ...
  var cs=_xdrModalChipsHtml(FAMS,sevC);
  ...
  if(!body)return;
  body.classList.add('xdr-modal-body');
  body.innerHTML='<div class="xdr-modal">'+_lgHtml(_xEv,_xIpMap,d)+_xdrModalBodyHtml(cs.chips,cs.sevs)+'</div>';
  _xdrModalBind(body);
