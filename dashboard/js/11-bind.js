...
(function(){
  ...
  if(btn) btn.addEventListener('click',function(e){pingJarvis(e);});
})();
...
document.addEventListener('keydown',function(e){
  if(e.key==='Escape'){
    ...
    if(pp&&pp.classList.contains('ip-dp--open')){pp.classList.remove('ip-dp--open');return;}
...
(function(){
  ...
  function _submitPopup(){
    ...
    if(!ip)return;
    if(popup)popup.classList.remove('ip-dp--open');
    if(window.openIpDeepModal)openIpDeepModal(ip);
  }
  function _togglePopup(){
    if(!popup)return;
    const visible=popup.classList.contains('ip-dp--open');
    if(visible)popup.classList.remove('ip-dp--open');else popup.classList.add('ip-dp--open');
  ...
  if(closeBtn)closeBtn.addEventListener('click',function(){if(popup)popup.classList.remove('ip-dp--open');});
  if(input)input.addEventListener('keydown',function(e){if(e.key==='Enter'){e.stopPropagation();_submitPopup();}});
  document.addEventListener('click',function(e){
    if(!popup||!popup.classList.contains('ip-dp--open'))return;
    if(popup.contains(e.target)||e.target===hdrBtn)return;
    popup.classList.remove('ip-dp--open');
  });
})();
...
// ── bindCards helper: close-hint ─────────────────────────────────────────────
function _bindCardCloseHint(card){
...
// ── bindCards helper: click handler per card type ─────────────────────────────
function _bindCardOnClick(card){
...
function bindCards(){
  document.querySelectorAll('#grid .card').forEach(function(card){
    _bindCardCloseHint(card);
    _bindCardOnClick(card);
  });
}

// ── Panneau info [data-panel] — fixe haut-droite · fermeture manuelle ──
function _initSocPanel(){
