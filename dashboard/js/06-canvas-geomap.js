...
function drawGeoMap(geoips, targetCanvas, wanInfo){
  ...
  geoips.forEach(function(ip){
    ...
    var color,rgb;
    if(stage==='BRUTE')      {color='#ff3b5c';rgb='255,59,92';}
    else if(stage==='EXPLOIT'){color='#ffd700';rgb='255,215,0';}
    else if(stage==='SCAN')  {color='#ff6b35';rgb='255,107,53';}
    else if(stage==='RECON') {color='#bf5fff';rgb='191,95,255';}
    else if(ip.blocked||csB) {color='#ff3b5c';rgb='255,59,92';}
    else if(ip.country==='FR'){color='#00ff88';rgb='0,255,136';}
  ...
  function frame(ts){
    ...
    if(animKey==='modal'){
      ...
      // Colonnes (base = _hc)
      var _col1w=Math.floor(W*0.20),_col2w=Math.floor(W*0.26),_col3w=Math.floor(W*0.28);
      ...
      var _wan=wanInfo&&wanInfo.ip?wanInfo.ip+(wanInfo.city?' · '+wanInfo.city:''):'—';
      var _z4hud=window._mapZone||'MONDE';
      var _outZone=_z4hud!=='MONDE'?dots.filter(function(d){return d.x<0||d.x>W||d.y<0||d.y>H;}).length:0;
      var _surveil=dots.length+' IPs · '+Object.keys(_atkMap).length+' pays'+(_outZone?' ('+_outZone+' hors zone)':'');
      var _hudStats=[
  ...
  canvas.onmousemove = function(e){
    ...
    if(hit&&_lfMapTip){
      ...
      var scenHtml=hit.cs_scenario?'<div class="mt-scen">'+esc(_kcFmtScenario(hit.cs_scenario))+'</div>':'';
...
var _prevAutoBanTs=localStorage.getItem(_LS_KEYS.PREV_BAN_TS)||'';  // persisté localStorage — survit aux rechargements
...
function _checkAutoBanAlerts(abl){
  ...
  localStorage.setItem(_LS_KEYS.PREV_BAN_TS,latestTs);
