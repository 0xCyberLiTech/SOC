...
function _gmHudHdr(ctx,hc,label,x,w){ctx.font='bold 6px "Courier New"';ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.62)';ctx.textAlign='left';ctx.textBaseline='top';ctx.fillText(label,x+8,hc+4);ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.18)';ctx.fillRect(x+8,hc+13,Math.min((w||80)-16,64),0.7);}
function _gmHudSep(ctx,hc,H,x){ctx.save();ctx.shadowBlur=4;ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.12)';ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.16)';ctx.lineWidth=0.5;ctx.beginPath();ctx.moveTo(x,hc+2);ctx.lineTo(x,H-4);ctx.stroke();ctx.restore();}
function _gmGetStaticLayer(W, H, dpr, _z, _zb){
  if(_mapStaticCanvas&&_mapStaticW===W&&_mapStaticH===H&&_mapStaticPoly===_worldPolygons&&_mapStaticZone===_z&&_mapStaticDpr===dpr){
    return _mapStaticCanvas;
  }
  ...
  bg.addColorStop(0,_COL_CANVAS_BG1_HEX); bg.addColorStop(0.5,_COL_CANVAS_BG2_HEX); bg.addColorStop(1,_COL_CANVAS_BG3_HEX);
  ...
  for(var _dy=8;_dy<H;_dy+=18){
    for(var _dx=8;_dx<W;_dx+=18){sctx.beginPath();sctx.arc(_dx,_dy,0.9,0,Math.PI*2);sctx.fill();}
  }
...
function _gmBuildDots(geoips, _zb, W, H){
  var dots=[];
  geoips.forEach(function(ip){
    ...
    else                     {color=_COL_CYAN_HEX;rgb=_COL_CYAN_RGB;}
...
function _gmDrawSweep(ctx, elapsed, W, H){
  var _swY=((elapsed*0.03)%H);
  ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.04)';ctx.fillRect(0,_swY,W,2);
  ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.015)';ctx.fillRect(0,_swY+2,W,4);
}
function _gmDrawCountryGlow(ctx, dots){
...
function _gmDrawFranceTarget(ctx, elapsed, frX, frY){
  ...
  [[14,0.65,1],[22,0.45,1],[34,0.22,0.7]].forEach(function(o,i){
    ...
    ctx.strokeStyle='rgba('+KC_PALETTE.BLOCKED.rgb+','+o[1].toFixed(2)+')';
  ...
  var ch=10;
  ctx.strokeStyle='rgba('+KC_PALETTE.BLOCKED.rgb+',0.7)';ctx.lineWidth=1;
  ...
  ctx.strokeStyle='rgba('+KC_PALETTE.BLOCKED.rgb+',0.28)';ctx.lineWidth=0.7;
  ...
  ctx.fillStyle=KC_PALETTE.BLOCKED.color;ctx.shadowColor=KC_PALETTE.BLOCKED.color;ctx.shadowBlur=14;ctx.fill();ctx.shadowBlur=0;
  ctx.font='bold 8px Courier New';ctx.fillStyle='rgba('+KC_PALETTE.BLOCKED.rgb+',0.65)';
  ctx.fillText('PROTECTED',frX+13,frY+3);
}
function _gmDrawAttackArcs(ctx, dots, frX, frY, elapsed){
  ...
  blocked.slice(0,25).forEach(function(d,idx){
    ...
    ctx.strokeStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+arcOpacity.toFixed(2)+')';ctx.lineWidth=1.0;ctx.stroke();
    ...
    for(var _pk=0;_pk<4;_pk++){
      ...
      ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+((1-_pk*0.22)*0.9*_pfade).toFixed(2)+')';
      ctx.shadowColor=KC_PALETTE.BRUTE.color;ctx.shadowBlur=_pk===0?8:2;ctx.fill();ctx.shadowBlur=0;
    }
  });
}
function _gmDrawCsArcs(ctx, dots, frX, frY, elapsed){
  ...
  csBanned.slice(0,15).forEach(function(d,idx){
    ...
    ctx.beginPath();ctx.moveTo(d.x,d.y);
    ctx.quadraticCurveTo(cpx,cpy,frX,frY);
    ctx.strokeStyle='rgba('+KC_PALETTE.SCAN.rgb+','+arcOpacity.toFixed(2)+')';ctx.lineWidth=1.3;ctx.setLineDash([4,3]);ctx.stroke();ctx.setLineDash([]);
    ...
    for(var _pk=0;_pk<3;_pk++){
      ...
      ctx.fillStyle='rgba('+KC_PALETTE.SCAN.rgb+','+((1-_pk*0.28)*0.95*_pfade).toFixed(2)+')';
      ctx.shadowColor=KC_PALETTE.SCAN.color;ctx.shadowBlur=_pk===0?10:2;ctx.fill();ctx.shadowBlur=0;
    }
  });
}
function _gmDrawStageArcs(ctx, dots, frX, frY, elapsed){
  // KC 7 maillons (2026-05-16) : ajout PROBE et WAF dans la palette d'arcs géographiques
  var STAGE_ARC_COLOR={'BRUTE':KC_PALETTE.BRUTE.rgb,'EXPLOIT':KC_PALETTE.EXPLOIT.rgb,'WAF':KC_PALETTE.WAF.rgb,'SCAN':KC_PALETTE.SCAN.rgb,'RECON':KC_PALETTE.RECON.rgb,'PROBE':KC_PALETTE.PROBE.rgb};
  var stageDots=dots.filter(function(d){return d.stage&&!d.cs_banned&&!d.blocked;});
  stageDots.sort(function(a,b){
    // Ordre croissant de gravité (PROBE bas, BRUTE haut) — les plus graves se dessinent au-dessus
    var so=['PROBE','RECON','SCAN','EXPLOIT','WAF','BRUTE'];
    return so.indexOf(b.stage)-so.indexOf(a.stage);
  });
  stageDots.slice(0,12).forEach(function(d,idx){
    var rgb=STAGE_ARC_COLOR[d.stage]||KC_PALETTE.RECON.rgb;
    var cpx=(d.x+frX)/2;
...
function _gmDrawKcCounter(ctx, elapsed, W){
  var _kc=(window._lastData&&window._lastData.kill_chain)||{};
  var _sc=_kc.stage_counts||{};
  // KC 7 maillons (2026-05-16) : compteur étendu — ordre desc gravité (les plus graves en haut)
  var _stages=[
    {k:'BRUTE',  sym:'⊗', rgb:KC_PALETTE.BRUTE.rgb},
    {k:'WAF',    sym:'◇', rgb:KC_PALETTE.WAF.rgb},
    {k:'EXPLOIT',sym:'◈', rgb:KC_PALETTE.EXPLOIT.rgb},
    {k:'SCAN',   sym:'⌖', rgb:KC_PALETTE.SCAN.rgb},
    {k:'RECON',  sym:'◎', rgb:KC_PALETTE.RECON.rgb},
    {k:'PROBE',  sym:'◍', rgb:KC_PALETTE.PROBE.rgb},
  ];
  ctx.save();
  ctx.font='bold 9px "Courier New"';
  var _stgW=84, _stgH=8+_stages.length*13+6, _stgX=W-_stgW-8, _stgY=8;
  ctx.fillStyle='rgba(2,8,20,0.68)';
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.12)';ctx.lineWidth=0.7;
  ctx.beginPath();ctx.rect(_stgX,_stgY,_stgW,_stgH);ctx.fill();ctx.stroke();
  ctx.font='bold 8px "Courier New"';ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.45)';
...
function _gmDrawTileBadges(ctx, wanInfo, _topAtk, _atkMax, W, H, elapsed){
  if(wanInfo&&wanInfo.ip){
    ...
    ctx.strokeStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+(0.28+wPulse*0.18)+')';ctx.lineWidth=0.8;
  ...
  if(_topAtk.length){
    ...
    ctx.save();
    ctx.fillStyle='rgba(2,8,20,0.72)';
    ctx.strokeStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.28)'; ctx.lineWidth=0.8;
    ...
    _topAtk.forEach(function(a,i){
      ...
      ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.12)';
      ctx.fillRect(_px+28,_ry,_bw,_rh-3);
      ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.55)';
...
function _gmHudCol1KC(ctx, _hc, _cx, _col1w, _scc, _maxStg, _hudStages){
...
function _gmHudCol2TopPays(ctx, _hc, _cx, _col2w, _topAtk, _atkMax){
  ...
  _topAtk.slice(0,5).forEach(function(a,i){
    ...
    if(bw>0){ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.2)';ctx.fillRect(_cx[1]+34,_by+1,bw,10);}
    ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.65)';if(bw>0)ctx.fillRect(_cx[1]+34+bw-2,_by+1,2,10);
    ctx.font='6.5px "Courier New"';ctx.fillStyle='rgba(255,160,170,0.7)';ctx.textAlign='right';
    ctx.fillText('×'+a.n,_cx[2]-8,_by+6);
  });
}
function _gmHudCol3Scenarios(ctx, _hc, _cx, _col3w, _topScen, _scenMax){
  ...
  if(_topScen.length){
    _topScen.forEach(function(sc,i){
      ...
      if(bw>0){ctx.fillStyle='rgba('+KC_PALETTE.SCAN.rgb+',0.12)';ctx.fillRect(_cx[2]+8,_by+1,bw,10);}
      ctx.fillStyle='rgba('+KC_PALETTE.SCAN.rgb+',0.6)';if(bw>0)ctx.fillRect(_cx[2]+8+bw-1.5,_by+1,1.5,10);
    ...
    ctx.font='6px "Courier New"';ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.3)';ctx.textAlign='left';ctx.textBaseline='middle';
    ctx.fillText('Aucun scénario actif',_cx[2]+8,_hc+40);
  }
}
function _gmHudCol4Protection(ctx, _hc, _cx, W, H, wanInfo, dots, _atkMap, _blkTot, _f2bOnly, _csOnly, _blkRate, _kr){
  ...
  var _hudStats=[
    ...
    {l:'Neutralisées',v:_blkTot+' / '+dots.length+' ('+_blkRate+'%)',c:KC_PALETTE.BRUTE.rgb},
    {l:'fail2ban',v:'×'+_f2bOnly,c:KC_PALETTE.SCAN.rgb},
...
function _gmDrawModalHud(ctx, dots, wanInfo, _topAtk, _atkMax, _atkMap, W, H, elapsed){
  ...
  ctx.save();ctx.shadowBlur=10;ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.2)';
  ...
  _igGrad.addColorStop(0,'rgba('+_COL_CYAN_RGB+',0.05)');_igGrad.addColorStop(1,'rgba('+_COL_CYAN_RGB+',0)');
  ...
  var _hudStages=[{k:'BRUTE',rgb:KC_PALETTE.BRUTE.rgb},{k:'EXPLOIT',rgb:KC_PALETTE.EXPLOIT.rgb},{k:'SCAN',rgb:KC_PALETTE.SCAN.rgb},{k:'RECON',rgb:KC_PALETTE.RECON.rgb}];
  ...
  _gmHudCol1KC(ctx,_hc,_cx,_col1w,_scc,_maxStg,_hudStages);
  _gmHudSep(ctx,_hc,H,_cx[1]);
  _gmHudCol2TopPays(ctx,_hc,_cx,_col2w,_topAtk,_atkMax);
  _gmHudSep(ctx,_hc,H,_cx[2]);
  _gmHudCol3Scenarios(ctx,_hc,_cx,_col3w,_topScen,_scenMax);
  _gmHudSep(ctx,_hc,H,_cx[3]);
  _gmHudCol4Protection(ctx,_hc,_cx,W,H,wanInfo,dots,_atkMap,_blkTot,_f2bOnly,_csOnly,_blkRate,_kr);
}
function _gmBindEvents(canvas, dots, _lfMapTip){
  canvas.onmousemove=function(e){
    ...
    if(hit&&_lfMapTip){
      ...
      var statusHtml=(hit.blocked||hit.cs_banned)?'<span style="color:var(--red)">■ BLOQUÉ</span>':'<span style="color:var(--green)">● AUTORISÉ</span>';
...
function drawGeoMap(geoips, targetCanvas, wanInfo){
  ...
  var sc=_gmGetStaticLayer(W,H,dpr,_z,_zb);
  var dots=_gmBuildDots(geoips,_zb,W,H);
  var _atkMap={};
  dots.filter(function(d){return d.blocked;}).forEach(function(d){_atkMap[d.country]=(_atkMap[d.country]||0)+d.count;});
  var _topAtk=Object.keys(_atkMap).map(function(k){return{cc:k,n:_atkMap[k]};}).sort(function(a,b){return b.n-a.n;}).slice(0,5);
  ...
  function frame(ts){
    ...
    if(ts-_mapLastDraw<33)return;
    _mapLastDraw=ts;
    var elapsed=ts-startT;
    var frX=_gmToX(-1.78,_zb,W), frY=_gmToY(46.50,_zb,H);
    ctx.setTransform(dpr,0,0,dpr,0,0);
    ctx.drawImage(sc,0,0,W,H);
    _gmDrawSweep(ctx,elapsed,W,H);
    _gmDrawCountryGlow(ctx,dots);
    _gmDrawFranceTarget(ctx,elapsed,frX,frY);
    _gmDrawAttackArcs(ctx,dots,frX,frY,elapsed);
    _gmDrawCsArcs(ctx,dots,frX,frY,elapsed);
    _gmDrawStageArcs(ctx,dots,frX,frY,elapsed);
    _gmDrawDots(ctx,dots,elapsed);
    _gmDrawKcCounter(ctx,elapsed,W);
    if(animKey!=='modal') _gmDrawTileBadges(ctx,wanInfo,_topAtk,_atkMax,W,H,elapsed);
    if(animKey==='modal') _gmDrawModalHud(ctx,dots,wanInfo,_topAtk,_atkMax,_atkMap,W,H,elapsed);
  }
  _mapAnimFrames[animKey]=requestAnimationFrame(frame);
  _gmBindEvents(canvas,dots,_lfMapTip);
}
...
function _geoModalKcBarsHtml(stCs,SC,stMax){
  return '<div class="geo-panel-hdr"><span class="geo-panel-ico">⊙</span>KILL CHAIN — ACTIF 15 MIN</div><div class="geo-kc-bars">'
...
function _geoModalCtryHtml(geoips,topCtry,cMax){
  return '<div class="geo-panel-hdr"><span class="geo-panel-ico">⊙</span>TOP PAYS</div><div class="geo-ctry-list">'
    +topCtry.slice(0,10).map(function(e){
        ...
        return '<div class="geo-ctry-row"><span class="geo-ctry-cc">'+esc(e.cc)+'</span>'
          ...
          +'<span class="geo-ctry-bpct" style="color:'+(bPct>70?'var(--red)':bPct>40?'var(--orange)':'var(--c-muted-4)')+'">'+bPct+'%</span>'
          +'</div>';
      }).join('')+'</div>';
}
function _geoModalScenHtml(topScen,sMax){
  return '<div class="geo-panel-hdr"><span class="geo-panel-ico">⊙</span>SCÉNARIOS CROWDSEC</div><div class="geo-scen-list">'
...
function _geoModalActHtml(activeIps,kc,SC){
  return '<div class="geo-panel-hdr"><span class="geo-panel-ico">⊙</span>ACTIFS — FENÊTRE '+(kc.window_minutes||15)+'MIN</div>'
    +(activeIps.length
...
function _geoModalIpTableHtml(geoips,SC){
  var sorted=geoips.slice().sort(function(a,b){return(b.count||0)-(a.count||0);}).slice(0,60);
  return '<div class="geo-panel-hdr"><span class="geo-panel-ico">⊙</span>TOP 60 IPs — 24H</div>'
...
function openGeoModal(){
  ...
  var SC={BRUTE:KC_PALETTE.BRUTE.color,EXPLOIT:KC_PALETTE.EXPLOIT.color,SCAN:KC_PALETTE.SCAN.color,RECON:KC_PALETTE.RECON.color};
  ...
  var statsH='<div class="geo-stats-bar">'
    ...
    +_geoStatBox('BLOQUÉES',blocked+'<span class="geo-stat-sub">('+rate+'%)</span>','var(--red)')
    +_geoStatBox('CROWDSEC BAN',csBanned,'var(--orange)')
    +_geoStatBox('PAYS DISTINCTS',topCtry.length,'var(--cyan)')
    +_geoStatBox('ACTIFS 15MIN',(kc.total_active||0),(kc.total_active||0)>5?'var(--red)':'var(--yellow)')
    +_geoStatBox('DÉCISIONS CS',(kc.cs_decisions||0),'var(--purple)')
    +'</div>';
  var cMax=topCtry[0]?topCtry[0].n:1;
  var sMax=topScen[0]?topScen[0].n:1;
  var kcH=_geoModalKcBarsHtml(stCs,SC,stMax);
  var ctryH=_geoModalCtryHtml(geoips,topCtry,cMax);
  var scenH=_geoModalScenHtml(topScen,sMax);
  var actH=_geoModalActHtml(activeIps,kc,SC);
  var ipH=_geoModalIpTableHtml(geoips,SC);
  var bodyH=statsH
