...
var _BAN_COOLDOWN_MS      = 15 * 60 * 1000; // cooldown ban/service auto (15min)
var _FOCUS_DELAY_MS       = 200;             // délai focus input après ouverture panel JARVIS
var _FETCH_RETRY_DELAY_MS = 5000;            // délai avant retry fetch JARVIS après erreur
var _FETCH_TMO_MS         = 10000;           // timeout fetch JARVIS (normal + retry)
...
function openPanel(){
  ...
  setTimeout(function(){ var inp=document.getElementById('jv-input'); if(inp)inp.focus(); },_FOCUS_DELAY_MS);
}
// ── Panel toggle ──────────────────────────────────────────────
...
window._jvCancel = function(){
  ...
  if(streaming){
    ...
    if(lbl) lbl.insertAdjacentHTML('beforeend',' <span class="jv-msg-cancelled">— annulé</span>');
...
function toggleEnabled(){
  if(_enabled && !confirm('Désactiver JARVIS AI ?')) return;
  _enabled = !_enabled;
  try{localStorage.setItem(_LS_KEYS.SOC_AI, _enabled ? 'on' : 'off');}catch(e){}
  updateBadge();
  updateFab();
...
function setOnline(val){
  ...
  if(jbd) jbd.classList.toggle('hidden', !val);
  ...
  if(jnode){
    if(val){
      ...
      if(jbdg)jbdg.innerHTML='<span class="dc-badge-ok">ACTIF</span>';
      jnode.classList.add('dc-node-active');
    } else {
      jnode.classList.add('dc-down');
      var jbdg2=jnode.querySelector('.dc-branch-bdg');
      if(jbdg2)jbdg2.innerHTML='<span class="dc-badge-down">DOWN</span>';
      jnode.classList.remove('dc-node-active');
    }
  }
}
...
function updateBadge(){
  ...
  b.title = !_enabled ? 'JARVIS AI désactivé — cliquer pour activer' : 'JARVIS AI actif — cliquer pour désactiver';
  var enBtn = document.getElementById('jv-enable-toggle');
  if(enBtn){
    enBtn.textContent = _enabled ? '■ ON' : '□ OFF';
    enBtn.className = 'jv-enable-btn' + (_enabled ? ' active' : '');
    enBtn.title = !_enabled ? 'JARVIS désactivé — cliquer pour activer' : 'JARVIS actif — cliquer pour désactiver';
  }
...
function updateStatusLbl(){
  ...
  if(!el) return;
  var _mdl=(window._dcJarvisState&&window._dcJarvisState.model)||'phi4:14b';
  el.textContent = _online ? _mdl+' · RTX 5080' : 'HORS LIGNE';
  el.classList.toggle('jv-model-online', _online);
  el.classList.toggle('jv-model-offline', !_online);
}
...
function checkNow(){
  ...
  .catch(function(e){ setOnline(false); });
}

// ── GPU tile live — màj depuis données JARVIS /api/stats, fusionne avec nvidia-smi ──
function updateGpuTile(data){
  var memTotal = data.mem_total||0;
  var memUsed  = data.mem_used||0;
  var pwrDraw  = data.power_draw||null;
  var pwrLimit = data.power_limit||null;
  var pwrPct   = (pwrDraw&&pwrLimit&&pwrLimit>0)?Math.round(pwrDraw*100/pwrLimit):null;
  // Fusionner avec la base nvidia-smi (perf_state, pcie, enc, dec, boost clocks, driver…)
  var baseGpu=(window._lastData&&window._lastData.windows_disk&&window._lastData.windows_disk.gpu)||{};
  var wdGpu=Object.assign({},baseGpu,{
    name:          data.name||baseGpu.name||'',
    usage:         data.gpu_util||0,
    temp:          typeof data.temp==='number'?data.temp:baseGpu.temp,
    vram_used_mb:  Math.round(memUsed*1024),
    vram_total_mb: Math.round(memTotal*1024),
    vram_pct:      memTotal>0?Math.round(memUsed*100/memTotal):(baseGpu.vram_pct||0),
    power_w:       pwrDraw!=null?pwrDraw:baseGpu.power_w,
    power_limit_w: pwrLimit!=null?pwrLimit:baseGpu.power_limit_w,
    power_pct:     pwrPct!=null?pwrPct:baseGpu.power_pct,
    fan_pct:       data.fan!=null?data.fan:baseGpu.fan_pct,
    clock_gpu_mhz: data.clk_gpu||baseGpu.clock_gpu_mhz
  });
  // Mettre à jour _lastData pour openGpuModal
  if(window._lastData){
    if(!window._lastData.windows_disk)window._lastData.windows_disk={available:true};
    window._lastData.windows_disk.gpu=wdGpu;
  }
  // Rendu via _gpuTileHtml (tuile arc de cercle — même code que 07-render.js)
  var h=typeof _gpuTileHtml==='function'?_gpuTileHtml(wdGpu):'';
  if(h){
    var existing=document.getElementById('win-gpu-card');
    if(existing) existing.outerHTML=h;
    else{var jvTile=document.getElementById('jv-tile');if(jvTile)jvTile.insertAdjacentHTML('beforebegin',h);}
  }
  // Historique GPU / VRAM / POWER (30s/point)
  ...
  _winPwrHistory.push(wdGpu.power_pct||0); if(_winPwrHistory.length>_winHistMax)_winPwrHistory.shift();
...
function _saveThresh(){ try{ localStorage.setItem(_LS_KEYS.JV_THRESH,JSON.stringify(_autoThresh)); }catch(e){} }
...
if(!_autoThresh.banStages)    _autoThresh.banStages    = ['EXPLOIT','BRUTE','SCAN'];
// Migration: SCAN ajouté aux stages auto-ban (IPs SCAN non protégées bannies automatiquement)
if(_autoThresh.banStages && _autoThresh.banStages.indexOf('SCAN')<0){
  _autoThresh.banStages.push('SCAN');
  _saveThresh();
}
...
var _autoState = {
  ...
  lastAlerts:       (function(){
    try{
      ...
      var now=Date.now(),cutoff=24*60*60*1000,clean={};
...
function _socRetryFetch(url, opts, onDone){
  ...
  if(!o.signal) o.signal=AbortSignal.timeout(_FETCH_TMO_MS);
  fetch(url, o)
    .then(function(r){return r.ok?r.json():Promise.reject(r.status);}).then(onDone)
    .catch(function(){
      setTimeout(function(){
        var o2=Object.assign({},o,{signal:AbortSignal.timeout(_FETCH_TMO_MS)});
        fetch(url, o2).then(function(r){return r.ok?r.json():Promise.reject(r.status);}).then(onDone)
          .catch(function(e){});
      }, _FETCH_RETRY_DELAY_MS);
    });
}
...
function checkDailySummary(){
  ...
  try{localStorage.setItem(_LS_KEYS.JV_LAST_SUMMARY, today);}catch(e){}
  ...
  streamFromJarvis(prompt, function(text){
    ...
    try{localStorage.setItem(_LS_KEYS.JV_SUMMARY_TEXT, text);}catch(e){}
    ensureDailyCard();
...
window._jvApplyPreset = function(name){
  ...
  _saveThresh();
  _syncThreshSliders();
  fetch(JV_URL+'/api/llm-params',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p.llm),signal:AbortSignal.timeout(_PING_TMO_MS)}).catch(function(e){ });
...
function buildThreshSection(){
  ...
  var presetHtml='<div class="jv-thresh-title">🎚 Profils rapides</div>'
    +'<div class="jv-preset-row">';
  Object.keys(_jvPresets).forEach(function(k){
    var p=_jvPresets[k];
    presetHtml+='<button id="jv-preset-'+k+'" class="jv-preset-btn" style="--jv-pcol:'+p.col+'">'+p.label+'</button>';
  });
  presetHtml+='</div>';
  sec.innerHTML = presetHtml
    ...
    +'</div>'
    +'<div class="jv-thresh-title jv-thresh-title--gap">⬡ Réponse proactive automatique</div>'
    +'<div class="jv-set-row">'
    +'<div class="jv-set-item"><div class="jv-set-lbl">Auto-ban SCAN/BRUTE — min req/15min <span id="tv-bmc">'+_autoThresh.banMinCount+'</span></div><input class="jv-set-range" id="tr-bmc" type="range" min="10" max="200" step="5" value="'+_autoThresh.banMinCount+'"></div>'
    ...
    +'<div class="jv-set-item"><div class="jv-set-lbl jv-set-lbl--orange">Auto-ban EXPLOIT CVE — min hits <span id="tv-exm">'+(_autoThresh.banExploitMin!==undefined?_autoThresh.banExploitMin:1)+'</span></div><input class="jv-set-range" id="tr-exm" type="range" min="1" max="10" step="1" value="'+(_autoThresh.banExploitMin!==undefined?_autoThresh.banExploitMin:1)+'"></div>'
    +'<div class="jv-set-item"><div class="jv-set-lbl jv-set-lbl--row">Auto-restart service DOWN <input type="checkbox" id="tr-rst" '+((_autoThresh.autoRestart)?'checked':'')+' class="jv-set-chk jv-set-chk--green"></div></div>'
    ...
    +'<div class="jv-set-item"><div class="jv-set-lbl jv-set-lbl--row">Voix Threat ÉLEVÉ/CRITIQUE <input type="checkbox" id="tr-tv" '+((_autoThresh.threatVoice)?'checked':'')+' class="jv-set-chk jv-set-chk--orange"></div></div>'
    +'</div>'
    +'<div class="jv-thresh-title jv-thresh-title--gap jv-thresh-title--voice">🔊 Voix automatique</div>'
    +'<div class="jv-set-row">'
    +'<div class="jv-set-item"><div class="jv-set-lbl">Score min → voix Threat <span id="tv-vms" class="jv-val-green">'+_autoThresh.voiceMinScore+'</span> <span id="tv-vms-lbl" class="jv-val-dim">'+(_autoThresh.voiceMinScore>=70?'CRITIQUE':_autoThresh.voiceMinScore>=50?'ÉLEVÉ':_autoThresh.voiceMinScore>=30?'MODÉRÉ':'QUASI-TOUT')+'</span></div>'
    +'<input class="jv-set-range" id="tr-vms" type="range" min="5" max="70" step="5" value="'+_autoThresh.voiceMinScore+'"></div>'
    +'<div class="jv-set-item"><div class="jv-set-lbl">Cooldown alertes <span id="tv-cdm" class="jv-val-green">'+_autoThresh.alertCooldownMin+'</span> min</div>'
    +'<input class="jv-set-range" id="tr-cdm" type="range" min="1" max="30" step="1" value="'+_autoThresh.alertCooldownMin+'"></div>'
    +'</div>'
    +'<div class="jv-thresh-footer">'
    +'<span class="jv-set-status" id="jv-thresh-status">✓ Seuils sauvés</span>'
    +'<button id="jv-thresh-save-btn" class="jv-set-save jv-set-save--danger">💾 Sauver seuils</button>'
    +'</div>';
  sp.appendChild(sec);
  _buildThreshBindings();
}
...
// ── Helper TTS durée — convertit '24h','48h','72h','168h' en texte parlé ──────
function _durToTTS(d){
  var _map={'24h':'vingt-quatre heures','48h':'quarante-huit heures',
            '72h':'soixante-douze heures','168h':'cent soixante-huit heures',
            '8760h':'un an'};
  if(_map[d]) return _map[d];
  var m=String(d).match(/^(\d+)h$/i);
  return m ? m[1]+' heures' : d;
}

function _buildCveMap(data){
  var surCveMap={};
  ((data.suricata||{}).recent_critical||[]).forEach(function(a){
    if(!a.src_ip) return;
    var m=(a.signature||'').match(/CVE-\d{4}-\d+/i);
    if(m) surCveMap[a.src_ip]=(surCveMap[a.src_ip]||[]);
...
function _buildSubnetMap(activeIps, geoMap, csDetail, LAN){
  var sub24count={};
...
function _banSuricataCrit(data, LAN, csDetail, now, svcCooldown){
  ...
  surRcrit.forEach(function(a){
    ...
    speakText('JARVIS alerte. Attaque réseau Suricata critique. Ban automatique '+_durToTTS(banDur)+'. IP '+ipSpeak+'.', true);
    _socRetryFetch(JV_URL+'/api/soc/ban-ip',{
      method:'POST',mode:'cors',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({ip:ip,reason:reason,duration:banDur})
    },function(res){
      var detail=(!res.ok&&res.result)?' — '+String(res.result).slice(0,60):'';
      updateJarvisTile('alerte',(res.ok?'✓ ':'✗ ')+'[SURICATA] Ban — '+ip+' '+banDur+detail);
      fetchSocActions();
...
function _buildCveSubnetFlags(ip, csDetail, surCveMap, sub24count){
  var isExploit = false;
  ...
  if(hasCve||hasCveScenario) isExploit=true;
  ...
  return{isExploit:isExploit,hasCve:hasCve,hasCveScenario:hasCveScenario,pfx:pfx,isCoordinated:isCoordinated};
}
...
function checkAutoBan(data){
  ...
  var stages        = _autoThresh.banStages||['EXPLOIT','BRUTE','SCAN'];
  var LAN=_LAN_RE;
  var svcCooldown=_BAN_COOLDOWN_MS;
  var now=Date.now();

  var geoMap={};
  ((data.traffic||{}).recent_geoips||[]).forEach(function(g){ if(g.ip) geoMap[g.ip]=g; });
  var csDetail=(data.crowdsec||{}).decisions_detail||{};
  var surCveMap=_buildCveMap(data);
  var sub24count=_buildSubnetMap(activeIps, geoMap, csDetail, LAN);
  ...
  activeIps.forEach(function(ipObj){
    ...
    var _f=_buildCveSubnetFlags(ip,csDetail,surCveMap,sub24count);
    var isExploit=_f.isExploit,hasCve=_f.hasCve,hasCveScenario=_f.hasCveScenario;
    var pfx=_f.pfx,isCoordinated=_f.isCoordinated;
    ...
    var msg = nhOnly
      ...
      : isCoordinated&&!hasCve&&!hasCveScenario
        ...
        : isExploit
          ? 'JARVIS alerte. Exploit CVE détecté. Ban automatique '+_durToTTS(banDur)+'. IP '+ipSpeak+'.'+(cveTag?' Scénario '+cveTag.replace(/\//g,' ')+'.'
          :'')
    ...
    _socRetryFetch(JV_URL+'/api/soc/ban-ip',{
      ...
      var detail=(!ok&&res.result)?' — '+String(res.result).slice(0,60):'';
      updateJarvisTile('alerte',(ok?'✓ ':'\u2717 ')+tag+'Ban — '+ip+' ['+ipObj.stage+'] '+banDur+detail);
      fetchSocActions();
  ...
  _banSuricataCrit(data, LAN, csDetail, now, svcCooldown);
}

function _buildReqHourCandidates(data, LAN, now){
  var activeIps=(data.kill_chain||{}).active_ips||[];
  var geoMap={};
  ((data.traffic||{}).recent_geoips||[]).forEach(function(g){ if(g.ip) geoMap[g.ip]=g; });
  var csDetail=(data.crowdsec||{}).decisions_detail||{};
  var banCooldown=_BAN_COOLDOWN_MS;
  ...
  return candidates.filter(function(ipObj){ return (ipObj.count||0) >= minHitsSpike; });
}
...
function checkReqPerHour(data){
  ...
  var cd = _ALERT_COOLDOWN_REQH_MS;
  if(_autoState.lastAlerts['reqh'] && (now - _autoState.lastAlerts['reqh']) < cd) return;
  _autoState.lastAlerts['reqh'] = now;
  try{localStorage.setItem(_LS_KEYS.JV_LAST_ALERTS, JSON.stringify(_autoState.lastAlerts));}catch(e){}

  // ── Ban auto : IPs kill chain non bannies lors d'un pic >500 req/h ──
  var LAN=_LAN_RE;
  var candidates = _buildReqHourCandidates(data, LAN, now);
  candidates.slice(0,3).forEach(function(ipObj){
    ...
    _socRetryFetch(JV_URL+'/api/soc/ban-ip',{
      ...
      var ok=res.ok;
      var detail=(!ok&&res.result)?' — '+String(res.result).slice(0,60):'';
      var spikeLabel=(ok?'✓ ':'\u2717 Échec ')+'Ban pic trafic — '+ip+' ['+ipObj.stage+']'+detail;
  ...
  if(sinceAnalysis > _autoState.analysisInterval){
    _autoState.lastAnalysisAt = now;
...
var _ALERT_COOLDOWN_REQH_MS=20*60*1000; // 20min cooldown alerte pic req/h
...
function _syncWhitelist(){
  if(!_online) return;
  fetch(JV_URL+'/api/soc/whitelist',{signal:AbortSignal.timeout(_PING_TMO_MS)})
...
function _syncPyBanned(){
  if(!_online) return;
  fetch(JV_URL+'/api/soc/recently-banned',{signal:AbortSignal.timeout(_PING_TMO_MS)})
    .then(function(r){ return r.json(); })
