...
window._jvCancel = function(){
  ...
  if(_abortCtrl){ try{ _abortCtrl.abort(); }catch(e){if(location.hostname==='localhost')console.error('[SOC]',e);} _abortCtrl=null; }
  // Stoppe le TTS
  stopSpeak();
  ...
  if(streaming){
    ...
    if(lbl) lbl.insertAdjacentHTML('beforeend',' <span style="color:var(--red);font-size:var(--fs-xs)">— annulé</span>');
...
function checkNow(){
  ...
  .catch(function(e){ if(location.hostname==='localhost')console.warn('[JARVIS] checkNow FAILED:', e && (e.name+': '+e.message)); setOnline(false); });
}
...
function _saveThresh(){ try{ localStorage.setItem(_LS_KEYS.JV_THRESH,JSON.stringify(_autoThresh)); }catch(e){if(location.hostname==='localhost')console.error('[SOC]',e);} }
    ...
    .catch(function(){
      setTimeout(function(){
        ...
        fetch(url, o2).then(function(r){return r.ok?r.json():Promise.reject(r.status);}).then(onDone)
          .catch(function(e){if(location.hostname==='localhost')console.warn('[SOC] retry failed:',url,e);});
      }, 5000);
    });
}
...
function streamFromJarvis(prompt, onDone){
  ...
  fetch(JV_URL+'/api/chat',{
    ...
    read();
  }).catch(function(e){ if(location.hostname==='localhost')console.warn('[JARVIS] streamFromJarvis failed:',e); });
}
...
function checkDailySummary(){
  ...
  streamFromJarvis(prompt, function(text){
    ...
    try{localStorage.setItem(_LS_KEYS.JV_SUMMARY_TEXT, text);}catch(e){if(location.hostname==='localhost')console.error('[SOC]',e);}
    ensureDailyCard();
...
window._jvApplyPreset = function(name){
  ...
  _saveThresh();
  _syncThreshSliders();
  fetch(JV_URL+'/api/llm-params',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p.llm),signal:AbortSignal.timeout(5000)}).catch(function(e){ if(location.hostname==='localhost')console.warn('[JARVIS] applyPreset llm-params failed:',e); });
...
function _syncWhitelist(){
  ...
  fetch(JV_URL+'/api/soc/whitelist',{signal:AbortSignal.timeout(5000)})
    ...
    .catch(function(e){ if(location.hostname==='localhost')console.warn('[JARVIS] _syncWhitelist failed:',e); });
}
...
function _syncPyBanned(){
  ...
  fetch(JV_URL+'/api/soc/recently-banned',{signal:AbortSignal.timeout(5000)})
    ...
    }).catch(function(e){ if(location.hostname==='localhost')console.warn('[JARVIS] _syncPyBanned failed:',e); });
}
