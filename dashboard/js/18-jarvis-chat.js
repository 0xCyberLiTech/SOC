...
// ── Quick prompts — items 0-9 ─────────────────────────────────────────────
function _buildPromptsCore(){
  return [
    ...
    {lbl:'🛡 Défenses',       fn: _jvQuickRoutine(
      'Contexte SOC complet disponible. Donne directement :\n'
      +'Fail2ban : IPs bannies total (4 hôtes : srv-nginx, Proxmox, clt, pa85), tentatives SSH bloquées.\n'
...
// ── Quick prompts — items 9-13 ────────────────────────────────────────────
function _buildPromptsExtra(){
  return [
    ...
    {lbl:'⏱ Uptime',          fn: _jvQuickRoutine(
      'Contexte SOC complet disponible. Donne directement :\n'
      +'srv-nginx : uptime exact, CPU %, RAM %.\n'
...
// ── Quick prompt chip builder ──────────────────────────────────────────────
function _buildPromptChip(p, qc){
...
function buildQuickPrompts(){
  ...
  if(!qc) return;
  var prompts = _buildPromptsCore().concat(_buildPromptsExtra());
  qc.innerHTML = '';
  prompts.forEach(function(p){ _buildPromptChip(p, qc); });
}
...
// ── Context: kill chain + top IPs ─────────────────────────────────────────
function _buildContextKillChain(d, kc, sc){
  var t = d.traffic||{};
  var lines = [
...
// ── Context: Freebox Delta ──────────────────────────────────────────────────
function _buildContextFreebox(fbx){
  var lines = ['', '--- FREEBOX DELTA ---'];
...
function buildContext(){
  ...
  _buildContextKillChain(d, kc, sc).forEach(function(l){ lines.push(l); });
  lines.push(
    '',
    '--- TRAFIC ---',
  ...
  if(Array.isArray(d.ssl)&&d.ssl.length) lines.push('', '--- CERTIFICATS SSL ---', d.ssl.map(function(s){return s.domain+' → '+s.days_left+'j restants';}).join(' | '));

  // ── Freebox Delta ──
  var fbx = d.freebox || d.fbx || {};
  if(fbx && fbx.available !== false){
    _buildContextFreebox(fbx).forEach(function(l){ lines.push(l); });
...
// ── Send message ──────────────────────────────────────────────
// L'historique part TEL QUEL : aucune incrustation de contexte SOC côté client.
// L'ancien procédé empilait un snapshot SOC dans CHAQUE message utilisateur —
// en multi-tours, les anciens snapshots périmés restaient dans l'historique et
// le LLM les mélangeait aux données fraîches. Désormais le serveur injecte le
// contexte frais dans le system prompt (jamais persisté) ; model_override='soc'
// force cette injection même sans mot-clé. Source de formatage unique : Python.
function _chatBuildPayload(msgs){
  return {history: msgs, soc_ctx_injected: false, num_predict: _socNumPredict(), model_override: 'soc'};
}
function _chatSseRead(reader, jarvisEl, msgsDiv, onChunk, onDone){
...
function sendMessage(text){
  ...
  var payload = _chatBuildPayload(_msgs);
  ...
  .then(function(r){
    ...
    _chatSseRead(r.body.getReader(), jarvisEl, msgsDiv,
      function(chunk){ fullText+=chunk; updateMsg(jarvisEl, fullText); },
      finalize);
  })
  .catch(function(e){
    removeThinking();
...
function appendMsg(role, text, streaming){
  ...
  var lbl = role==='user' ? 'VOUS' : 'JARVIS · '+esc(_jvActiveModel||'phi4');
...
function saveToHistory(label, text){
  ...
  try{ localStorage.setItem(_LS_KEYS.JV_HISTORY, JSON.stringify(_jvHistory)); }catch(e){}
  refreshHistPanel();
}
...
function stopSpeak(){
  ...
  _ttsRelease();
  fetch(JV_URL + '/api/speak/stop', {method:'POST', mode:'cors', signal:AbortSignal.timeout(3000)}).catch(function(e){ });
}
...
var _TTS_WDOG_EXTRA_MS = 10000; // buffer watchdog TTS au-delà du timeout (marge sécurité)
...
// ── AudioContext partagé — création différée au premier geste (autoplay policy) ──
var _jvAudioCtx = null;
var _jvAudioGestured = false;
function _getAudioCtx(){
  if(!_jvAudioGestured) return null;
  if(!_jvAudioCtx || _jvAudioCtx.state === 'closed'){
    _jvAudioCtx = new (window.AudioContext||window.webkitAudioContext)();
  }
  return _jvAudioCtx;
}
// Création + unlock au premier clic utilisateur
document.addEventListener('click', function(){
  _jvAudioGestured = true;
  try{
    if(!_jvAudioCtx || _jvAudioCtx.state === 'closed'){
      _jvAudioCtx = new (window.AudioContext||window.webkitAudioContext)();
    }
    _jvAudioCtx.resume();
  }catch(e){ /* AudioContext peut échouer selon la policy navigateur — ignoré */ }
...
function _ttsFlush(){
  ...
  _ttsWdog = setTimeout(function(){ _ttsRelease(); _ttsFlush(); }, _TTS_TMO + _TTS_WDOG_EXTRA_MS);
  // ── /api/tts → ArrayBuffer → AudioContext (ou <audio> si pas de geste) ──
  // JARVIS retourne edge-tts (Antoine) ou Piper si Antoine indisponible — jamais voix Windows
  ...
  .then(function(buf){
    ...
    if(!ctx){
      // AudioContext pas encore débloqué — lire l'audio JARVIS (edge/Piper) via <audio>
      if(_ttsWdog){ clearTimeout(_ttsWdog); _ttsWdog=null; }
      var blob = new Blob([buf], {type:'audio/mpeg'});
      var url  = URL.createObjectURL(blob);
      var a    = new Audio(url);
      var _cleanup = function(){
        a.src = ''; URL.revokeObjectURL(url); _cleanup = function(){};
        _ttsRelease(); _ttsFlush();
      };
      a.onended = function(){ _cleanup(); };
      a.onerror = function(){ _cleanup(); };
      a.play().catch(function(){ _cleanup(); });
      return;
    }
    return ctx.resume().then(function(){ return ctx.decodeAudioData(buf); });
  })
  .then(function(audioBuffer){
    if(!audioBuffer) return; // chemin <audio> déjà géré
    var ctx = _getAudioCtx();
    var src = ctx.createBufferSource();
    src.buffer = audioBuffer;
    src.connect(ctx.destination);
    src.onended = function(){ _ttsRelease(); _ttsFlush(); };
    // Annuler le watchdog dès que l'audio démarre — src.onended prend le relais
    if(_ttsWdog){ clearTimeout(_ttsWdog); _ttsWdog = null; }
    src.start(0);
  })
  .catch(function(){
    ...
    if(_ttsWdog){ clearTimeout(_ttsWdog); _ttsWdog=null; }
    // JARVIS indisponible — silence (jamais voix Windows)
    _ttsRelease(); _ttsFlush();
  });
}
...
function toggleTts(){
  ...
  try{localStorage.setItem(_LS_KEYS.SOC_AI_TTS, _ttsEnabled ? 'on' : 'off');}catch(e){}
