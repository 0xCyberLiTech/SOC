...
function _injectCss(){
  ...
  s.textContent = [
    ...
    '.aide-badge.alert{background:rgba(255,60,60,.12);color:'+_COL_AIDE_ALERT_HEX+';',
    ...
    '.aide-count-item.chg{background:rgba(255,160,0,.1);color:'+_COL_AIDE_WARN_HEX+';border:1px solid rgba(255,160,0,.3)}',
    '.aide-count-item.add{background:rgba(0,255,136,.08);color:var(--green);border:1px solid rgba(0,255,136,.25)}',
    '.aide-count-item.rem{background:rgba(255,60,60,.1);color:'+_COL_AIDE_ALERT_HEX+';border:1px solid rgba(255,60,60,.3)}',
    ...
    '.aide-m-row .val.alert{color:'+_COL_AIDE_ALERT_HEX+'}',
    '.aide-m-row .val.warn{color:'+_COL_AIDE_WARN_HEX+'}',
...
function _openModal(a){
  ...
  const h = '<div class="aide-modal">'
    ...
    + '<div class="aide-m-title">◉ Statut AIDE — srv-nginx (' + SOC_INFRA.SRV_NGIX + ')</div>'
  ...
  if(_ht) _ht.innerHTML = '<span style="margin-right:.45rem;opacity:.6">⬡</span>AIDE — INTÉGRITÉ FICHIERS · srv-nginx';
