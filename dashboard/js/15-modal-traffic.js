...
function _trRenderLive(){
  ...
  var csJailsHtml=csJails.length
    ?'<div style="margin-top:.35rem">'
      +csJails.slice(0,3).map(function(j){
        return `<div style="display:flex;justify-content:space-between;font-size:var(--fs-xs);margin-bottom:.15rem"><span style="color:var(--muted)">${esc(j.name)}</span><span style="color:var(--red);font-family:'Courier New',monospace">${fmt(j.active||j.current||0)} IPs</span></div>`;
      }).join('')+'</div>':'';
  var protosHtml=protos.length
    ?protos.slice(0,8).map(function(e){
        ...
        return `<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.3rem"><span style="min-width:3.8rem;font-size:var(--fs-xs);color:${pm.c};font-family:'Courier New',monospace">${esc(pm.l)}</span><div style="flex:1;height:5px;background:rgba(255,255,255,0.05);border-radius:2px;overflow:hidden"><div style="width:${pct}%;height:100%;background:${pm.c};box-shadow:0 0 5px ${pm.c};border-radius:2px;transition:width .5s ease"></div></div><span style="min-width:2.5rem;font-size:var(--fs-xs);color:var(--text);font-family:'Courier New',monospace;text-align:right">${fmt(v)}</span><span style="min-width:2rem;font-size:var(--fs-xs);color:var(--muted);text-align:right">${vpct}%</span></div>`;
      }).join('')
...
function openTrafficModal(d){
  ...
  var topCountriesHtml=tc.length
    ?tc.slice(0,10).map(function(c){
        ...
        return `<div style="margin-bottom:.25rem"><div style="display:flex;justify-content:space-between;font-size:var(--fs-xs);margin-bottom:.12rem"><span style="font-family:'Courier New',monospace;color:var(--text)">${esc(cn)}</span><span style="color:var(--muted);font-family:'Courier New',monospace">${fmt(cv)}</span></div><div class="pb-track"><div class="pb pb-c" style="width:${pct}%"></div></div></div>`;
      }).join('')
  ...
  var topPagesHtml=tp.length
    ?'<table><thead><tr><th>URI</th><th style="text-align:right">Hits</th></tr></thead><tbody>'
      +tp.slice(0,12).map(function(p){
          ...
          return `<tr><td style="color:var(--cyan);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(uri)}</td><td><div class="td-bar"><div class="td-bar-inner"><div class="td-bar-fill pb-c" style="width:${pct}%;height:3px"></div></div><span class="td-bar-num">${fmt(hits)}</span></div></td></tr>`;
        }).join('')+'</tbody></table>'
    :'<div style="color:var(--muted);font-size:var(--fs-xs);padding:.4rem">Aucune donnée disponible</div>';
