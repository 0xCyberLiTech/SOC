...
var _DC_MAIN=[
  ...
  },
  {
    ...
    lbl:'Suricata IDS',sub:'Analyse réseau · ~90k règles',
  ...
  },
  {
    ...
    role:'Analyse les logs nginx/SSH/Apache en temps réel. Bannit les IPs après N échecs via iptables/nftables. Couvre srv-ngix, Proxmox, SITE01 et SITE02.',
    ...
    getMetrics:function(d){
      ...
      return [
        ...
        {k:'SITE01',v:f.site01&&f.site01.available?fmt(f.site01.total_banned||0)+' bans':'N/A'},
        {k:'SITE02',v:f.site02&&f.site02.available?fmt(f.site02.total_banned||0)+' bans':'N/A'},
  ...
  },
  {
    ...
    role:'Serveur web principal. Termine les connexions TLS, route les requêtes vers les vhosts SITE01/SITE02, sert le dashboard SOC. Bouncer CrowdSec intégré comme module natif.',
    getOk:function(d){
      var svcs=d.services||{};
...
var _DC_BRANCHES=[
  ...
  {
    id:'site01',lbl:'SITE01',
    subs:[
      {
        id:'aa_clt',ico:'⬡',col:'rgba(0,160,200,.85)',
        lbl:'AppArmor',sub:'Apache2 SITE01 confiné',
        role:'Profil AppArmor enforce pour Apache2 sur VM SITE01. Limite Apache2 aux ressources nécessaires pour servir le site cybersécurité.',
  ...
  {
    id:'site02',lbl:'SITE02',
    subs:[
      {
        id:'aa_pa85',ico:'⬡',col:'rgba(120,80,200,.85)',
        lbl:'AppArmor',sub:'Apache2 SITE02 confiné',
        role:'Profil AppArmor enforce pour Apache2 sur VM SITE02. Confine Apache2 pour le site associatif SITE02 avec formulaires PHP (contact, RDV).',
        ...
        getMetrics:function(d){
          ...
          return [
            {k:'Mode',v:a.enforce===true?'enforce':a.available===false?'N/A':'complain'},
            {k:'Workers confinés',v:fmt(a.processes_confined||0)},
            {k:'VM',v:'site-02 — 192.168.x.x'}
          ];
        },
        getDeps:function(){return 'VM 107 — Apache2 + site associatif SITE02';}
      },
      {
        id:'ms_pa85',ico:'◈',col:'rgba(120,80,200,.75)',
        lbl:'ModSecurity',sub:'WAF OWASP CRS Apache',
        role:'WAF ModSecurity avec le Core Rule Set OWASP sur VM SITE02. Même protection que SITE01 — SQLi, XSS, LFI filtrés au niveau Apache2.',
...
function _dcPollJarvis(){
  ...
  function _poll(){
    ...
    fetch('http://localhost:5000/api/status',{signal:_ctrl.signal})
...
function _dcNodeHtml(def,d,maxLoad){
  ...
  return '<div class="'+cls+'" style="--dc-col:'+def.col+pulseStyle+'" data-dcid="'+def.id+'">'
   ...
   +'<div class="dc-node-sub">'+def.sub+'</div>'
   +'</div>'
