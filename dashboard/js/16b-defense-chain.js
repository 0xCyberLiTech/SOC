...
// v1.3.0 — 2026-05-17 — XDR 5 hôtes (router retiré : )
// v1.2.2 — 2026-05-08 — branche srv-dev-1 + XDR 6 hôtes
var _DC_ABORT_MS = 3000;  // abort AbortController fetch JARVIS status
var _DC_POLL_MS  = 30000; // intervalle poll JARVIS status
...
var _DC_CSS_ALL=[
    ...
    '.dc-label-top{color:var(--red);border:1px solid rgba(255,59,92,.3);background:rgba(255,59,92,.06);}',
    '.dc-label-bot{color:var(--green);border:1px solid rgba(0,255,136,.3);background:rgba(0,255,136,.06);}',
    ...
    '.dc-node.dc-down{border-left-color:var(--red)!important;animation:dc-pulse 1.8s ease-in-out infinite;}',
    '.dc-node.dc-na{border-left-color:'+_COL_DC_NA_HEX+'!important;opacity:.6;}',
    ...
    'border:1px solid rgba(0,217,255,.08);border-left:2px solid var(--dc-col,var(--cyan));',
    ...
    '.dc-branch-node.dc-down{border-left-color:var(--red)!important;animation:dc-pulse 1.8s ease-in-out infinite;}',
    ...
    '.dc-popup-close:hover{color:var(--red);opacity:1;}'
  ];
...
var _DC_MAIN=[
  ...
  },
  {
    ...
    role:'Analyse les logs nginx/SSH/Apache en temps réel. Bannit les IPs après N échecs via iptables/nftables. Couvre srv-nginx, Proxmox, CLT, PA85 et srv-dev-1.',
    ...
    getLoadNum:function(d){
      ...
      if(f.srv_dev1&&f.srv_dev1.available)tot+=f.srv_dev1.total_banned||0;
      return tot;
    },
    getVal:function(d){
      var f=d.fail2ban||{};
    ...
    getMetrics:function(d){
      var f=d.fail2ban||{};
      return [
        {k:'srv-nginx',  v:fmt(f.total_banned||0)+' bans · '+((f.jails||[]).length)+' jails'},
        ...
        {k:'SRV-DEV-1', v:f.srv_dev1&&f.srv_dev1.available?fmt(f.srv_dev1.total_banned||0)+' bans (sshd)':'N/A'},
  ...
  },
  {
    ...
    role:'Moteur de corrélation XDR : centralise les logs de srv-nginx, Proxmox, CLT, PA85 et srv-dev-1 via rsyslog. Corrèle les événements cross-hôtes pour détecter les IPs actives sur plusieurs vecteurs simultanément (kill chain multi-étapes, récidivistes). Alimente directement le score de menace global.',
    ...
    getMetrics:function(d){
      ...
      return [
        ...
        {k:'Hôtes couverts',v:'srv-nginx · proxmox · clt · pa85 · srv-dev-1'},
...
var _DC_BRANCHES=[
    ...
    subs:[
      {
        ...
        role:'Profil AppArmor enforce pour nginx sur srv-nginx. Confine les workers — restreint l\'accès aux fichiers, sockets et syscalls autorisés uniquement.',
      ...
      },
      {
        ...
        role:'AIDE surveille l\'intégrité des fichiers critiques sur srv-nginx (~49 945 entrées). Détecte toute modification non autorisée des configs nginx, CrowdSec, scripts SOC, SSH, fail2ban. Vérification cron 03h00 — alerte si différence.',
  ...
  {
    id:'srv_dev1',lbl:'SRV-DEV-1',
    subs:[
      {
        id:'dev1_ssh',ico:'◈',col:'rgba(0,200,255,.85)',
        lbl:'SSH isolé',sub:'port <SSH-PORT> · clé id_dev · JARVIS CODE',
        role:'Accès SSH restreint à la clé id_dev uniquement (port <SSH-PORT>). Seul JARVIS MODE CODE peut y écrire et exécuter des scripts. VM isolée de toute production — zéro exposition publique.',
        getOk:function(d){var s=d.rsyslog_srv_dev1||d.srv_dev1||{};return s.available!==false;},
        getVal:function(){return 'id_dev';},
        getMetrics:function(){
          return [
            {k:'VM',v:'srv-dev-1 — '+SOC_INFRA.SRV_DEV1},
            {k:'Port SSH',v:'<SSH-PORT> · clé id_dev'},
            {k:'Usage',v:'JARVIS MODE CODE — scripts/debug'},
            {k:'Isolation',v:'prod isolée — zéro route publique'}
          ];
        },
        getDeps:function(){return 'JARVIS CODE mode · qwen2.5-coder:14b · SCP/SSH whitelisté';}
      },
      {
        id:'dev1_snap',ico:'⬡',col:'rgba(255,170,0,.85)',
        lbl:'Sandbox Proxmox',sub:'VM isolée · snapshot DR',
        role:'VM Debian dédiée sur Proxmox (VMID 101 — 1 vCPU · 1 Go RAM · 10 Go). Isolée de toute VM de production. Snapshot Proxmox = recovery instantané si casse. Carte blanche JARVIS CODE.',
        getOk:function(){return true;},
        getVal:function(){return 'VMID 101';},
        getMetrics:function(){
          return [
            {k:'VMID',v:'101'},
            {k:'Ressources',v:'1 vCPU · 1 Go RAM · 10 Go disk'},
            {k:'OS',v:'Debian 12'},
            {k:'DR',v:'snapshot Proxmox — recovery instantané'},
            {k:'Backup',v:'D:\\BACKUP-PROXMOX\\auto\\srv-dev-1'}
          ];
        },
        getDeps:function(){return 'Proxmox VE '+SOC_INFRA.PROXMOX+' · backup auto inclus';}
      }
    ]
  }
];

// ── Ping JARVIS (côté navigateur — localhost:5000 inaccessible depuis srv-nginx) ──
...
function _dcInjectCss(){
  if(_dcCssInjected)return;_dcCssInjected=true;
  var s=document.createElement('style');
  s.textContent=_DC_CSS_ALL.join('');
...
function _dcPollJarvis(){
  ...
  function _poll(){
    var _ctrl=new AbortController();setTimeout(function(){_ctrl.abort();},_DC_ABORT_MS);
...
function _dcRenderTile(d,g){
  ...
  var h='<div class="card wide" id="defense-chain-card">'
   ...
   +'</div>'
   +'<div class="dc-wrap">'
   +'<div class="dc-label-top">⬡ INTERNET — TRAFIC ENTRANT'
   +((d.nginx_stats&&d.nginx_stats.req_24h)?'&nbsp;<span style="opacity:.65;font-weight:400">· '+fmt(d.nginx_stats.req_24h)+' req/24h</span>':'')
   +'<br><span style="font-size:.65rem;opacity:.6;font-weight:400;letter-spacing:.5px">srv-nginx &middot; clt &middot; pa85</span></div>'
