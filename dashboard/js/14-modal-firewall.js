function openFirewallModal(d){
  ...
  var _genTs=isNaN(_g.getTime())?'—':_g.getFullYear()+'-'+String(_g.getMonth()+1).padStart(2,'0')+'-'+String(_g.getDate()).padStart(2,'0')+' '+String(_g.getHours()).padStart(2,'0')+':'+String(_g.getMinutes()).padStart(2,'0')+':'+String(_g.getSeconds()).padStart(2,'0');
  var hostCardsHtml=['srv-ngix','site01','site02','proxmox'].map(function(n){
    var hh=hm[n]||{name:n};
    ...
    var bodyHtml=hh.error
      ?`<div style="color:var(--red);font-size:var(--fs-xs)">⚠ ${hh.error}</div>`
      :`<div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:.18rem"><div><div style="font-size:var(--fs-xl);font-weight:700;font-family:'Courier New',monospace;color:${scc};line-height:1;text-shadow:0 0 10px ${scc}">${sc}</div><div style="font-size:var(--fs-xs);color:var(--muted);text-transform:uppercase;letter-spacing:.4px">SCORE</div></div>${ufwBadge}</div>`
        ...
        +(hh.issues||[]).map(function(iss){return`<div style="font-size:var(--fs-xs);color:var(--red)">▸ ${iss}</div>`;}).join('');
    return `<div class="fw-hc" style="border-color:${bc}"><div style="font-size:var(--fs-xs);font-weight:700;font-family:'Courier New',monospace;color:${hcol[n]}">${n}</div><div style="font-size:var(--fs-xs);color:var(--muted);margin-bottom:.35rem">${hh.ip||''}${hh.role?' · '+hh.role:''}</div>${bodyHtml}</div>`;
  }).join('');
  var _known=[80,443,2272,8080,8006,587,25,22,6060,6061,8089,7422,3128,85];
  var _anomPorts=[];
  (hosts||[]).forEach(function(hh){(hh.listening_ports||[]).forEach(function(p){if(_known.indexOf(p)<0){_anomPorts.push((hh.host||hh.ip||'?')+':'+p);}});});
