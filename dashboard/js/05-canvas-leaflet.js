...
var _lfArcData=[], _lfParticleCanvas=null, _lfParticleFrame=null, _lfClockInt=null;
var _lfHeatData=[], _lfWanLatG=46.4978, _lfWanLonG=-1.7831;
var _lfHistBuf=[];
var _hmHover={idx:-1};
// ── Config heatmap attaques — source unique (no-hardcode) ───────────────
// Tous les paramètres visuels de l'overlay rouge regroupés ici. Changement
// d'un seul endroit pour toute la chaîne (drawAttackHeatmap → _hmDrawHours).
var _HM_ATK_CFG={
  overlayRatio: 0.15,   // % de pH max pour overlay rouge en mode 'combined'
  minPx:        3,      // hauteur minimum garantie quand d.atk>0
  alphaBase:    0.55,   // alpha base barre rouge non-pic
  alphaAmp:     0.18,   // amplitude pulsation alpha (±)
  peakAlphaBase:0.70,   // alpha base pic d'attaque
  peakAlphaAmp: 0.20,   // amplitude pulsation alpha pic
  glowBase:     6,      // shadowBlur de base pic
  glowAmp:      10,     // amplitude pulsation glow pic
  pulsePeriodS: 2.4     // période pulsation visible (synchro livePhase)
};
// _lastThreatResult déclaré dans 01-utils.js (NDT-99 — partagé 07-render/05-leaflet)
var _LF_COL_RGB={};
_LF_COL_RGB[_COL_YELLOW_HEX]='255,215,0';_LF_COL_RGB[_COL_RED_HEX]='255,59,92';
_LF_COL_RGB[_COL_ORANGE_HEX]='255,107,53';_LF_COL_RGB[_COL_PURPLE_HEX]='191,95,255';
_LF_COL_RGB[_COL_GREEN_HEX]='0,255,136';_LF_COL_RGB[_COL_CYAN_HEX]=_COL_CYAN_RGB;
...
// _rcFmtV : helper format SI/k partagé (utilisé par heatmap attaques tooltip).
...
function _leafletStageColor(g){
  ...
  if(g.blocked||g.cs_banned)return _COL_RED_HEX;
  if(g.country==='FR')return _COL_GREEN_HEX;
  return _COL_CYAN_HEX;
}
...
var _lfStartT=0,_lfLastDraw=0;
function _lfPframeRings(ctx,W,H){
...
if(_wanPt){
  [[1000,'rgba('+_COL_CYAN_RGB+',.07)'],[3000,'rgba('+_COL_CYAN_RGB+',.045)'],[6000,'rgba('+_COL_CYAN_RGB+',.025)']].forEach(function(rr){
    var km=rr[0];
    var dlat=km/111;
...
function _lfPframe(ts){
  if(!_lfMap||!_lfActive){_lfParticleFrame=null;return;}
  _lfParticleFrame=requestAnimationFrame(_lfPframe);
  if(ts-_lfLastDraw<30)return;
  _lfLastDraw=ts;
  ...
  var elapsed=(ts-_lfStartT)*0.001;
...
function _lfStartParticles(){
  ...
  _lfStartT=performance.now();
  requestAnimationFrame(_lfPframe);
}
...
function _buildLeafletArcs(geoips, wanLat, wanLon){
  ...
  sorted.forEach(function(g){
    ...
    _lfArcData.push({lat1:g.lat,lon1:g.lon,lat2:wanLat,lon2:wanLon,cpLat:_cpLat,cpLon:_cpLon,color:col,rgb:_LF_COL_RGB[col]||_COL_CYAN_RGB,stage:stage,period:_per,pSize:_psz,nParts:_npt,phase:_ipHash(g.ip||'')});
  });
...
function _lfDrawGauge(lvl,lvlC,_gs){
  ...
  for(var ti=0;ti<12;ti++){
    ...
    ctx.strokeStyle=ti%3===0?'rgba('+_COL_CYAN_RGB+',.22)':'rgba('+_COL_CYAN_RGB+',.1)';ctx.lineWidth=1;ctx.stroke();
  }
  // Arc fond
  ctx.beginPath();ctx.arc(cx,cy,R,-Math.PI/2,Math.PI*1.5);
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',.07)';ctx.lineWidth=6;ctx.lineCap='round';ctx.stroke();
  // Arc menace
  if(_gs>0){
  ...
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',.04)';ctx.lineWidth=1;ctx.stroke();
  // Point central
...
function _lfDrawSparkline(){
  ...
  if(_lfHistBuf.length>1){
    ...
    // Grille pointillée
    sctx.setLineDash([2,4]);sctx.strokeStyle='rgba('+_COL_CYAN_RGB+',.06)';sctx.lineWidth=0.5;
    ...
    grd.addColorStop(0,'rgba('+_COL_CYAN_RGB+',.16)');grd.addColorStop(1,'rgba('+_COL_CYAN_RGB+',.01)');
    sctx.fillStyle=grd;sctx.fill();
    // Ligne
    sctx.beginPath();
    vals.forEach(function(v,i){var x=2+i*stpV,y=sH-3-Math.round((v-mn)/rngV*(sH-7));i===0?sctx.moveTo(x,y):sctx.lineTo(x,y);});
    sctx.strokeStyle='rgba('+_COL_CYAN_RGB+',.75)';sctx.lineWidth=1.2;sctx.shadowColor='rgba('+_COL_CYAN_RGB+',.45)';sctx.shadowBlur=3;sctx.stroke();sctx.shadowBlur=0;
    // Dot dernier point
    sctx.beginPath();sctx.arc(lxV,lyV,2,0,Math.PI*2);sctx.fillStyle='rgba('+_COL_CYAN_RGB+',1)';sctx.shadowColor='rgba('+_COL_CYAN_RGB+',1)';sctx.shadowBlur=4;sctx.fill();sctx.shadowBlur=0;
    // Label valeur max
    sctx.font='5px Courier New';sctx.fillStyle='rgba('+_COL_CYAN_RGB+',.45)';sctx.textAlign='right';
    sctx.fillText(_lfFmtN(vals[vals.length-1]),sW-2,lyV>6?lyV-3:lyV+7);
  } else {
    sctx.font='5px Courier New';sctx.fillStyle='rgba('+_COL_CYAN_RGB+',.22)';sctx.textAlign='center';
...
function _buildLfLeftPanel(geoips,stages,total,lvl,lvlC,blkPct,totalReqs){
  ...
  if(sd){
    sd.innerHTML=
      '<div class="lf-lp-stat"><span class="lf-lp-stat-l">HOSTILES ACTIFS</span><span class="lf-lp-stat-v lf-v-red">'+total+'</span></div>'
      +'<div class="lf-lp-stat"><span class="lf-lp-stat-l">IOC ACTIFS</span><span class="lf-lp-stat-v lf-v-yellow">'+ioc+'</span></div>'
      +'<div class="lf-lp-stat"><span class="lf-lp-stat-l">VECTEURS/MIN</span><span class="lf-lp-stat-v lf-v-orange">'+rateMin+'</span></div>'
      +'<div class="lf-lp-stat"><span class="lf-lp-stat-l">NEUTRALISATION</span><span class="lf-lp-stat-v lf-v-green">'+blkPct+'%</span></div>'
      +'<div class="lf-lp-stat"><span class="lf-lp-stat-l">PAYS SOURCES</span><span class="lf-lp-stat-v lf-v-cyan08">'+nPays2+'</span></div>';
  }
...
function _buildLeafletTopBar(geoips){
  ...
  var rows=sorted.map(function(g){
    ...
    var col=stCol[s]||_COL_CYAN_HEX;
    var bdr=stBdr[s]||'rgba('+_COL_CYAN_RGB+',.15)';
    var bg=stBg[s]||'rgba('+_COL_CYAN_RGB+',.04)';
...
function _lfCalcHudStats(geoips){
// ── Calculs ────────────────────────────────────────────────────────────────
...
var gCol=blkPct>70?'var(--green)':blkPct>40?'var(--yellow)':'var(--red)';
...
var cColors=[_COL_RED_HEX,_COL_ORANGE_HEX,_COL_YELLOW_HEX,'rgba('+_COL_CYAN_RGB+',.7)','rgba('+_COL_CYAN_RGB+',.5)','rgba('+_COL_CYAN_RGB+',.35)'];
var scen={};
...
var lvlC=_ltr?_ltr.color:_COL_GREEN_HEX;
...
var ts=new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
  return{total:total,totalReqs:totalReqs,csBanned:csBanned,f2bOnly:f2bOnly,blocked:blocked,blkPct:blkPct,gCol:gCol,stages:stages,maxSt:maxSt,SCOL:SCOL,ctry:ctry,topC:topC,maxC:maxC,cColors:cColors,scen:scen,topSc:topSc,maxSc:maxSc,topAtk:topAtk,maxA:maxA,nPays:nPays,lvl:lvl,lvlC:lvlC,lvlBg:lvlBg,ts:ts};
}

function _lfBuildHudBarsHtml(s){
  var stages=s.stages,maxSt=s.maxSt,SCOL=s.SCOL,topC=s.topC,ctry=s.ctry;
  var maxC=s.maxC,cColors=s.cColors,topSc=s.topSc,scen=s.scen,maxSc=s.maxSc;
  var topAtk=s.topAtk,maxA=s.maxA;
...
var csHtml=scList.map(function(sc,i){
  ...
  return{ckHtml:ckHtml,cpHtml:cpHtml,csHtml:csHtml};
}

function _lfUpdateTop3(geoips){
  ...
  if(!_top.length)_top=geoips.filter(function(g){var s=g.kc_stage||g.cs_stage||'';return s==='SCAN';})
    .sort(function(a,b){return b.count-a.count;}).slice(0,3);
  var _SCOL=_LF_STAGE_COLOR;
  var t3emptyHtml=!_top.length?'<div class="lf-t3-empty">Surveillance active…</div>':'';
  var t3rowsHtml=_top.map(function(g){
    var _s=g.kc_stage||g.cs_stage||'';var _c=_SCOL[_s]||_COL_CYAN_HEX;
...
function _buildLeafletHUD(geoips){
  ...
  var _s=_lfCalcHudStats(geoips);
  var total=_s.total,totalReqs=_s.totalReqs,csBanned=_s.csBanned,f2bOnly=_s.f2bOnly;
  var blocked=_s.blocked,blkPct=_s.blkPct,gCol=_s.gCol,stages=_s.stages,maxSt=_s.maxSt;
  var topC=_s.topC,maxC=_s.maxC,scen=_s.scen,topAtk=_s.topAtk;
  var nPays=_s.nPays,lvl=_s.lvl,lvlC=_s.lvlC,lvlBg=_s.lvlBg,ts=_s.ts,SCOL=_s.SCOL;
  var _bars=_lfBuildHudBarsHtml(_s);
  var ckHtml=_bars.ckHtml,cpHtml=_bars.cpHtml,csHtml=_bars.csHtml;
  ...
  var h='<div class="lf-ct">'
   ...
   +'<div class="lf-cv"></div>'
   +'<div class="lf-cm">'
    +'<div class="lf-cm-item"><span class="lf-cm-v lf-v-red">'+total+'</span><span class="lf-cm-l">HOSTILES</span></div>'
    +'<div class="lf-cm-item"><span class="lf-cm-v lf-v-orange">'+_lfFmtN(totalReqs)+'</span><span class="lf-cm-l">VECTEURS</span></div>'
    +'<div class="lf-cm-item"><span class="lf-cm-v lf-v-green">'+blocked+'</span><span class="lf-cm-l">NEUTRALISÉS</span></div>'
   ...
   +'<div class="lf-cv"></div>'
   +'<div class="lf-cd">'
    +'<div class="lf-cd-row lf-cd-row--cs"><span>&#9632;</span><span>CS</span><span>'+csBanned+'&nbsp;BAN</span></div>'
    +'<div class="lf-cd-row lf-cd-row--f2b"><span>&#9632;</span><span>F2B</span><span>'+f2bOnly+'&nbsp;BLK</span></div>'
    +'</div>'
   +'<div class="lf-cv"></div>'
   +'<div class="lf-cts">'
...
function _buildLeafletHome(wanLat, wanLon, wanCity){
  ...
  var html='<div class="lf-radar-bg">'
    ...
    +'<div class="lf-radar-ring lf-radar-ring-1"></div>'
    +'<div class="lf-radar-ring lf-radar-ring-2"></div>'
    +'<div class="lf-radar-ring lf-radar-ring-3"></div>'
    +'<div class="lf-radar-ring lf-radar-ring-4"></div>'
    +'<div class="lf-radar-cross-h"></div>'
    +'<div class="lf-radar-cross-v"></div>'
    +'<div class="lf-home-wrap lf-home-wrap--center">'
    +'<div class="lf-home-r1"></div>'
    +'<div class="lf-home-r2"></div>'
    +'<div class="lf-home-r3"></div>'
  ...
  _lfHomeMarker=window.L.marker([wanLat,wanLon],{
    ...
    zIndexOffset:1000
  }).bindPopup('<div class="lf-popup-inner"><div class="lf-popup-stage" style="color:var(--green);background:rgba(0,255,136,.1);border:1px solid rgba(0,255,136,.3)">HOME</div><div class="lf-popup-ip">'+(wanCity||'Les Sables-d\'Olonne')+'</div><div class="lf-popup-loc">&#9670; Vendée · FR · Cible des attaques</div></div>',{className:'lf-popup',maxWidth:200});
...
function _loadLeaflet(cb){
  ...
  s.onerror=function(){_lfLoading=false;_lfFailed=true;};
...
function _toggleLeaflet(btn){
  ...
  if(_lfActive){
    ...
    if(_lfParticleFrame){cancelAnimationFrame(_lfParticleFrame);_lfParticleFrame=null;}
    if(_lfClockInt){clearInterval(_lfClockInt);_lfClockInt=null;}
    ...
    _lfClockInt=setInterval(function(){var el=document.getElementById('lf-time');if(el){var n=new Date();el.textContent=n.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});}},1000);
...
function computeSystemStatus(d, ts){
  ...
  if(exploitUnblocked>=3||(exploitUnblocked>=1&&defDown)){
    return{label:'COMPROMIS',color:_COL_COMPROMIS_HEX,icon:'✖',sub:'Intrusion active non neutralisée — intervention immédiate requise'};
  }
  if(exploitUnblocked>=1){
    return{label:'SOUS PRESSION',color:_COL_ORANGE_HEX,icon:'⚠',sub:'IP(s) EXPLOIT non bannie(s) — surveillance rapprochée requise'};
  }
  if(defDown){
    return{label:'DÉGRADÉ',color:_COL_YELLOW_HEX,icon:'▲',sub:'Protection partielle — service(s) de défense indisponible(s)'};
  }
  return{label:'SAIN',color:_COL_RTR_RX_HEX,icon:'✓',sub:'Attaques en cours — défenses actives — aucune intrusion confirmée'};
}

// computeRouterScore retirée 2026-05-17

function _hmDrawAxes(ctx,xOff,W,pH,PAD_T,yMax,nowH,cW,H){
...
for(var ti=0;ti<=N_TICKS;ti++){
  ...
  ctx.strokeStyle=ti===0?'rgba('+_COL_CYAN_RGB+',0.10)':'rgba(255,255,255,0.04)';
  ctx.lineWidth=0.5;
  ...
  ctx.fillStyle=ti===N_TICKS?'rgba('+_COL_CYAN_RGB+',0.75)':ti===0?'rgba(122,154,184,0.30)':'rgba(122,154,184,0.55)';
...
ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.05)';ctx.fillRect(curX,PAD_T,cW,pH);
ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.14)';ctx.lineWidth=0.5;ctx.strokeRect(curX,PAD_T,cW,pH);
...
function _hmDrawHours(ctx,hours,xOff,cW,PAD_T,pH,yMax,ep,peakIdx,nowH,H,livePhase,mode){

mode = mode || 'combined';
var labelStep=cW>=18?1:cW>=9?2:cW>=6?3:6;
// ── Échelle pour overlay attaque (mode='combined' uniquement) ──
// Tous les paramètres visuels via _HM_ATK_CFG (source unique no-hardcode).
// En mode 'attacks' pur : toute la hauteur (yMax=maxAtk déjà calé en amont).
// En mode 'combined' : overlay secondaire à _HM_ATK_CFG.overlayRatio × pH.
var maxAtk=Math.max.apply(null,hours.map(function(d){return d.atk;}))||1;
var atkH=pH*_HM_ATK_CFG.overlayRatio;
var atkPeakIdx=hours.reduce(function(pi,d,i){return d.atk>hours[pi].atk?i:pi;},0);
// Pulsation cyclique — utilise livePhase si dispo, sinon valeur statique 0.5
var atkPulse=livePhase!==undefined?(0.5+0.5*Math.sin(livePhase*Math.PI*2)):0.5;
hours.forEach(function(d,i){
  var x=xOff+i*cW;
  var isCur=(i===nowH);
  // ── BARRES BLEUES (requêtes) — modes 'combined' + 'traffic' ──
  if(mode!=='attacks'){
    var isPeak=(i===peakIdx&&d.req>0);
    var rh=Math.round(d.req/yMax*pH*ep);
    if(rh>0){
      var g=ctx.createLinearGradient(0,PAD_T+pH-rh,0,PAD_T+pH);
      if(isPeak){g.addColorStop(0,'rgba('+_COL_CYAN_RGB+',0.92)');g.addColorStop(1,'rgba('+_COL_CYAN_RGB+',0.18)');}
      else if(isCur){g.addColorStop(0,'rgba('+_COL_CYAN_RGB+',0.68)');g.addColorStop(1,'rgba('+_COL_CYAN_RGB+',0.10)');}
      else{g.addColorStop(0,'rgba('+_COL_CYAN_RGB+',0.44)');g.addColorStop(1,'rgba('+_COL_CYAN_RGB+',0.06)');}
      ctx.fillStyle=g;ctx.fillRect(x+0.5,PAD_T+pH-rh,cW-1,rh);
      if(isPeak){
        ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.6)';ctx.shadowBlur=8;
        ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.88)';ctx.fillRect(x+0.5,PAD_T+pH-rh,cW-1,rh);
        ctx.shadowBlur=0;
        if(ep>=0.98){
          ctx.fillStyle='rgba('+_COL_CYAN_RGB+',1)';ctx.font='bold 8px "Courier New"';
          ctx.textAlign='center';
          ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.5)';ctx.shadowBlur=4;
  ...
  // ── BARRES ROUGES (Bloqués+Erreurs) — modes 'combined' + 'attacks' ──
  // - 'combined' : overlay 15% pH avec scaling sqrt sur maxAtk (secondaire)
  // - 'attacks'  : barres pleine hauteur linéaire sur yMax (=maxAtk · primaire)
  // Pulsation alpha + glow rouge sur pic d'attaque (animation cyclique 2.4s).
  if(mode!=='traffic'){
    var ah;
    var C=_HM_ATK_CFG;
    if(mode==='attacks'){
      // Échelle linéaire pleine hauteur — métrique PRIMAIRE en tuile dédiée.
      ah=d.atk>0?Math.max(C.minPx,Math.round(d.atk/yMax*pH*ep)):0;
    }else{
      // Mode combiné : overlay secondaire avec sqrt scaling (différenciation).
      ah=d.atk>0?Math.max(C.minPx,Math.round(Math.sqrt(d.atk/maxAtk)*atkH*ep)):0;
    }
    if(ah>0){
      var isAtkPeak=(i===atkPeakIdx&&d.atk>0);
      // Pulsation alpha base (centrée 0.5 → amplitude ±C.alphaAmp)
      var baseA=C.alphaBase+(atkPulse-0.5)*(C.alphaAmp*2);
      if(isAtkPeak){
        // Pic d'attaque : glow rouge pulsé + alpha renforcé
        ctx.shadowColor='rgba(255,59,92,'+(0.4+atkPulse*0.5).toFixed(2)+')';
        ctx.shadowBlur=C.glowBase+atkPulse*C.glowAmp;
        ctx.fillStyle='rgba(255,59,92,'+(C.peakAlphaBase+atkPulse*C.peakAlphaAmp).toFixed(2)+')';
        ctx.fillRect(x+0.5,PAD_T+pH-ah,cW-1,ah);
        ctx.shadowBlur=0;
        ctx.fillStyle='rgba(255,80,110,1)';ctx.fillRect(x+0.5,PAD_T+pH-ah,cW-1,2);
        // Label valeur sur pic en mode 'attacks' (parité avec bleu en 'traffic')
        if(mode==='attacks' && ep>=0.98){
          ctx.fillStyle='rgba(255,80,110,1)';ctx.font='bold 8px "Courier New"';
          ctx.textAlign='center';
          ctx.shadowColor='rgba(255,59,92,0.5)';ctx.shadowBlur=4;
          var ly3=PAD_T+pH-ah-4;if(ly3<PAD_T+9)ly3=PAD_T+pH-ah+9;
          ctx.fillText(_rcFmtV(d.atk),x+cW/2,ly3);ctx.shadowBlur=0;
          ctx.textAlign='left';
        }
      }else{
        ctx.fillStyle='rgba(255,59,92,'+baseA.toFixed(2)+')';
        ctx.fillRect(x+0.5,PAD_T+pH-ah,cW-1,ah);
        ctx.fillStyle='rgba(255,59,92,'+(0.85+atkPulse*0.15).toFixed(2)+')';
        ctx.fillRect(x+0.5,PAD_T+pH-ah,cW-1,1);
      }
    }
  }
  ...
  if(d.h%labelStep===0){
    var lc=isCur?'rgba('+_COL_CYAN_RGB+',1)':'rgba(122,154,184,0.65)';
    ctx.fillStyle=lc;ctx.font=(isCur?'bold ':'')+'8px "Courier New"';
    ctx.textAlign='center';ctx.fillText(d.h+'h',x+cW/2,H-3);
    ctx.strokeStyle=isCur?'rgba('+_COL_CYAN_RGB+',0.4)':'rgba(122,154,184,0.25)';ctx.lineWidth=0.5;
    ctx.beginPath();ctx.moveTo(x+cW/2,PAD_T+pH);ctx.lineTo(x+cW/2,PAD_T+pH+3);ctx.stroke();
  }
});
}

function _hmDrawDecor(ctx,xOff,plotW,W,pH,PAD_T,nowH,cW,H,hours,yMax,ep,canvas){

ctx.textAlign='left';
// ── Baseline ──
ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.14)';ctx.fillRect(xOff,PAD_T+pH,plotW,1);
...
ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.55)';ctx.lineWidth=1;ctx.setLineDash([3,3]);
ctx.beginPath();ctx.moveTo(nx,PAD_T);ctx.lineTo(nx,PAD_T+pH);ctx.stroke();ctx.setLineDash([]);
ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.9)';ctx.shadowBlur=6;
ctx.fillStyle='rgba('+_COL_CYAN_RGB+',1)';
ctx.beginPath();ctx.arc(nx,PAD_T+pH-2,2.5,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;
ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.85)';ctx.font='bold 7px "Courier New"';
...
if(hIdx>=0&&hIdx<24){
  var hx=xOff+hIdx*cW;
  ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.09)';ctx.fillRect(hx,PAD_T,cW,pH);
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.38)';ctx.lineWidth=1;
  ctx.strokeRect(hx+0.5,PAD_T,cW-1,pH);
  var hd=hours[hIdx];
  var hrh=Math.round(hd.req/yMax*pH*ep);
  if(hrh>3){
    ctx.fillStyle='rgba('+_COL_CYAN_RGB+',1)';ctx.font='bold 8px "Courier New"';ctx.textAlign='center';
    ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.8)';ctx.shadowBlur=6;
...
// ── Bordure verre ──
ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.11)';ctx.lineWidth=1;
ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.05)';ctx.shadowBlur=3;
ctx.strokeRect(xOff,PAD_T,plotW,pH);ctx.shadowBlur=0;
}

function drawAttackHeatmap(canvas, reqH, blkH, errH, customH, progress, livePhase, mode){
  // mode : 'combined' (défaut · bleu + overlay rouge) | 'traffic' (bleu only)
  //        | 'attacks' (rouge only avec échelle dédiée 0→maxAtk)
  if(!canvas)return;
  mode = mode || 'combined';
  var ep=progress===undefined?1:(1-Math.pow(1-Math.min(1,progress),4));
  // Fallback width : offsetWidth peut être 0 si layout pas encore appliqué
  // (cas modal cloné via innerHTML). On retombe sur clientWidth puis 800px
  // par défaut pour éviter `canvas.width=0` → rien dessiné (modal vide).
  var W=canvas.offsetWidth||canvas.clientWidth||canvas.parentElement&&canvas.parentElement.clientWidth||800;
  ...
  var maxAtk=Math.max.apply(null,hours.map(function(d){return d.atk;}))||1;
  // Échelle dédiée : si mode='attacks', l'axe Y est calé sur maxAtk (et pas
  // sur maxReq) — les barres rouges utilisent toute la hauteur disponible.
  var yMaxBase=mode==='attacks'?maxAtk:maxReq;
  var peakIdx=mode==='attacks'
    ?hours.reduce(function(pi,d,i){return d.atk>hours[pi].atk?i:pi;},0)
    :hours.reduce(function(pi,d,i){return d.req>hours[pi].req?i:pi;},0);
  ...
  var yMax=_HM_NICE[0];
  for(var ni=0;ni<_HM_NICE.length;ni++){if(_HM_NICE[ni]>=yMaxBase*1.10){yMax=_HM_NICE[ni];break;}}
  ...
  _hmDrawAxes(ctx,xOff,W,pH,PAD_T,yMax,nowH,cW,H,mode);
  _hmDrawHours(ctx,hours,xOff,cW,PAD_T,pH,yMax,ep,peakIdx,nowH,H,livePhase,mode);
  _hmDrawDecor(ctx,xOff,plotW,W,pH,PAD_T,nowH,cW,H,hours,yMax,ep,canvas,mode);
}
function _hmSetupInteraction(canvas,reqH,blkH,errH){
  ...
  if(!tt){
    ...
    tt.style.cssText='position:fixed;z-index:9999;pointer-events:none;background:rgba(0,8,20,0.94);'
      +'border:1px solid rgba('+_COL_CYAN_RGB+',0.28);border-radius:3px;padding:.3rem .5rem;'
      +'font:8px "Courier New",monospace;color:rgba('+_COL_CYAN_RGB+',0.9);display:none;'
      +'min-width:108px;line-height:1.65;box-shadow:0 0 14px rgba('+_COL_CYAN_RGB+',0.1)';
  ...
  canvas._hmMM=function(e){
    ...
    var nowH=new Date().getHours();
    var tag=idx===nowH?' <span style="color:rgba('+_COL_CYAN_RGB+',0.55)">[NOW]</span>':idx>nowH?' <span style="color:rgba(122,154,184,0.45)">[J-1]</span>':'';
    tt.innerHTML='<div style="color:rgba(255,255,255,0.85);font-weight:700;margin-bottom:.2rem;letter-spacing:.5px">\u25b8 '+idx+'h00'+tag+'</div>'
      +'<div>Req&nbsp;&nbsp;&nbsp;&nbsp;\u00a0: <b style="color:rgba('+_COL_CYAN_RGB+',1)">'+_rcFmtV(req)+'</b></div>'
      ...
      +(req>0?'<div style="margin-top:.15rem;border-top:1px solid rgba(255,255,255,.07);padding-top:.15rem">'
        +'Taux&nbsp;&nbsp;&nbsp;&nbsp; : <b style="color:'+(pct>15?'rgba(255,59,92,0.9)':pct>5?'rgba(255,200,0,0.9)':'rgba('+_COL_CYAN_RGB+',0.7)')+'">'+pct+'%</b></div>':'');
...
function animateAttackHeatmap(canvas, reqH, blkH, errH, customH, mode){
  _hmSetupInteraction(canvas,reqH,blkH,errH);
  var dur=700, start=null, liveStart=null;
  function frame(ts){
    if(!start)start=ts;
    var p=(ts-start)/dur;
    drawAttackHeatmap(canvas,reqH,blkH,errH,customH,Math.min(1,p),undefined,mode);
  ...
  function live(ts){
    ...
    drawAttackHeatmap(canvas,reqH,blkH,errH,customH,1,((ts-liveStart)%1500)/1500,mode);
...
var _protoAnim=null;
var PROTO_DEF=[
  {k:'HTTPS',     l:'HTTPS',      desc:'Trafic chiffré TLS légitime',      c:_COL_CYAN_HEX,  grp:'ok'},
  {k:'ASSETS',    l:'ASSETS',     desc:'Ressources statiques CSS/JS/img',  c:_COL_GREEN_HEX,  grp:'ok'},
  {k:'HTTP',      l:'HTTP',       desc:'Trafic HTTP brut non redirigé (bots/scans port 80)',c:PROTO_PALETTE.WATCH, grp:'watch'},
  {k:'HTTP_REDIRECT',l:'REDIRECT',desc:'Redirection HTTP → HTTPS (301/302 légitimes)',      c:PROTO_PALETTE.NEUTRAL, grp:'ok'},
  {k:'NOT_FOUND', l:'404',        desc:'Ressource introuvable — scan de paths probable',c:PROTO_PALETTE.RECON,grp:'watch'},
  {k:'ERROR_5XX', l:'5XX',        desc:'Erreur interne serveur (5xx)',      c:PROTO_PALETTE.ERR,  grp:'watch'},
  {k:'LEGIT_BOT', l:'LEGIT-BOT',  desc:'Robot identifié (Googlebot, Bing)',  c:PROTO_PALETTE.BOT_LEGIT, grp:'watch'},
  {k:'GEO_BLOCK', l:'GEO-BLOCK',  desc:'IP bloquée par règle GeoIP2',      c:PROTO_PALETTE.GEO,  grp:'threat'},
  {k:'CLOSED',    l:'CLOSED',     desc:'Connexion fermée sans réponse (444)',c:PROTO_PALETTE.THREAT, grp:'threat'},
  {k:'RATE_LIMIT',l:'RATE-LIMIT', desc:'Limite de taux dépassée (429)',     c:PROTO_PALETTE.RATE,  grp:'threat'},
  {k:'BOT',       l:'BOT',        desc:'Robot non identifié / suspect',     c:PROTO_PALETTE.BOT,  grp:'threat'},
  {k:'SCANNER',   l:'SCANNER',    desc:'Outil de scan ou exploit détecté',  c:_COL_RED_HEX,  grp:'threat'},
  {k:'OTHER',     l:'AUTRE',      desc:'Requête non classifiée',            c:'rgba(100,130,155,0.7)',grp:'other'},
  // ── Suricata IDS réseau (SUR_*) ──────────────────────────────────────────
  {k:'SUR_DNS',  l:'DNS',        desc:'Suricata — requêtes DNS observées',   c:_COL_GREEN_HEX,  grp:'sur'},
  {k:'SUR_HTTP', l:'S-HTTP',     desc:'Suricata — trafic HTTP analysé',      c:PROTO_PALETTE.WATCH,  grp:'sur'},
  {k:'SUR_TLS',  l:'TLS/SSL',    desc:'Suricata — flux TLS/SSL (JA3 inclus)',c:_COL_CYAN_HEX,  grp:'sur'},
  {k:'SUR_SSH',  l:'SSH-NET',    desc:'Suricata — sessions SSH réseau',      c:PROTO_PALETTE.RECON,  grp:'sur'},
  {k:'SUR_FLOW', l:'FLOW',       desc:'Suricata — flux réseau total',        c:PROTO_PALETTE.FLOW,  grp:'sur'},
  {k:'SUR_SCAN', l:'SCAN-NET',   desc:'Suricata — port scan NMAP détecté',   c:_COL_RED_HEX,  grp:'sur_threat'},
  {k:'SUR_C2',   l:'C2/TROJAN',  desc:'Suricata — C2 Feodo/Trojan sév.1',   c:PROTO_PALETTE.ERR,  grp:'sur_threat'},
];
...
function buildProtoThreatGauge(proto){
  ...
  return '<div class="proto-threat-gauge">'
    ...
    +'<div class="proto-tg-leg-item"><span class="proto-tg-leg-dot proto-tg-leg-dot--cyan"></span><span>LÉGITIMES — Visites réelles navigateurs (2xx / 3xx)</span></div>'
    +'<div class="proto-tg-leg-item"><span class="proto-tg-leg-dot proto-tg-leg-dot--amber"></span><span>SURVEILLANCE — Monitoring interne, APIs, crawlers connus</span></div>'
    +'<div class="proto-tg-leg-item"><span class="proto-tg-leg-dot proto-tg-leg-dot--critical"></span><span>MENACES — Connexions bloquées (444 · GeoIP 403 · Rate-limit 429)</span></div>'
    +'</div>'
    +'</div>';
}
...
function buildProtoModalLegend(proto){
  ...
  if(!total)return '';
  var groups=[
    {id:'ok',     lbl:'LÉGITIMES',    col:_COL_CYAN_HEX},
    {id:'watch',  lbl:'SURVEILLANCE', col:PROTO_PALETTE.WATCH},
    {id:'threat', lbl:'MENACES',      col:PROTO_PALETTE.THREAT},
...
function drawProtoSpark(canvas,buckets){
  ...
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.55)';ctx.lineWidth=1.5;ctx.stroke();
  // fill
  ctx.lineTo((vals.length-1)*step,H);ctx.lineTo(0,H);ctx.closePath();
  ctx.fillStyle='rgba('+_COL_CYAN_RGB+',0.06)';ctx.fill();
  // labels first/last
...
function updateThreatFeed(geoips){
  ...
  list.innerHTML=blocked.slice(0,7).map(function(g,i){
    var dotCol=g.cs_banned?_COL_ORANGE_HEX:_COL_RED_HEX;
    var sc=g.cs_banned&&g.cs_scenario
      ?'<span style="font-size:var(--fs-sm);color:var(--orange);background:rgba(255,107,53,0.15);padding:.05rem .3rem;border-radius:2px;margin-left:.2rem">⊛'+esc(_csLabel(g.cs_scenario,12))+'</span>'
      :'';
