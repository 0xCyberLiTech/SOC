...
var _online       = false;
var _enabled      = (function(){try{return localStorage.getItem(_LS_KEYS.SOC_AI)!=='off';}catch(e){return true;}})();
var _ttsEnabled   = (function(){try{return localStorage.getItem(_LS_KEYS.SOC_AI_TTS)==='on';}catch(e){return false;}})();
var _ttsSingleShot  = false;
var _thinking       = false;
var _pendingBanParse = false;
...
function _injectPanel(){
  ...
  panel.innerHTML =
    ...
    + '<div class="jv-enable-btn'+(_enabled?' active':'')+'" id="jv-enable-toggle" title="'+(_enabled?'JARVIS actif — cliquer pour désactiver':'JARVIS désactivé — cliquer pour activer')+'">'+(_enabled?'■ ON':'□ OFF')+'</div>'
  ...
  panel.querySelector('#jv-enable-toggle').addEventListener('click', toggleEnabled);
...
function fetchSecStat(){
  if(!_online) return;
  fetch(JV_URL+'/api/security',{signal:AbortSignal.timeout(_PING_TMO_MS)})
...
window._secClear = function(){
  fetch(JV_URL+'/api/security/clear',{method:'POST',signal:AbortSignal.timeout(_PING_TMO_MS)})
...
window._secRefreshModal = function(){
  ...
  var hdr='<div style="display:grid;grid-template-columns:140px 80px 1fr;gap:.5rem;font-size:var(--fs-xs);color:var(--muted);font-family:\'Courier New\',monospace;margin-bottom:.5rem;padding-bottom:.4rem;border-bottom:1px solid rgba(255,255,255,0.06)">'
...
function pingJarvis(e){
  ...
  .then(function(r){
    var ms=Date.now()-t0;
    if(r.ok){
      toast.classList.add('jv-toast-ok');
      toast.innerHTML='<span class="jv-toast-msg-ok">✓ JARVIS a reçu — voix TTS déclenchée</span><span class="jv-toast-ms">'+ms+'ms</span>';
      _online=true; fetchSecStat(); fetchSocActions(); updateBadge();
      // Bonus : récupérer l'état du circuit breaker Ollama et l'afficher dans le toast + badge bouton
      fetch(JV_URL+'/api/ollama-status',{signal:AbortSignal.timeout(_PING_TMO_MS)})
        .then(function(rr){return rr.ok?rr.json():null;})
        .then(function(d){
          if(!d||!d.state) return;
          var stateMap={closed:{cls:'cb-closed',ico:'●',txt:'OK'},
                        half_open:{cls:'cb-half_open',ico:'◐',txt:'recovery '+(d.retry_in_s||0)+'s'},
                        open:{cls:'cb-open',ico:'◯',txt:'DOWN retry '+(d.retry_in_s||0)+'s'}};
          var s=stateMap[d.state]||stateMap.closed;
          // Ajout au toast (bloc Ollama)
          var ollamaInfo=document.createElement('div');
          ollamaInfo.className='jv-toast-ollama '+s.cls;
          ollamaInfo.innerHTML=s.ico+' Ollama : '+s.txt;
          toast.appendChild(ollamaInfo);
          // Badge persistant sur le bouton (vert/orange/rouge selon état)
          if(btn){btn.classList.remove('cb-closed','cb-half_open','cb-open');btn.classList.add(s.cls);}
        })
        .catch(function(){});
    }else{
      toast.classList.add('jv-toast-err');
      toast.innerHTML='<span class="jv-toast-msg-err">✗ JARVIS HTTP '+r.status+'</span>';
    }
  })
  .catch(function(err){
    toast.classList.add('jv-toast-err');
    toast.innerHTML='<span class="jv-toast-msg-err">✗ JARVIS injoignable</span><div class="jv-toast-detail">'+esc(err.message)+'</div>';
  })
  .finally(function(){
...
function fetchSocActions(){
  if(!_online) return;
  fetch(JV_URL+'/api/soc/actions',{signal:AbortSignal.timeout(_PING_TMO_MS)})
    .then(function(r){ return r.ok?r.json():Promise.reject(r.status); })
...
function updateProTile(d){
  ...
  if(caps){
    var on='color:var(--green);font-size:var(--fs-xs)';
    ...
    caps.innerHTML=
      ...
      +row('⚡','Ban auto — EXPLOIT CVE ('+(t.banExploitMin||1)+' tentative)', jvOn)
...
function _updateJvLastAct(acts){
  ...
  var typeLabel={'ban_ip':'BAN-IP','unban_ip':'UNBAN','restart_service':'RESTART'}[a.type]||esc(a.type);
...
function _drawBanSpark(acts){
  ...
  bans.forEach(function(a){
    ...
    var ts=0;
    if(a.ts){ try{ ts=new Date(a.ts.replace(' ','T')).getTime(); }catch(e){} }
...
// ── Modal détail JARVIS ────────────────────────────────────────
function _jvModalJournalHtml(acts){
...
var journalBodyHtml2=!acts.length
  ...
  :acts.map(function(a){
    ...
    var bl=TYPE_LBL[a.type]||esc(a.type||'?');
  ...
  return journalBodyHtml2;
}

function _jvModalHistoryHtml(){
...
function _jvModalBodyHtml(d,gs,secD,journalBodyHtml2,historyHtml2,temp,tempCol,vramPct,vramCol,gpuCol,cpuCol,sshOk){
var bt=d.by_type||{};
...
function openJvModal(){
  ...
  var journalBodyHtml2=_jvModalJournalHtml(acts);
  var historyHtml2=_jvModalHistoryHtml();
  var h=_jvModalBodyHtml(d,gs,secD,journalBodyHtml2,historyHtml2,temp,tempCol,vramPct,vramCol,gpuCol,cpuCol,sshOk);
...
window.openJvModal = openJvModal;
...
window._proClear = function(){
  fetch(JV_URL+'/api/soc/actions/clear',{method:'POST',signal:AbortSignal.timeout(_PING_TMO_MS)})
...
window._proRefreshModal = function(){
  ...
  var hdr='<div style="display:grid;grid-template-columns:140px 120px 70px 1fr;gap:.5rem;font-size:var(--fs-xs);color:var(--muted);font-family:\'Courier New\',monospace;margin-bottom:.5rem;padding-bottom:.4rem;border-bottom:1px solid rgba(255,255,255,0.06)">'
...
function _loadSnaps(){
  try{ return JSON.parse(localStorage.getItem(_LS_KEYS.JV_FW_SNAPS)||'[]'); }catch(e){ return []; }
}
function _saveSnaps(arr){
  try{ localStorage.setItem(_LS_KEYS.JV_FW_SNAPS,JSON.stringify(arr)); }catch(e){}
}
...
function _fwRenderSnapHist(activeIdx){
  ...
  if(prev){
    ...
    if(diffs.length){
      detailHtml += '<div class="snap-diff-title" style="color:'+_COL_RTR_TX_HEX+';margin-top:.5rem">⚠ Différences vs snapshot précédent ('+esc(prev.label)+')</div>';
...
// ═══════════════════════════════════════════════════════════════
function buildJarvisTileInner(){
  var _sc=[
    {id:'jv-cnt-a',    lbl:'Analyses', col:'0,217,255', cls:'jv-sv-a'},
    {id:'jv-cnt-al',   lbl:'Alertes',  col:'255,59,92', cls:'jv-sv-al'},
    {id:'jv-cnt-ban-t',lbl:'Bans auto',col:'255,149,0', cls:'jv-sv-ban'},
    {id:'jv-cnt-rst-t',lbl:'Restarts', col:'0,217,100', cls:'jv-sv-rst'}
  ];
  var _statCards=_sc.map(function(s){
    return '<div style="background:linear-gradient(135deg,rgba('+s.col+',.09),rgba('+s.col+',.02));'
      +'border:1px solid rgba('+s.col+',.22);border-radius:7px;padding:.45rem .3rem;text-align:center;position:relative;overflow:hidden">'
      +'<div style="position:absolute;bottom:.15rem;right:.3rem;font-size:var(--fs-lg);color:rgba('+s.col+',.07);font-weight:900;line-height:1">◈</div>'
      +'<div class="jv-stat-val '+s.cls+'" id="'+s.id+'" style="color:rgb('+s.col+')">0</div>'
      +'<div style="font-size:calc(var(--fs-xs) - 1px);color:rgba('+s.col+',.42);text-transform:uppercase;letter-spacing:.5px;margin-top:.14rem">'+s.lbl+'</div>'
      +'</div>';
  }).join('');
  return '<div class="jv-tile-hdr">'
    ...
    +'</div>'
    +'<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:.4rem;margin:.1rem 0 .45rem">'+_statCards+'</div>'
    ...
    +'<button id="jv-btn-detail" class="jv-tile-open hidden">◈ DÉTAIL COMPLET</button>'
    +'</div>';
}
window.buildJarvisTileInner = buildJarvisTileInner;
...
function updateJarvisTile(type, text){
  ...
  var clt=document.getElementById('jv-tile-last'); if(clt) clt.textContent='dernière : '+ts;
