// 03-canvas-kci.js — Mini Kill Chain animée (investigation modal)
// Dépend de : 02-canvas-kc.js (_kciAnimFrame, _kcHex, _kcFmtCat) · 01-utils.js (fmtScenario)
//             06-canvas-geomap.js (_kcFmtAge, _kcFmtDelta)
(function(){
'use strict';

function _kciDrawBeacons(ctx,nodes,nodeR,cY,phase,W){
  ...
  ctx.save();
  ctx.strokeStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+(0.72*bcPulse)+')';
  ctx.lineWidth=2; ctx.lineCap='round';
  ctx.shadowColor='rgba('+KC_PALETTE.BRUTE.rgb+',0.7)'; ctx.shadowBlur=8;
  ...
  if(reconX>bcLX+6){
    ...
    tgL.addColorStop(0,'rgba('+KC_PALETTE.BRUTE.rgb+',0.45)'); tgL.addColorStop(1,'rgba('+KC_PALETTE.RECON.rgb+',0.15)');
    ...
    if(fa>0.05){
      ctx.save(); ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+','+fa+')';
      ctx.shadowColor='rgba('+KC_PALETTE.BRUTE.rgb+','+fa+')'; ctx.shadowBlur=4;
      ctx.beginPath(); ctx.moveTo(fx-4,cY-3); ctx.lineTo(fx,cY); ctx.lineTo(fx-4,cY+3); ctx.lineTo(fx-2,cY);
      ctx.closePath(); ctx.fill(); ctx.restore();
    }
  }
  ctx.save(); ctx.fillStyle='rgba('+KC_PALETTE.BRUTE.rgb+',0.62)';
  ctx.font='bold 7px "Courier New",monospace'; ctx.textAlign='left'; ctx.textBaseline='top';
  ctx.fillText('INTERNET',bcLX,8); ctx.restore();
  ctx.save();
  ctx.strokeStyle='rgba('+KC_PALETTE.BLOCKED.rgb+','+(0.72*bcDPulse)+')';
  ctx.lineWidth=2; ctx.lineCap='round';
  ctx.shadowColor='rgba('+KC_PALETTE.BLOCKED.rgb+',0.7)'; ctx.shadowBlur=8;
  ...
  if(bcRX>blockedX+6){
    ...
    tgR.addColorStop(0,'rgba('+KC_PALETTE.BLOCKED.rgb+',0.15)'); tgR.addColorStop(1,'rgba('+KC_PALETTE.BLOCKED.rgb+',0.45)');
    ...
    if(fa2>0.05){
      ctx.save(); ctx.fillStyle='rgba('+KC_PALETTE.BLOCKED.rgb+','+fa2+')';
      ctx.shadowColor='rgba('+KC_PALETTE.BLOCKED.rgb+','+fa2+')'; ctx.shadowBlur=4;
      ctx.beginPath(); ctx.moveTo(fx2+4,cY-3); ctx.lineTo(fx2,cY); ctx.lineTo(fx2+4,cY+3); ctx.lineTo(fx2+2,cY);
      ctx.closePath(); ctx.fill(); ctx.restore();
    }
  }
  ctx.save(); ctx.fillStyle='rgba('+KC_PALETTE.BLOCKED.rgb+',0.62)';
  ctx.font='bold 7px "Courier New",monospace'; ctx.textAlign='right'; ctx.textBaseline='top';
  ctx.fillText('DÉFENSE',bcRX,8); ctx.restore();
}
function _kciDrawTubes(ctx,nodes,travMax,curIdx,nodeR,_stRgbs){
...
function _kciDrawParticle(ctx,nodes,curIdx,pt,nodeR,_stRgbs){
  if(curIdx>0&&pt>=0){
...
function _kciDrawArrows(ctx,nodes,travMax,nodeR,_stRgbs){
  for(var i=0;i<nodes.length-1;i++){
    var n1=nodes[i],n2=nodes[i+1];
...
function _kciNodeCritBadge(ctx,node,rgb,phase,nodeR,_mcCounts,isCur,isTrav,attackIdx,i){
  if(!(isCur||(isTrav&&i===attackIdx)))return;
  var _mcCnt=_mcCounts[node.id]||0;
  var _mcThresh=(window._kcCritThresh||{})[node.id];
  if(!(_mcThresh&&_mcCnt>=_mcThresh))return;
...
function _kciNodeTimers(ctx,node,rgb,isCur,isTrav,ipObj,curStage,_stIds,ann,i,nodeR,lblFontSz,annFontSz,lblY,pulse){
  if(isTrav){
...
function _kciDrawOneNode(ctx,node,i,curIdx,travMax,attackIdx,nodeR,ann,phase,ipObj,curStage,_stRgbs,_stCols,_stIcons,_mcCounts,_stIds){
  var rgb=_stRgbs[node.id],color=_stCols[node.id];
  var isCur=(i===curIdx),isTrav=(i<=travMax&&i!==curIdx);
  var pulse=isCur?(0.58+0.42*Math.sin(phase*Math.PI*2)):1;
  _kciNodeCritBadge(ctx,node,rgb,phase,nodeR,_mcCounts,isCur,isTrav,attackIdx,i);
  if(isCur){
    ctx.save();
  ...
  _kciNodeTimers(ctx,node,rgb,isCur,isTrav,ipObj,curStage,_stIds,ann,i,nodeR,lblFontSz,annFontSz,lblY,pulse);
}
function _kciDrawNodes(ctx,nodes,curIdx,travMax,attackIdx,nodeR,ann,phase,ipObj,curStage,_stRgbs,_stCols,_stIcons,_mcCounts,_stIds){
  nodes.forEach(function(node,i){
    _kciDrawOneNode(ctx,node,i,curIdx,travMax,attackIdx,nodeR,ann,phase,ipObj,curStage,_stRgbs,_stCols,_stIcons,_mcCounts,_stIds);
  });
}
function _kciAnimMiniChain(canvas, ipObj, csInfo, surInfo){
  ...
  // KC 7 maillons (2026-05-16) : aligné sur KCK_STAGES — ajout PROBE et WAF
  var _stIds =['PROBE','RECON','SCAN','EXPLOIT','WAF','BRUTE','BLOCKED'];
  var _stCols={PROBE:KC_PALETTE.PROBE.color, RECON:KC_PALETTE.RECON.color, SCAN:KC_PALETTE.SCAN.color, EXPLOIT:KC_PALETTE.EXPLOIT.color, WAF:KC_PALETTE.WAF.color, BRUTE:KC_PALETTE.BRUTE.color, BLOCKED:KC_PALETTE.BLOCKED.color};
  var _stRgbs={PROBE:KC_PALETTE.PROBE.rgb,   RECON:KC_PALETTE.RECON.rgb,   SCAN:KC_PALETTE.SCAN.rgb,   EXPLOIT:KC_PALETTE.EXPLOIT.rgb,   WAF:KC_PALETTE.WAF.rgb,   BRUTE:KC_PALETTE.BRUTE.rgb,   BLOCKED:KC_PALETTE.BLOCKED.rgb};
  var _stIcons=['◍','◎','⌖','◈','◇','⊗','⬡'];
  var curStage=(ipObj.stage||'RECON').toUpperCase();
  var attackIdx=_stIds.indexOf(curStage);if(attackIdx<0)attackIdx=1;  // fallback = RECON (1)
  ...
  var ann=['','','','','','',''];  // 7 emplacements (1 par stage)
  if(ipObj.count>0) ann[1]='×'+ipObj.count+' req/15min';  // RECON = premier stage HTTP réel
  if(csInfo.scenario){
    var sc=fmtScenario(csInfo.scenario);
    if(/exploit|cve|inject|rce|lfi|rfi|xss|sql|traversal|webshell/i.test(csInfo.scenario)) ann[3]=sc;  // EXPLOIT
    else if(/brute|password|login|ssh|ftp|smtp|wp-login/i.test(csInfo.scenario)) ann[5]=sc;  // BRUTE
    else if(/scan|probe|crawl|enum/i.test(csInfo.scenario)) ann[2]=sc;  // SCAN
    else ann[annIdx]=sc;
  }
  if(surInfo.cves&&surInfo.cves.length) ann[3]=ann[3]?ann[3]:surInfo.cves[0];  // EXPLOIT
  if(surInfo.cat&&!ann[annIdx]) ann[annIdx]=_kcFmtCat(surInfo.cat).slice(0,20);
  if(ipObj.banned) ann[6]='fail2ban ✓';  // BLOCKED = index 6
  else if(ipObj.cs_decision) ann[6]='⊛ CS actif';
  ...
  function _draw(){
    ...
    ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.025)';ctx.lineWidth=1;
    ...
    var padLX=nodeR+22, padRX=nodeR+44;
    ...
    _kciDrawBeacons(ctx,nodes,nodeR,cY,phase,W);
    _kciDrawTubes(ctx,nodes,travMax,curIdx,nodeR,_stRgbs);
    _kciDrawParticle(ctx,nodes,curIdx,pt,nodeR,_stRgbs);
    _kciDrawArrows(ctx,nodes,travMax,nodeR,_stRgbs);
    _kciDrawNodes(ctx,nodes,curIdx,travMax,attackIdx,nodeR,ann,phase,ipObj,curStage,_stRgbs,_stCols,_stIcons,_mcCounts,_stIds);
  }
  function _frame(){
...
window._kciAnimMiniChain = _kciAnimMiniChain;
// ══════════════════════════════════════════════════════
