function loadRouter(){
  ...
  Promise.all([_f('/router.json'),_f('/router-history.json'),_f('/router-flows.json')]).then(function(res){
    ...
    if(rd&&rd.available){
      ...
      // Mettre à jour la ligne gt-be98 dans le SSH status card
      ...
      if(sshCard){
        sshCard.querySelectorAll('tbody tr').forEach(function(row){
          ...
          if(nc&&nc.textContent.trim().indexOf('gt-be98')===0){
  ...
  var topCountriesHtml=(t.top_countries&&t.top_countries.length)
    ?'<div style="font-size:var(--fs-xs);color:var(--muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:.22rem">Top pays bloqués</div>'
      +bcc.map(function(c){
        ...
        return `<div style="display:flex;align-items:center;gap:.3rem;margin-bottom:.15rem"><span style="color:var(--red);font-family:'Courier New',monospace;font-size:var(--fs-xs);min-width:22px">${c[0]}</span><div style="flex:1;background:rgba(255,255,255,0.04);height:2px"><div style="width:${p}%;height:2px;background:rgba(255,59,92,0.7)"></div></div><span style="font-size:var(--fs-xs);color:var(--muted);min-width:26px;text-align:right">${c[1]}</span></div>`;
      }).join(''):'';
  ...
  var protosHtml=(ufw.protos&&Object.keys(ufw.protos).length)
    ?'<div style="display:flex;gap:.3rem;flex-wrap:wrap;margin-top:.4rem">'
      +Object.entries(ufw.protos).sort(function(a,b){return b[1]-a[1];}).slice(0,4).map(function(e){
        return `<span style="font-size:var(--fs-xs);color:var(--cyan);background:rgba(0,217,255,0.06);padding:.03rem .28rem;border:1px solid rgba(0,217,255,0.12);font-family:'Courier New',monospace">${e[0]} ${Math.round(e[1]/totalP*100)}%</span>`;
      }).join('')+'</div>':'';
...
function buildFwMatrix(hosts){
  ...
  var rowsHtml=ROWS.map(function(r){
    return `<tr><td>:${r.p}</td><td style="color:var(--muted);font-size:var(--fs-xs)">${r.s}</td><td>${_fwMatrixCell(r.ng,r.p,'srv-ngix',hm)}</td><td>${_fwMatrixCell(r.ct,r.p,'site01',hm)}</td><td>${_fwMatrixCell(r.p85,r.p,'site02',hm)}</td><td>${_fwMatrixCell(r.pve,r.p,'proxmox',hm)}</td></tr>`;
  }).join('');
  ...
  var extraHtml=ek.length
    ?'<div style="margin-top:.5rem;font-size:var(--fs-xs);color:var(--muted)">Ports supplémentaires détectés: '
      +ek.map(function(p){return`<span style="color:var(--yellow);margin-right:.5rem">:${p} [${extra[p].join(',')}]</span>`;}).join('')+'</div>':'';
  return `<table class="fw-mt"><tr><th>PORT</th><th>SERVICE</th><th>ngix</th><th>site01</th><th>site02</th><th>proxmox</th></tr>${rowsHtml}</table>${extraHtml}`;
}
...
var _FW_JAIL_COLOR={'nginx-site01-444':'var(--red)','nginx-site01-400':'var(--red)','nginx-limit-req':'var(--orange)','nginx-http-auth':'var(--yellow)','sshd':'var(--yellow)'};
function _fwRenderJails(jails){
  var s='';
  (jails||[]).forEach(function(j){
    var hasBan=j.cur_banned>0;
    var jc=_FW_JAIL_COLOR[j.jail]||'var(--muted)';
    s+='<div style="display:flex;align-items:center;gap:.3rem;padding:.12rem 0;border-bottom:1px solid rgba(255,255,255,0.025)">';
    s+='<span style="font-size:var(--fs-xs);color:'+(hasBan?jc:'var(--muted)')+';flex:1;font-family:\'Courier New\',monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+j.jail.replace('nginx-','')+'</span>';
    s+='<span style="font-size:var(--fs-xs);color:'+(hasBan?'var(--orange)':'rgba(122,154,184,0.4)')+';min-width:14px;text-align:center">'+(hasBan?'▲'+j.cur_banned:'·')+'</span>';
    s+='<span style="font-size:var(--fs-xs);color:var(--muted);min-width:26px;text-align:right">'+j.tot_banned+'</span>';
    s+='</div>';
  });
  return s;
}
...
function buildFwRules(hosts){
  var hcol={'srv-ngix':'var(--cyan)','site01':'var(--green)','site02':'var(--green)','proxmox':'var(--purple)'};
  return (hosts||[]).map(function(hh){
    ...
    var rulesHtml=rules.map(function(r,i){
      ...
      return `<div class="fw-rr"><span class="fw-ri">${i+1}</span><span class="fw-rac ${ac}">${r.a}</span><span class="fw-rd2">${r.d}</span></div>`;
    }).join('');
    ...
    var issuesHtml=(hh.issues||[]).map(function(iss){return`<div class="fw-issue2">⚠ ${iss}</div>`;}).join('');
    return `<div class="fw-rm"><div class="fw-rh" style="border-left-color:${col};color:${col}"><span>${n}</span><span style="color:var(--muted);font-size:var(--fs-xs)">${esc(hh.ip||'')}</span>${badge}<span style="font-size:var(--fs-xs);color:${scc};margin-left:auto">${sc}/100</span></div>${emptyMsgHtml}${rulesHtml}${portsHtml}${issuesHtml}</div>`;
  }).join('');
}
...
var _FW_VM_ID_MAP={106:'site01',107:'site02',108:'srv-ngix'};
...
function _fwSvcUp(n,hm,svcs,pvmStatus,nodes){
  ...
  if(n==='site01'){var s=svcs['site01 (0xcyberlitech.com)'];return s?(s.status==='UP'):null;}
  if(n==='site02'){var s=svcs['site02 (0xcyberlitech.fr)'];return s?(s.status==='UP'):null;}
...
function drawFwFlux(d){
  ...
  var hm={};
  ((d.firewall_matrix||{}).hosts||[]).forEach(function(h){hm[h.name]=h;if(h.name.match(/^srv-/))hm[h.name.replace(/^srv-/,'')]=h;});
  ...
  var NODES=[
    ...
    {id:'srv-ngix',lbl:'srv-ngix', sub:SOC_INFRA.SRV_NGIX,     c:'#00d9ff',zc:'0,217,255', role:'PROXY+FW',  xf:0.50,zone:'dmz',          ports:[{t:':80→site01',c:'#00ff88'},{t:' · ',c:'rgba(180,200,230,0.3)'},{t:':80→site02',c:'#00d9ff'},{t:' · ',c:'rgba(180,200,230,0.3)'},{t:':8006',c:'#bf5fff'}]},
    {id:'site01',     lbl:'site01',      sub:SOC_INFRA.SITE01,     c:'#00ff88',zc:'0,255,136', role:'WEB',       xf:0.28,zone:'int',          ports:[{t:':80 ← ngix',c:'#00ff88'}]},
    {id:'site02',    lbl:'site02',     sub:SOC_INFRA.SITE02,     c:'#00d9ff',zc:'0,217,255', role:'WEB+SMTP',  xf:0.72,zone:'int',          ports:[{t:':80 ← ngix',c:'#00d9ff'},{t:' · ',c:'rgba(180,200,230,0.3)'},{t:':587→smtp',c:'#f97316'}]},
  ...
  var WALLS=[
    {y:_fwZBound('ext'),wide:true,host:'srv-ngix',c:'#00d9ff',
     ...
     outTxt:'OUT: site01:80 · site02:80 · :8006 · :2272 SSH · DNS'},
    {y:_fwZBound('dmz'),split:true,
     left: {x:W*0.28,host:'site01', c:'#00ff88',lbl:'UFW site01', inTxt:'IN: :80 ← ngix',outTxt:'OUT: :443 :80 DNS'},
     right:{x:W*0.72,host:'site02',c:'#00ff88',lbl:'UFW site02',inTxt:'IN: :80 ← ngix',outTxt:'OUT: :443 :80 :587 DNS'}},
  ...
  // Connexions — HTTPS, HTTP, SSH visibles, symétrie site01/site02
  var CONNS=[
    ...
    // srv-ngix → site01 : HTTP + SSH (deux parallèles, côté gauche)
    {f:'srv-ngix',t:'site01',     port:':80',   type:'HTTP',    c:'#00ff88',w:1.5,           xOff:-14},
    {f:'srv-ngix',t:'site01',     port:':2272', type:'SSH',     c:'#ffd700',w:1.1,dash:true, xOff:2},
    // srv-ngix → site02 : HTTP + SSH (miroir — symétrie)
    {f:'srv-ngix',t:'site02',    port:':80',   type:'HTTP',    c:'#00d9ff',w:1.5,           xOff:14},
    {f:'srv-ngix',t:'site02',    port:':2272', type:'SSH',     c:'#ffd700',w:1.1,dash:true, xOff:-2},
    ...
    // site02 → internet : SMTP sortant (rev, côté droit)
    {f:'site02',    t:'internet',port:':587',  type:'SMTP OUT',c:'#f97316',w:1.5,dash:true, rev:true,xOff:42,label_t:0.22},
...
function drawFwTopo(d){
  ...
  // Normalisation : strip "srv-" pour matcher firewall_matrix (srv-clt→site01)
  ...
  if(!pveVms.length){
    pveVms=[
      {id:106,name:'srv-site01', status:'running',type:'vm',cpu:null,mem_pct:null},
      {id:107,name:'srv-site02',status:'running',type:'vm',cpu:null,mem_pct:null},
    ];
  }
