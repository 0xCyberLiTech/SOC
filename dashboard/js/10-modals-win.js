...
function _showIpHud(h,stageCol,ip,dot,evt){
  var hud=document.getElementById('ip-hud');
...
function openIpModal(dot,evt){
  ...
  var SCOL={EXPLOIT:KC_PALETTE.EXPLOIT.color,BRUTE:KC_PALETTE.BRUTE.color,SCAN:KC_PALETTE.SCAN.color,RECON:KC_PALETTE.RECON.color};
  ...
  var csHtml=dot.cs_banned||csInfo.scenario
    ? _ipRow('Statut','<span style="color:var(--green)">✓ BANNI</span>')
      + _ipRow('Scénario',`<span style="color:var(--cyan);font-size:var(--fs-xs)">${esc(fmtScenario(csInfo.scenario||dot.cs_scenario||csMeta.scenario||'—'))}</span>`)
  ...
  if(surMeta){
    ...
    var rcritRows=rcritIp.slice(0,2).map(function(a){
      return `<div style="font-size:var(--fs-xs);font-family:'Courier New',monospace;padding:.15rem 0;border-bottom:1px solid rgba(255,107,53,0.1);color:'+_COL_WIN_SUR_HEX+';line-height:1.4;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
<span style="color:var(--muted);margin-right:.35rem">${esc((a.ts||'').slice(11,16))}</span>${esc((a.signature||'?').slice(0,38))}${(a.signature||'').length>38?'…':''}</div>`;
    ...
    surHtml = _ipRow('Alertes 24h',`<span style="color:'+_COL_WIN_SUR_HEX+'">${surMeta.count}</span>`)
...
function pbCol(pct){return pct>_THR_GPU_CRIT?'r':pct>_THR_CPU_WARN?'y':'g';}
function tempCol(t){return t===null||t===undefined?'var(--muted)':t>_THR_TEMP_CRIT?'var(--red)':t>_THR_TEMP_WARN?'var(--amber)':'var(--green)';}
...
function _winDriveRowHtml(dv){
  var dPct=dv.pct||0;
  var dc=dPct>_THR_CPU_CRIT?'r':dPct>_THR_CPU_WARN?'y':'g';
...
// ── MODAL RESSOURCES ──────────────────────────────────
function _buildWinGpuHtml(gpu){
  var gpuPct=gpu.usage||0;
  var gpuStroke=gpuPct>_THR_GPU_CRIT?'var(--red)':gpuPct>_THR_CPU_WARN?'var(--yellow)':'var(--purple)';
  var gpuTempColor=tempCol(gpu.temp);
  var vramPct=gpu.vram_pct||0;
  return `<div class="win-section">
...
function _buildWinNetIoHtml(wdNet,wdDIO){
  if(wdNet.up_mbs===undefined&&wdDIO.read_mbs===undefined) return '';
...
function buildWindowsModal(wd){
  ...
  var cpuStroke=cpuPct>_THR_GPU_CRIT?'var(--red)':cpuPct>_THR_CPU_WARN?'var(--yellow)':'var(--green)';
  ...
  var gpuHtml=gpu&&gpu.name?_buildWinGpuHtml(gpu):'';
  var netIoHtml=_buildWinNetIoHtml(wdNet,wdDIO);
  ...
  var ramStroke=ramPct>_THR_GPU_CRIT?'var(--red)':ramPct>_THR_CPU_WARN?'var(--yellow)':'var(--green)';

  var wdJarvis=wd.jarvis||null;
  var jarvisModalHtml=wdJarvis
    ? `<div class="win-section" style="margin-top:.6rem">
<div class="win-section-hdr"><span style="color:var(--purple)">▸</span> JARVIS — SERVICES LOCAUX</div>
<div style="display:flex;gap:1.4rem;font-size:var(--fs-xs);font-family:Courier New,monospace;padding:.3rem 0">
<span>Flask <span style="color:var(--muted)">:5000</span> <span style="color:${wdJarvis.flask_up?'var(--green)':'var(--red)'}">● ${wdJarvis.flask_up?'UP':'DOWN'}</span></span>
<span>MCP <span style="color:var(--muted)">:5010</span> <span style="color:${wdJarvis.mcp_up?'var(--green)':'var(--red)'}">● ${wdJarvis.mcp_up?'UP':'DOWN'}</span></span>
</div></div>` : '';
...
// ── MODAL SAUVEGARDES ─────────────────────────────────
function _buildBkpCronHtml(crons){
...
function buildBackupModal(wd,d){
  ...
  var crons=wd.cron||[];
  var bkpPath=bk.path||'D:\\BACKUP-PROXMOX';
  var bkPct=bk.pct||0, bkStroke=bkPct>_THR_CPU_CRIT?'var(--red)':bkPct>_THR_CPU_WARN?'var(--yellow)':'var(--green)';
  ...
  var cronHtml=_buildBkpCronHtml(crons);

  // Deux zones côte à côte
  var zonesHtml='<div style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem;margin-bottom:.65rem">'
    +_bkpZoneTableHtml(manual,bkpPath+'\\manual\\',_allVms)
    +_bkpZoneTableHtml(auto,bkpPath+'\\auto\\',_allVms)
    +'</div>';
  ...
  var quotaHtml='<div class="win-section">'
    +'<div class="win-section-hdr"><span style="color:var(--cyan)">▸</span> QUOTA '+esc(bkpPath)+'</div>'
...
// ══════════════════════════════════════════════════════
function _gpuEncDecHtml(gpu){
  if(gpu.enc_pct==null&&gpu.dec_pct==null&&gpu.mem_ctrl_pct==null)return '';
  var encItems='';
  if(gpu.enc_pct!=null){
    var eC=gpu.enc_pct>_THR_GPU_CRIT?'pb-r':gpu.enc_pct>40?'pb-y':'pb-g';
  ...
  if(gpu.dec_pct!=null){
    var dC=gpu.dec_pct>_THR_GPU_CRIT?'pb-r':gpu.dec_pct>40?'pb-y':'pb-g';
  ...
  if(gpu.mem_ctrl_pct!=null){
    var mC=gpu.mem_ctrl_pct>_THR_GPU_CRIT?'pb-r':gpu.mem_ctrl_pct>_THR_CPU_WARN?'pb-y':'pb-g';
  ...
  return `<div class="gpu-enc-row">${encItems}</div>`;
}
function _gpuStatsCards(gpu,pci){
...
function _gpuChartsHtml(){
  return `<div class="gpu-charts-grid">
...
function _gpuThrottleHtml(gpu,ps){
  if(!ps&&gpu.thr_power===undefined)return '';
  var pStateBox=ps?`<div class="gpu-thr-item ${ps==='P0'?'gpu-thr-ok':ps==='P8'||ps==='P12'?'gpu-thr-idle':'gpu-thr-mid'}"><div class="gpu-thr-lbl">P-STATE</div><div class="gpu-thr-val">${esc(ps)}</div><div class="gpu-thr-sub">${ps==='P0'?'MAX PERF':ps==='P8'||ps==='P12'?'IDLE':ps==='P5'||ps==='P4'?'ÉCONOMIE':'NORMAL'}</div></div>`:'';
  ...
  var thrHw=gpu.thr_hw!==undefined?`<div class="gpu-thr-item ${gpu.thr_hw?'gpu-thr-warn':'gpu-thr-ok'}"><div class="gpu-thr-lbl">HW LIMIT</div><div class="gpu-thr-val">${gpu.thr_hw?'⚡ ACTIF':'✓ OFF'}</div><div class="gpu-thr-sub">Fréquence réduite</div></div>`:'';
  var thrTh=gpu.thr_thermal!==undefined?`<div class="gpu-thr-item ${gpu.thr_thermal?'gpu-thr-warn':'gpu-thr-ok'}"><div class="gpu-thr-lbl">SW THERMAL</div><div class="gpu-thr-val">${gpu.thr_thermal?'⚡ ACTIF':'✓ OFF'}</div><div class="gpu-thr-sub">Protection T°</div></div>`:'';
  ...
  return `<div class="gpu-throttle-section"><div class="win-section-hdr" style="color:rgba(191,95,255,0.7)"><span style="color:var(--purple)">&#9658;</span> P-STATE &amp; THROTTLE</div><div class="gpu-thr-grid">${pStateBox}${thrPower}${thrHw}${thrTh}${thrHwT}</div></div>`;
}
function buildGpuModal(wd){
  ...
  var gpuStroke=gpuPct>_THR_GPU_CRIT?'var(--red)':gpuPct>_THR_CPU_WARN?'var(--yellow)':'var(--purple)';
  var vramPct=gpu.vram_pct||0;
  var vramStroke=vramPct>_THR_GPU_CRIT?'var(--red)':vramPct>_THR_CPU_WARN?'var(--yellow)':'var(--cyan)';
  ...
  var pwrStroke=pwrPct>_THR_CPU_CRIT?'var(--red)':pwrPct>_THR_CPU_WARN?'var(--amber)':'var(--green)';
  ...
  var pci=gpu.pcie_gen&&gpu.pcie_width?'Gen '+gpu.pcie_gen+' × '+gpu.pcie_width+' lanes':'';
  var pwrGaugeHtml=gpu.power_pct!=null
    ...
    :'';
  var encDecHtml=_gpuEncDecHtml(gpu);
  var statsCards=_gpuStatsCards(gpu,pci);
  var thrHtml=_gpuThrottleHtml(gpu,ps);
...
${pci?`<span class="gpu-hdr-badge" style="color:rgba(0,217,255,0.7);border-color:rgba(0,217,255,0.3);background:rgba(0,217,255,0.07)">PCIe ${pci.replace('Gen ','').replace(' lanes','')}</span>`:''}
...
<span style="font-size:var(--fs-xs);color:rgba(191,95,255,0.5)">${gpu.driver_ver&&gpu.driver_ver!=='N/A'?'Driver '+esc(gpu.driver_ver)+' · ':''}NVIDIA GPU · CUDA COMPUTE</span>
</div>
...
<div class="gpu-gauge-item">${winGaugeSVG(vramPct,vramStroke,'VRAM',vramUsedGo+'/'+vramTotalGo+' Go')}<div class="gpu-gauge-lbl" style="color:rgba(0,217,255,0.6)">Mémoire</div></div>
...
<div class="win-section-hdr" style="color:var(--cyan)"><span style="color:var(--cyan)">&#9658;</span> VRAM &mdash; Répartition mémoire</div>
...
function openWindowsModal(wd,type){
  ...
  _modalBody.style.fontSize='1em';
  if(type==='bkp'){
    if(!_modalBody._boundBkp){_modalBody._boundBkp=true;_modalBody.addEventListener('click',function(e){
