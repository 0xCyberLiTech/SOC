...
function drawNetDualSparkline(target,rxVals,txVals,maxVal){
  ...
  drawChannel(buildPts(rxVals,true),_COL_CYAN_RGB,true);
  drawChannel(buildPts(txVals,false),KC_PALETTE.BLOCKED.rgb,false);
}

// ── Graphe latence WAN — histPts : [{ts (ISO), wan_ms, box_ms, status}] ──
function _lcDrawGrid(ctx,PAD_L,PAD_T,cW,cH,yMax,pts,tStart,tRange,H){
...
[0,30,80,yMax].forEach(function(ms){
  ...
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.07)'; ctx.lineWidth=0.5;
...
[[30,KC_PALETTE.BLOCKED.rgb],[80,'255,160,0']].forEach(function(th){
  if(th[0]>=yMax)return;
...
function _lcDrawLatLine(ctx,coords,PAD_L,PAD_T,cW,cH){
...
aG.addColorStop(0,'rgba('+_COL_CYAN_RGB+',0.30)');
aG.addColorStop(0.55,'rgba('+_COL_CYAN_RGB+',0.10)');
aG.addColorStop(1,'rgba('+_COL_CYAN_RGB+',0.01)');
...
ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.90)'; ctx.lineWidth=1.5;
ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.45)'; ctx.shadowBlur=5;
ctx.stroke(); ctx.shadowBlur=0;
}

function _lcDrawFinish(ctx,pts,coords,PAD_L,PAD_T,cW,cH,W,H,PAD_B){
...
for(var i=0;i<pts.length;i+=step){
  ...
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.06)'; ctx.lineWidth=0.5;
...
// Bordure verre
ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.12)'; ctx.lineWidth=1;
ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.06)'; ctx.shadowBlur=3;
...
ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.85)'; ctx.shadowBlur=8;
ctx.fill(); ctx.shadowBlur=0;
}
...
function drawLatencyChart(canvas, histPts){
  ...
  var tRange=tEnd-tStart||pts.length*60;  _lcDrawGrid(ctx,PAD_L,PAD_T,cW,cH,yMax,pts,tStart,tRange,H);

  // Coordonnées
  var coords=pts.map(function(p,i){return[_lcXOf(i,pts,PAD_L,tStart,tRange,cW),_lcYOf(p.wan_ms,PAD_T,cH,yMax)];});  _lcDrawLatLine(ctx,coords,PAD_L,PAD_T,cW,cH);
  _lcDrawFinish(ctx,pts,coords,PAD_L,PAD_T,cW,cH,W,H,PAD_B);
...
// period : '24h' | '7d'
function _tcDrawBackground(ctx,satThresh,yMax,PAD_L,PAD_T,cW,cH){
...
function _tcDrawGridLines(ctx,N_YTICKS,yMax,period,tStart,tEnd,tRange,cW,cH,PAD_L,PAD_T,PAD_B,H){
  ...
  for(var ts=tFirst;ts<=tEnd;ts+=xTickInterval){
    ...
    ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.08)'; ctx.lineWidth=0.5;
...
function _tcDrawGapBands(ctx,gapBands,PAD_T,cH){
...
function _tcDrawSpikeMarkers(ctx,txSpikes,rxSpikes,txC,rxC,hasSpikes,PAD_T,cH,PAD_L,cW){
...
function _tcDrawDecorations(ctx,PAD_L,PAD_T,cW,cH,H,PAD_B){
  ctx.strokeStyle='rgba('+_COL_CYAN_RGB+',0.12)'; ctx.lineWidth=1;
  ctx.shadowColor='rgba('+_COL_CYAN_RGB+',0.08)'; ctx.shadowBlur=3;
  ctx.strokeRect(PAD_L,PAD_T,cW,cH); ctx.shadowBlur=0;
...
function drawTrafficChart(canvas, data, period, opts){
  ...
  _tcDrawBackground(ctx,satThresh,yMax,PAD_L,PAD_T,cW,cH);
  _tcDrawGridLines(ctx,N_YTICKS,yMax,period,tStart,tEnd,tRange,cW,cH,PAD_L,PAD_T,PAD_B,H);
  _tcSplitSegs(_tcPtCoords(rxClean,timestamps,tStart,tRange,cW,PAD_L,PAD_T,cH,yMax),gapIdx).forEach(function(sg){_tcDrawSeries(ctx,sg,KC_PALETTE.BLOCKED.rgb,0.24,PAD_T,cH);});
  _tcSplitSegs(_tcPtCoords(txClean,timestamps,tStart,tRange,cW,PAD_L,PAD_T,cH,yMax),gapIdx).forEach(function(sg){_tcDrawSeries(ctx,sg,_COL_CYAN_RGB,0.30,PAD_T,cH);});
  _tcDrawGapBands(ctx,gapBands,PAD_T,cH);
  var txC=_tcPtCoords(txClean,timestamps,tStart,tRange,cW,PAD_L,PAD_T,cH,yMax);
  var rxC=_tcPtCoords(rxClean,timestamps,tStart,tRange,cW,PAD_L,PAD_T,cH,yMax);
  _tcDrawSpikeMarkers(ctx,txSpikes,rxSpikes,txC,rxC,hasSpikes,PAD_T,cH,PAD_L,cW);
  _tcDrawDecorations(ctx,PAD_L,PAD_T,cW,cH,H,PAD_B);
