...
function _tcFmtBps(bps){var k=bps*8/1000;if(k>=1000000)return (k/1000000).toFixed(2)+' Gbps';if(k>=1000)return (k/1000).toFixed(k>=10000?0:1)+' Mbps';return Math.round(k)+' Kbps';}
...
function _setMapZone(zone,btn){
  window._mapZone=zone; _mapStaticCanvas=null;
  document.querySelectorAll('.map-zone-btn').forEach(function(b){b.classList.remove('active');});
  if(btn)btn.classList.add('active');
  if(_lfLastGeoips&&_lfLastGeoips.length)drawGeoMap(_lfLastGeoips,null,_lfLastWanIp);
}

// ── LEAFLET INTERACTIVE MAP ──
var _lfMap=null, _lfMarkers=[], _lfArcs=[], _lfHomeMarker=null, _lfStatsCtrl=null;
