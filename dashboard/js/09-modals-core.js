...
var _SUR_PROTO_ITEMS=[{k:'SUR_DNS',col:_COL_GREEN_HEX,l:'DNS'},{k:'SUR_HTTP',col:PROTO_PALETTE.WATCH,l:'HTTP'},{k:'SUR_TLS',col:_COL_CYAN_HEX,l:'TLS/SSL'},{k:'SUR_SSH',col:PROTO_PALETTE.RECON,l:'SSH'},{k:'SUR_FLOW',col:PROTO_PALETTE.FLOW,l:'FLOW'}];
function _mBuildSurSummaryHtml(surData){
  ...
  var threats='';
  if(sd.SUR_SCAN>0) threats+='<span style="padding:.12rem .35rem;background:rgba(255,59,92,0.15);border:1px solid rgba(255,59,92,0.4);border-radius:2px;font-size:var(--fs-xs);color:var(--red);font-weight:700">⚡ SCAN '+sd.SUR_SCAN+' hits</span> ';
  if(sd.SUR_C2>0)   threats+='<span style="padding:.12rem .35rem;background:rgba(220,38,38,0.18);border:1px solid rgba(220,38,38,0.45);border-radius:2px;font-size:var(--fs-xs);color:'+_COL_C2_BADGE_HEX+';font-weight:700">☠ C2 ×'+sd.SUR_C2+'</span>';
...
function _surKpiGaugeHtml(sp){
  ...
  var thr=scanHits||c2
    ?'<div style="display:flex;gap:.4rem;margin-top:.4rem;flex-wrap:wrap">'
      +(scanHits?'<span style="padding:.15rem .4rem;background:rgba(255,59,92,0.15);border:1px solid rgba(255,59,92,0.4);border-radius:3px;font-size:var(--fs-xs);color:var(--red);font-weight:700">⚡ SCAN '+scanHits+' hits</span>':'')
      +(c2?'<span style="padding:.15rem .4rem;background:rgba(220,38,38,0.18);border:1px solid rgba(220,38,38,0.45);border-radius:3px;font-size:var(--fs-xs);color:'+_COL_C2_BADGE_HEX+';font-weight:700">☠ C2/TROJAN ×'+c2+'</span>':'')
      +'</div>':'';
...
var _PROTO_ARC_CIRC=251.33; // NDT-232 — 2π×40, circonférence arc protocoles (r=40)
// NDT-230 — _setActive promu module-level (était fermé sur b5/b24/bSur dans _raf2)
function _protoSetActive(btn,btns){btns.forEach(function(b){if(b)b.classList.remove('active');});if(btn)btn.classList.add('active');}
...
function _mcUpdateProtoPane(proto,rpm,_modalBody,surData){
  ...
  if(af){af.setAttribute('stroke-dasharray',Math.max(3,Math.round(s.hp/100*_PROTO_ARC_CIRC))+' 999');af.setAttribute('stroke',s.ac);af.style.filter='drop-shadow(0 0 6px '+s.ac+')';}
...
function _openModalProtoSetup(){
  ...
  _modalBody.innerHTML=
    ...
    +'<path id="pm-arc-fill" d="M 10,90 A 80,80 0 0,1 190,90" fill="none" stroke="'+s0.ac+'" stroke-width="14" stroke-linecap="round" stroke-dasharray="'+Math.max(3,Math.round(s0.hp/100*_PROTO_ARC_CIRC))+' 999" style="filter:drop-shadow(0 0 6px '+s0.ac+')"/>'
  ...
  _raf2(function(){
    ...
    if(b5)b5.onclick=function(){
      _protoSetActive(b5,[b5,b24,bSur]);mainPane.style.display='';surPane.style.display='none';
      _mcUpdateProtoPane(proto5,(full5.rpm||'—')+' req/min',_modalBody,surData);
    };
    if(b24)b24.onclick=function(){
      _protoSetActive(b24,[b5,b24,bSur]);mainPane.style.display='';surPane.style.display='none';
      _mcUpdateProtoPane(proto24,'données 24h',_modalBody,surData);
    };
    if(bSur)bSur.onclick=function(){
      _protoSetActive(bSur,[b5,b24,bSur]);mainPane.style.display='none';surPane.style.display='';
      var rEl=_modalBody.querySelector('#pt-rpm');if(rEl)rEl.textContent='Suricata IDS réseau';
    };
    if(!hasLive&&bSur){_protoSetActive(bSur,[b5,b24,bSur]);mainPane.style.display='none';surPane.style.display='';}
  });
}
function _openModalRafCb(isMap,isHeat,isKc,isProto){
  if(isMap){
  ...
  if(hmc&&window._lastTraffic){
    ...
    // Détection mode via ID : heatmap-canvas (TRAFIC) | heatmap-canvas-atk (ATTAQUES)
    var _hmMode = (hmc.id==='heatmap-canvas-atk') ? 'attacks' : 'traffic';
    // ⚠ ID duplication : innerHTML clone garde l'ID original déjà présent dans la card.
    // Si on laisse l'ID dupliqué, drawAttackHeatmap peut perturber le rendu live de la
    // card en arrière-plan. On renomme le canvas modal pour isoler.
    hmc.id = (hmc.id||'heatmap-canvas') + '-modal';
    // Retry boucle si offsetWidth=0 (transition CSS modal en cours, max 500ms)
    var _retries = 0;
    var _drawWhenReady = function(){
      if(hmc.offsetWidth > 0 || _retries > 10){
        animateAttackHeatmap(hmc,tt.requests_per_hour||{},tt.blocks_per_hour||{},tt.errors_per_hour||{},260,_hmMode);
      } else {
        _retries++;
        setTimeout(_drawWhenReady, 50);
      }
    };
    _drawWhenReady();
  }
  ...
  if(isProto) _openModalProtoSetup();
}
function openModal(cardEl){
  ...
  else if(isHeat||isKc) mc.classList.add('modal-wide');
  ...
  _modalBody.classList.add('modal-fs-lg');
  _overlay.classList.add('open');
  document.body.style.overflow='hidden';
  _raf2(function(){ _openModalRafCb(isMap,isHeat,isKc,isProto); });
}
...
// ════════════════════════════════════════════════════════
// ── openSysModal helpers ─────────────────────────────────────────────────────
function _sysNginxSecHtml(traffic, reqVals){
  ...
  return '<div class="win-section"><div class="win-section-hdr"><span style="color:var(--cyan)">▸</span> NGINX — MOTEUR</div>'
...
function _sysCmSecHtml(fw, f2b, cs){
  ...
  return '<div class="win-section"><div class="win-section-hdr"><span style="color:var(--green)">▸</span> CONTRE-MESURES</div>'
...
function _sysAfterRender(h, reqVals){
  _modalBody.innerHTML=h;
  _modalBody.classList.remove('modal-fs-lg');
  _overlay.classList.add('open');
  _isOpen=true;
  document.body.style.overflow='hidden';
  animateGauges();
...
function openSysModal(data){
  ...
  var fw=((data.firewall_matrix&&data.firewall_matrix.hosts)||[]).find(function(h){return h.name==='srv-nginx';})||{};
  ...
  var mc=document.getElementById('modal-card');
  mc.classList.remove('modal-wide','modal-proto','modal-xl','modal-win','modal-gpu','modal-kci','modal-geomap','modal-auto-height','theme-red','theme-green','theme-cyan','theme-orange','theme-yellow','theme-purple');
  ...
  var cpuC=cpu>_THR_CPU_CRIT?'var(--red)':cpu>_THR_CPU_WARN?'var(--yellow)':'var(--cyan)';
  ...
  var memC=memPct!=null?(memPct>_THR_CPU_CRIT?'var(--red)':memPct>_THR_CPU_WARN?'var(--yellow)':'var(--purple)'):'var(--muted)';
  ...
  var reqVals=Object.keys(traffic.requests_per_hour||{}).sort().map(function(k){return (traffic.requests_per_hour||{})[k]||0;});
  var nginxSec=_sysNginxSecHtml(traffic, reqVals);
  ...
  var netSec=_netIfaces.length?'<div class="win-section"><div class="win-section-hdr"><span style="color:var(--cyan)">▸</span> RÉSEAU <span style="font-size:calc(var(--fs-xs) - 1px);color:rgba(122,154,184,.3);font-weight:400;margin-left:.4rem;font-family:\'Courier New\',monospace">— cumul session</span></div>'
    ...
    +'</div>':'';
  var cmSec=_sysCmSecHtml(fw, f2b, cs);
...
function _vmCardHtml(v,vi){
  ...
  var mc=v.mem_pct>_THR_CPU_CRIT?'var(--red)':v.mem_pct>_THR_CPU_WARN?'var(--yellow)':'var(--purple)';
...
// ════════════════════════════════════════════════════════
// ── openProxmoxModal helpers ─────────────────────────────────────────────────
function _prxNetworkSecHtml(netIfaces, pveNetHist){
  if(!netIfaces.length) return '';
  return '<div class="win-section"><div class="win-section-hdr"><span style="color:var(--cyan)">▸</span> RÉSEAU <span style="font-size:calc(var(--fs-xs) - 1px);color:rgba(122,154,184,.3);font-weight:400;margin-left:.4rem;font-family:\'Courier New\',monospace">— 4H</span></div>'
...
function _prxVmsSectionHtml(vms){
  ...
  return '<div class="win-section"><div class="win-section-hdr"><span style="color:var(--yellow)">▸</span> MACHINES VIRTUELLES</div>'
...
function _prxProcsSecHtml(topProcs){
  if(!topProcs.length) return '';
  return '<div class="win-section"><div class="win-section-hdr"><span style="color:var(--green)">▸</span> PROCESSUS SYSTÈME</div>'
...
function _prxAfterRender(netIfaces, pveNetHist){
  _modalBody.classList.remove('modal-fs-lg');
  _overlay.classList.add('open');
  _isOpen=true;
  document.body.style.overflow='hidden';
...
function openProxmoxModal(data){
  ...
  var mc=document.getElementById('modal-card');
  mc.classList.remove('modal-wide','modal-proto','modal-xl','modal-win','modal-gpu','modal-kci','modal-geomap','modal-auto-height','theme-red','theme-green','theme-cyan','theme-orange','theme-yellow');
  ...
  var cpuC=cpuPct>_THR_CPU_CRIT?'var(--red)':cpuPct>_THR_CPU_WARN?'var(--yellow)':'var(--purple)';
  var cpuTmpC=node.cpu_temp!=null?(node.cpu_temp>_THR_TEMP_CRIT?'var(--red)':node.cpu_temp>_THR_TEMP_WARN?'var(--yellow)':'var(--cyan)'):'var(--muted)';
  var acpiTmpC=node.acpi_temp!=null?(node.acpi_temp>_THR_ACPI_CRIT?'var(--red)':node.acpi_temp>_THR_ACPI_WARN?'var(--yellow)':'var(--cyan)'):'var(--muted)';
  ...
  var resourceBarsHtml=(function(){
    ...
    var mc=memPct!=null?(memPct>_THR_CPU_CRIT?'var(--red)':memPct>_THR_CPU_WARN?'var(--yellow)':'var(--purple)'):'var(--muted)';
    return '<div style="margin-top:.35rem">'+_rbar('CPU',cpuPct,cpuC)+(memPct!=null?_rbar('RAM',memPct,mc):'')+'</div>';
  })();
  ...
  var networkSectionHtml=_prxNetworkSecHtml(netIfaces,pveNetHist);
  var vms=node.vms||[];
  var vmsSectionHtml=_prxVmsSectionHtml(vms);
  var topProcs=node.top_procs||[];
  var procsSectionHtml=_prxProcsSecHtml(topProcs);
  ...
  _modalBody.innerHTML=h;
  _prxAfterRender(netIfaces,pveNetHist);
}
;
...
// NDT-229 — mini-bar Suricata promu module-level (était fermé dans openSuricataModal)
function _surMiniBar(pct,col,lbl){
...
// ── openSuricataModal helpers ────────────────────────────────────────────────
function _surBannerHtml(s1){
  if(!s1) return '';
  return '<div style="display:flex;align-items:center;gap:.6rem;padding:.35rem .9rem;border-left:3px solid var(--red);background:rgba(255,59,92,.07);border-radius:0 4px 4px 0;margin-bottom:1.2rem;animation:blink 1.5s ease-in-out infinite">'
...
function _surKpiHtml(s1, s2, s3, tot, trunc){
  return '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:0;border:1px solid rgba(255,255,255,.07);border-radius:6px;overflow:hidden;margin-bottom:1.2rem">'
    +[
...
function _surProtoHtml(evts, rules){
...
function _surTopIpsHtml(topIps){
  ...
  var ipMax=topIps[0].count||1;
  return '<div><div style="font-size:var(--fs-2xs);color:var(--c-label);text-transform:uppercase;letter-spacing:.8px;margin-bottom:.5rem">Top IPs attaquantes</div>'
...
function _surCritAlertsHtml(rc){
  if(!rc.length) return '<div style="color:var(--green);font-size:var(--fs-xs);font-family:\'Courier New\',monospace;padding:1.5rem 0;text-align:center">✓ Aucune alerte critique en 24h</div>';
  return rc.map(function(a){
...
function _surSigsHtml(topSigs){
  if(!topSigs.length) return '<div style="color:var(--muted);font-size:var(--fs-xs);text-align:center;padding:1.5rem 0">Aucune signature détectée</div>';
  var sigMax=topSigs[0].count||1;
  return topSigs.map(function(s){
...
function _surCircuitCss(){
  return '<style>'
    ...
    +'.sur-dot-on{width:5px;height:5px;border-radius:50%;background:var(--green);flex-shrink:0;animation:sur-pulse .8s ease-in-out infinite}'
    +'.sur-wire{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,100,50,0) 0,rgba(255,100,50,0) 5px,rgba(255,100,50,.7) 5px,rgba(255,100,50,.7) 9px,rgba(255,100,50,0) 9px,rgba(255,100,50,0) 24px);background-size:24px 100%;animation:sur-flow .5s linear infinite}'
    +'.sur-wire-fat{height:4px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,120,60,0) 0,rgba(255,120,60,0) 4px,rgba(255,120,60,.9) 4px,rgba(255,120,60,.9) 10px,rgba(255,120,60,0) 10px,rgba(255,120,60,0) 24px);background-size:24px 100%;animation:sur-flow .3s linear infinite}'
    +'.sur-wire-drop{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,59,92,0) 0,rgba(255,59,92,0) 5px,rgba(255,59,92,.9) 5px,rgba(255,59,92,.9) 9px,rgba(255,59,92,0) 9px,rgba(255,59,92,0) 24px);background-size:24px 100%;animation:sur-flow .22s linear infinite}'
    +'.sur-wire-crit{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,140,0,0) 0,rgba(255,140,0,0) 4px,rgba(255,140,0,.85) 4px,rgba(255,140,0,.85) 9px,rgba(255,140,0,0) 9px,rgba(255,140,0,0) 24px);background-size:24px 100%;animation:sur-flow .35s linear infinite}'
    +'.sur-node{border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;font:700 9px "Courier New",monospace;letter-spacing:.06em;padding:.35rem .5rem;border:1px solid;text-align:center}'
    +'.sur-bar-high{background:linear-gradient(90deg,#ff3b5c44,var(--red),#ff6b8844,var(--red));background-size:200% 100%;animation:sur-shimmer .6s linear infinite}'
    +'.sur-bar-med{background:linear-gradient(90deg,#00d9ff44,var(--cyan),#40eeff44,var(--cyan));background-size:200% 100%;animation:sur-shimmer 1.2s linear infinite}'
    ...
    +'.sur-bar-drop{background:linear-gradient(90deg,#ff000044,var(--red),#ff000044);background-size:200% 100%;animation:sur-shimmer .3s linear infinite}'
    +'</style>';
}
function _surNicHtml(nic, gPkts){
  var hasNicData=Object.keys(nic).length>0||gPkts>0;
  var nicErrors=(nic.rx_errors||0)+(nic.tx_errors||0), nicDrops=nic.rx_drop||0;
  return '<div class="sur-node" style="border-color:rgba(0,217,255,.4);background:rgba(0,217,255,.07);color:rgba(0,217,255,.9);min-width:82px">'
    +'<div style="display:flex;align-items:center;gap:.25rem;margin-bottom:.12rem"><span style="font-size:var(--fs-xs)">⬡</span>'+(hasNicData?'<div class="sur-dot-on"></div>':'')+'</div>'
    ...
    +'<div style="display:flex;gap:.1rem;margin-bottom:.22rem;justify-content:center">'+_sPill('PROMIS','rgba(0,217,255,.7)')+_sArr()+_sPill('CAP','rgba(0,217,255,.7)')+_sArr()+_sPill('FILTER','rgba(0,217,255,.7)')+'</div>'
    +(Object.keys(nic).length
      ?'<div style="text-align:left;width:100%"><div style="font-size:var(--fs-3xs);color:rgba(0,217,255,.5)">RX '+fmtBytes(nic.rx_bytes||0)+'</div>'
        +'<div style="font-size:var(--fs-3xs);color:rgba(0,160,200,.45)">TX '+fmtBytes(nic.tx_bytes||0)+'</div>'
        +'<div style="font-size:var(--fs-3xs);color:'+(nicErrors>0?'var(--red)':'rgba(0,255,136,.7)')+';margin-top:.08rem">'+(nicErrors>0?'⚠ '+nicErrors+' err':'✓ 0 err')+'</div>'
        +(nicDrops>0?'<div style="font-size:var(--fs-3xs);color:rgba(255,59,92,.6)">'+nicDrops+'↓ drops</div>':'')+'</div>'
      :(gPkts>0?'<div style="font-size:var(--fs-3xs);color:rgba(0,217,255,.5);margin-top:.1rem">'+fmt(gPkts)+' pkts</div>':''))
    +'</div>';
}
function _surRingHtml(satPct, satCol, ringProcPct, ringDropPct, ringTruncPct, trunc){
  return '<div class="sur-node" style="border-color:'+satCol+'88;background:rgba(20,10,0,.4);color:'+satCol+';min-width:92px;position:relative">'
    +'<div style="font-size:var(--fs-3xs);opacity:.55;margin-bottom:.15rem">AF-PACKET</div>'
    +'<div style="display:flex;gap:.1rem;margin-bottom:.18rem;justify-content:center">'+_sPill('AF_PKT',satCol)+_sArr()+_sPill('RING',satCol)+_sArr()+_sPill('KERN',satCol)+'</div>'
    +'<div style="position:relative;width:44px;height:44px;margin-bottom:.12rem">'+ring(satPct,satCol,44)
    +'<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center"><span style="font:700 9px \'Courier New\';color:'+satCol+'">'+satPct+'%</span></div></div>'
    ...
    +'</div>'
    +(trunc>0?'<div style="font-size:var(--fs-3xs);color:'+satCol+';margin-top:.04rem">'+fmt(trunc)+'↓ tronq.</div>':'<div style="font-size:var(--fs-3xs);color:var(--cyan);margin-top:.04rem">✓ 0 tronq.</div>')
    +'<div style="font-size:var(--fs-3xs);opacity:.4;margin-top:.03rem">ring-size: 100k</div>'
    +'</div>';
}
function _surWorkersPipeHtml(workers, maxPkts, totalWorkerDrops, totalWorkerPkts){
  return '<div style="display:flex;flex-direction:column;gap:.18rem;justify-content:center;padding:.35rem .45rem;border:1px solid rgba(0,217,255,.18);background:rgba(0,217,255,.04);border-radius:4px;min-width:190px">'
    ...
    +(totalWorkerDrops>0?'<span style="margin-left:auto;font-size:var(--fs-3xs);color:var(--red);font-family:\'Courier New\'">'+totalWorkerDrops+'↓</span>':'<span style="margin-left:auto;font-size:var(--fs-3xs);color:rgba(0,255,136,.5)">✓ 0 drop</span>')
    +'</div>'
    +workers.map(function(w){
      ...
      var wCol=w.drops>0?'var(--red)':pct>70?_COL_SUR_WARN_HEX:'var(--cyan)';
      return '<div style="display:flex;align-items:center;gap:.3rem">'
        ...
        +'<div style="flex:1;height:5px;background:rgba(255,255,255,.06);border-radius:2px;overflow:hidden"><div class="'+barCls+'" style="width:'+pct+'%;height:100%;border-radius:2px"></div></div>'
        ...
        +(w.drops>0?'<span style="font-size:var(--fs-3xs);color:var(--red)"> ↓</span>':'')
        +'</div>';
    }).join('')
...
function _surWorkersDetailHtml(workers, maxPkts){
  if(!workers.length) return '';
  return '<div style="font-size:var(--fs-2xs);color:var(--c-label);text-transform:uppercase;letter-spacing:.8px;margin-bottom:.5rem;margin-top:.8rem">Workers — métriques par thread</div>'
    ...
    +workers.map(function(w){
      ...
      return '<div style="padding:.4rem .55rem;background:rgba(0,0,0,.25);border:1px solid '+wCol+'22;border-radius:4px">'
        ...
        +(w.drops>0?'<span style="font-size:var(--fs-3xs);color:var(--red);font-family:\'Courier New\'">'+w.drops+'↓</span>':'')
        +'</div>'
        +'<div style="height:6px;background:rgba(255,255,255,.05);border-radius:3px;overflow:hidden;margin-bottom:.3rem"><div class="'+barCls+'" style="width:'+pct+'%;height:100%;border-radius:3px"></div></div>'
        ...
        +'<span>'+fmt(w.pkts)+' pkts</span><span>'+fmtBytes(w.bytes||0)+' déc.</span>'
        +'<span style="color:'+alrCol+'">'+(w.alerts_detect||0)+' alr</span><span>'+(w.flows_active||0)+' flows</span>'
        +'<span>avg '+(w.avg_pkt||0)+'B</span><span>'+(w.trunc>0?'<span style="color:'+_COL_SUR_WARN_HEX+'">'+w.trunc+' tronq.</span>':'0 tronq.')+'</span>'
        +'</div></div>';
    }).join('')+'</div>';
}
function _surEngineNodesHtml(dec, det, rules, tot, s1, s2, s3, eveFillBar, overflowBadge, tcpPct, udpPct, otherPct, alertPct, suppPct){
  return '<div style="display:flex;flex-direction:column;gap:.4rem;justify-content:center">'
    ...
    +'<div style="display:flex;align-items:center;gap:.3rem;margin-bottom:.2rem"><span style="font-size:var(--fs-2xs);color:rgba(255,150,80,.9)">⚙</span>'
    +'<span style="font:700 9px \'Courier New\';color:rgba(255,150,80,.9);letter-spacing:.06em">DECODER</span>'
    +'<span style="margin-left:auto;font-size:var(--fs-3xs);color:var(--c-label)">avg '+Math.max(0,dec.avg_pkt||0)+'B</span></div>'
    +'<div style="display:flex;gap:.1rem;margin-bottom:.2rem;flex-wrap:nowrap">'+_sPill('AF_PKT','rgba(255,150,80,.8)')+_sArr()+_sPill('DECODE','rgba(255,150,80,.8)')+_sArr()+_sPill('PROTO','rgba(255,150,80,.8)')+_sArr()+_sPill('FRAG','rgba(255,150,80,.8)')+'</div>'
    +_surMiniBar(tcpPct,_COL_TCP_HEX,'TCP')+_surMiniBar(udpPct,_COL_SUR_WARN_HEX,'UDP')+_surMiniBar(otherPct,'rgba(160,220,255,.3)','OTH')
    +'<div style="font-size:var(--fs-3xs);color:rgba(160,220,255,.3);margin-top:.12rem">'+fmt(dec.pkts||0)+' pkts · '+fmtBytes(dec.bytes||0)+' déc.</div></div>'
    ...
    +'<div style="display:flex;align-items:center;gap:.3rem;margin-bottom:.2rem"><span style="font-size:var(--fs-2xs);color:rgba(255,120,120,.9)">◈</span>'
    +'<span style="font:700 9px \'Courier New\';color:rgba(255,120,120,.9);letter-spacing:.06em">RULES ENGINE</span>'+overflowBadge+'</div>'
    +'<div style="display:flex;gap:.1rem;margin-bottom:.2rem;flex-wrap:nowrap">'+_sPill('MATCH','rgba(255,100,100,.8)')+_sArr()+_sPill('SCORE','rgba(255,100,100,.8)')+_sArr()+_sPill('THRESH','rgba(255,100,100,.8)')+_sArr()+_sPill('ALERT','rgba(255,100,100,.8)')+'</div>'
    +_surMiniBar(alertPct,'var(--red)','ALR')+_surMiniBar(suppPct,'rgba(160,160,160,.4)','SUP')
    +'<div style="font-size:var(--fs-3xs);color:rgba(160,220,255,.3);margin-top:.12rem">'+Math.round(rules/1000)+'k règles · '+(det.alerts||0)+' alr · '+(det.suppressed||0)+' sup</div></div>'
    ...
    +'<div style="display:flex;align-items:center;gap:.3rem;margin-bottom:.2rem"><span style="font-size:var(--fs-2xs);color:rgba(0,255,136,.8)">▶</span>'
    +'<span style="font:700 9px \'Courier New\';color:rgba(0,255,136,.8);letter-spacing:.06em">EVE.JSON</span>'
    +'<div class="sur-dot-on" style="margin-left:auto"></div><span style="font-size:var(--fs-3xs);color:rgba(160,220,255,.3)">24h</span></div>'
    +'<div style="display:flex;gap:.1rem;margin-bottom:.2rem;flex-wrap:wrap">'+_sPill('ALERT','rgba(0,255,136,.8)')+_sArr()+_sPill('FLOW','rgba(0,255,136,.8)')+_sArr()+_sPill('DNS','rgba(0,255,136,.8)')+_sArr()+_sPill('HTTP','rgba(0,255,136,.8)')+_sArr()+_sPill('TLS','rgba(0,255,136,.8)')+'</div>'
    +eveFillBar+'</div></div>';
}
function _surMoteurPanelHtml(ts, evts, tot, s1, s2, s3, trunc, rules){
  var workers=ts&&ts.workers?ts.workers:[];
  var gPkts=ts?ts.global_pkts||0:0, gDrops=ts?ts.global_drops||0:0;
  var nic=ts&&ts.nic||{};
  var dec=ts&&ts.decoder||{}, det=ts&&ts.detect||{};
  ...
  var satCol=satPct>15?'var(--red)':satPct>5?_COL_SUR_WARN_HEX:'var(--cyan)';
  ...
  var alertPct=Math.round(((det.alerts||0)*100)/Math.max((det.alerts||0)+(det.suppressed||0),1));
  var suppPct=100-alertPct;
  var overflowBadge=det.overflow>0?'<span style="font-size:var(--fs-3xs);color:var(--red);margin-left:.3rem">⚠ OVERFLOW '+det.overflow+'</span>':'<span style="font-size:var(--fs-3xs);color:var(--green)">✓ OK</span>';
  var s1Pct=tot>0?Math.round(s1*100/tot):0, s2Pct=tot>0?Math.round(s2*100/tot):0;
  ...
  var eveFillBar='<div style="display:flex;justify-content:space-between;font-size:var(--fs-4xs);color:rgba(160,220,255,.25);margin-bottom:.06rem"><span>SEVERITY FILL</span><span>'+fmt(tot)+' events</span></div>'
    ...
    +(tot===0?'<div style="flex:1;background:rgba(0,255,136,.3)"></div>':'')+'</div>'
    +'<div style="display:flex;gap:.3rem;font-size:var(--fs-4xs);font-family:\'Courier New\'">'
    +'<span style="color:rgba(255,59,92,.7)">S1:'+s1+'</span><span style="color:rgba(255,153,0,.6)">S2:'+s2+'</span><span style="color:rgba(0,217,255,.5)">S3:'+s3+'</span></div>';
  ...
  return _surCircuitCss()
    ...
    +_surNicHtml(nic, gPkts)
    +'<div style="'+wSz+'"><div class="'+w1Cls+'"></div></div>'
    +_surRingHtml(satPct, satCol, ringProcPct, ringDropPct, ringTruncPct, trunc)
    +(gDrops>0?'<div style="display:flex;flex-direction:column;align-items:center;gap:.15rem;flex-shrink:0"><div style="'+wSz+'"><div class="sur-wire sur-wire-drop"></div></div><div style="font-size:var(--fs-3xs);color:var(--red);white-space:nowrap">'+gDrops+' drops</div></div>':'<div style="'+wSz+'"><div class="'+w2Cls+'"></div></div>')
    +_surWorkersPipeHtml(workers, maxPkts, totalWorkerDrops, totalWorkerPkts)
    +'<div style="'+wSz+'"><div class="'+w3Cls+'"></div></div>'
    +_surEngineNodesHtml(dec, det, rules, tot, s1, s2, s3, eveFillBar, overflowBadge, tcpPct, udpPct, otherPct, alertPct, suppPct)
    +'</div>'
    +_surWorkersDetailHtml(workers, maxPkts)
    ...
    +[{l:'Paquets capturés',v:fmt(gPkts),c:'rgba(0,217,255,.7)'},{l:'Drops kernel',v:gDrops>0?gDrops:'✓ 0',c:gDrops>0?'var(--red)':'var(--green)'},{l:'Saturation ring',v:satPct+'%',c:satCol}]
    .map(function(k){return '<div style="padding:.5rem .7rem;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.06);border-radius:4px"><div style="font-size:var(--fs-lg);font-weight:700;font-family:\'Courier New\',monospace;color:'+k.c+';line-height:1">'+k.v+'</div><div style="font-size:var(--fs-2xs);color:var(--c-label);margin-top:.2rem;text-transform:uppercase;letter-spacing:.4px">'+k.l+'</div></div>';}).join('')
    +'</div>';
}
function _surTabsHtml(s1, topSigs, alertBanner, kpiHtml, protoHtml, topIpsPanel, critPanel, sigsPanel, moteurPanel){
  ...
  return '<div id="sur-tabs" style="display:flex;align-items:flex-end;gap:0;border-bottom:1px solid rgba(255,59,92,.12);margin-bottom:1rem">'
...
function openSuricataModal(sur){
  ...
  var alertBanner  = _surBannerHtml(s1);
  var kpiHtml      = _surKpiHtml(s1, s2, s3, tot, trunc);
  var protoHtml    = _surProtoHtml(evts, rules);
  var topIpsPanel  = _surTopIpsHtml(topIps);
  var critPanel    = _surCritAlertsHtml(rc);
  var sigsPanel    = _surSigsHtml(topSigs);
  var moteurPanel  = _surMoteurPanelHtml(ts, evts, tot, s1, s2, s3, trunc, rules);
  var h            = _surTabsHtml(s1, topSigs, alertBanner, kpiHtml, protoHtml, topIpsPanel, critPanel, sigsPanel, moteurPanel);
  var mc=document.getElementById('modal-card');
  mc.classList.add('modal-wide','theme-red');
  mc.classList.add('modal-auto-height');
  ...
  mb.innerHTML=h;
  mb.classList.add('modal-body-scroll');
  mb.querySelectorAll('[data-surtab]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      ['resume','critical','signatures','moteur'].forEach(function(t){
...
// ── openF2bModal constants ───────────────────────────────────────────────────
var _F2B_HOST_COL={'SRV-NGIX':'rgba(0,217,255,.7)','PROXMOX':'rgba(255,215,0,.65)','CLT':'rgba(160,220,255,.6)','PA85':'rgba(200,180,255,.6)'};
// ── openF2bModal helpers (suite) ─────────────────────────────────────────────
function _f2bBuildHosts(f2b){
...
function _f2bBuildTotals(allHosts){
  return {
    totalBanned: allHosts.reduce(function(a,h){return a+(h.avail?h.jails.reduce(function(x,j){return x+(j.cur_banned||0);},0):0);},0),
    totalFailed: allHosts.reduce(function(a,h){return a+(h.avail?h.jails.reduce(function(x,j){return x+(j.tot_failed||0);},0):0);},0),
    activeJails: allHosts.reduce(function(a,h){return a+(h.avail?h.jails.filter(function(j){return j.cur_banned>0;}).length:0);},0),
    prebanJails: allHosts.reduce(function(a,h){return a+(h.avail?h.jails.filter(function(j){return j.cur_failed>0&&j.cur_banned===0;}).length:0);},0),
    totalJails:  allHosts.reduce(function(a,h){return a+(h.avail?h.jails.length:0);},0)
  };
}
function _f2bHostCardHtml(host){
...
function _f2bJailsPanelHtml(allHosts, t){
  var banCol=t.totalBanned>0?'var(--red)':'var(--green)';
  return '<div style="display:flex;align-items:center;gap:0;margin-bottom:.85rem;background:rgba(0,0,0,0.28);border:1px solid rgba(255,255,255,0.06);border-radius:3px;overflow:hidden">'
    +_f2bKpiHtml(t.totalBanned,'IPs bannies',banCol,false)
    +_f2bKpiHtml(t.activeJails,'Jails actives',t.activeJails>0?'var(--red)':'var(--c-muted-4)',false)
    +(t.prebanJails>0?_f2bKpiHtml(t.prebanJails,'Pré-ban ⚠','var(--amber)',true):'')
    +_f2bKpiHtml(t.totalFailed,'Échecs cumul',t.totalFailed>0?'var(--yellow)':'var(--c-muted-4)',false)
    +'<div style="padding:.55rem .8rem;text-align:right;flex-shrink:0"><div style="font-size:var(--fs-xs);color:rgba(122,154,184,.3);line-height:1.6">'+t.totalJails+' jails · 4 hôtes</div><div style="font-size:var(--fs-xs);color:rgba(122,154,184,.2)">refresh 5 min</div></div>'
    +'</div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem;margin-bottom:.8rem">'+allHosts.map(function(h){return _f2bHostCardHtml(h);}).join('')+'</div>'
...
function _f2bCircuitCss(){
  return '<style>'
    ...
    +'.f2b-wire{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,107,53,0) 0,rgba(255,107,53,0) 5px,rgba(255,107,53,.6) 5px,rgba(255,107,53,.6) 9px,rgba(255,107,53,0) 9px,rgba(255,107,53,0) 24px);background-size:24px 100%;animation:f2b-flow .7s linear infinite}'
    +'.f2b-wire-ban{background:repeating-linear-gradient(90deg,rgba(255,59,92,0) 0,rgba(255,59,92,0) 5px,rgba(255,59,92,.9) 5px,rgba(255,59,92,.9) 9px,rgba(255,59,92,0) 9px,rgba(255,59,92,0) 24px);background-size:24px 100%;animation:f2b-flow .28s linear infinite}'
    +'.f2b-bar-ban{background:linear-gradient(90deg,#ff3b5c44,var(--red),#ff6b8844,var(--red));background-size:200% 100%;animation:f2b-shimmer .7s linear infinite}'
    ...
    +'.f2b-node{border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;font:700 9px "Courier New",monospace;letter-spacing:.06em;padding:.38rem .55rem;border:1px solid;text-align:center}'
    ...
    +'.f2b-wire-fat{height:4px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,107,53,0) 0,rgba(255,107,53,0) 4px,rgba(255,107,53,.75) 4px,rgba(255,107,53,.75) 8px,rgba(255,107,53,0) 8px,rgba(255,107,53,0) 20px);background-size:20px 100%;animation:f2b-flow .42s linear infinite}'
    +'.f2b-wire-enforce{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(255,59,92,0) 0,rgba(255,59,92,0) 4px,rgba(255,59,92,.9) 4px,rgba(255,59,92,.9) 8px,rgba(255,59,92,0) 8px,rgba(255,59,92,0) 20px);background-size:20px 100%;animation:f2b-flow .22s linear infinite}'
    +'</style>';
}
function _f2bLogFilesHtml(allHosts, t){
  ...
  return '<div class="f2b-node" style="border-color:rgba(255,107,53,.3);background:rgba(255,107,53,.05);color:rgba(255,170,100,.9);min-width:115px'+(f2bLogSrcs.length>0?';animation:f2b-glow-w 2.5s ease-in-out infinite':'')+'\">'
    ...
    +'<span style="font-size:var(--fs-3xs);color:rgba(160,220,255,.28)">'+t.totalJails+' jails</span>'
    +'</div>'
    +'</div>';
}
function _f2bFiltersHtml(allHosts, t){
  ...
  return '<div class="f2b-node" style="border-color:rgba(255,215,0,.3);background:rgba(255,215,0,.04);color:rgba(255,215,0,.9);min-width:118px'+(t.prebanJails>0?';animation:f2b-glow-w 1.8s ease-in-out infinite':'')+'\">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">◈</div>'
    ...
    +'<div style="font-size:var(--fs-2xl);font-weight:800;font-family:\'Courier New\';color:rgba(255,215,0,.95);line-height:1">'+fmt(t.totalFailed)+'</div>'
    ...
    +(t.prebanJails>0?'<div style="font-size:var(--fs-3xs);color:rgba(245,158,11,.8);font-weight:700">⚠ '+t.prebanJails+' pré-ban</div>':'')
    +'<div style="font-size:var(--fs-4xs);color:rgba(160,220,255,.2);margin-top:.06rem">algo: maxretry · findtime</div>'
    +'</div>';
}
function _f2bJailsEngHtml(allHosts, t){
  ...
  var f2bTotalForFill=Math.max(t.totalBanned,1);
  return '<div class="f2b-node" style="border-color:rgba(255,59,92,.3);background:rgba(255,59,92,.05);color:rgba(255,100,100,.9);min-width:148px'+(t.totalBanned>0?';animation:f2b-glow-b 1.6s ease-in-out infinite':'')+'\">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">⊘</div>'
    ...
    +allHosts.map(function(h){
      ...
      var hCol=!h.avail?'rgba(160,220,255,.2)':hBan>0?'var(--red)':'var(--green)';
      return '<div style="margin-bottom:.18rem;width:100%">'
        +'<div style="display:flex;align-items:center;gap:.2rem;margin-bottom:.07rem">'
        +'<span style="font-size:var(--fs-3xs);color:'+(_F2B_HOST_COL[h.label]||hCol)+';min-width:50px;font-family:\'Courier New\';font-weight:700">'+h.label+'</span>'
        +(h.avail
    ...
    +allHosts.filter(function(h){return h.avail&&h.jails.reduce(function(a,j){return a+(j.cur_banned||0);},0)>0;}).map(function(h){
      ...
      var hCol=_F2B_HOST_COL[h.label]||'rgba(255,59,92,.6)';
      return '<div style="width:'+pct+'%;height:100%;background:'+hCol+';animation:f2b-shimmer .7s linear infinite"></div>';
    }).join('')
    +'</div>'
    +'<div style="display:flex;justify-content:space-between;font-size:var(--fs-4xs);color:rgba(160,220,255,.25);margin-bottom:.06rem"><span>FILL LEVEL</span><span>'+t.totalBanned+' bans</span></div>'
    +'<div style="font-size:var(--fs-3xs);color:rgba(160,220,255,.3)">'+t.activeJails+' active'+(t.activeJails>1?'s':'')+' · '+t.totalJails+' total</div>'
    +'</div>';
}
function _f2bActionsHtml(allHosts, t){
  var f2bMaxBan=Math.max.apply(null,allHosts.map(function(h){return h.avail?h.jails.reduce(function(a,j){return a+(j.cur_banned||0);},0):0;}).concat([1]));
  var f2bBanCol=t.totalBanned>0?'var(--red)':'var(--green)';
  var f2bActBtimes=[];
  allHosts.forEach(function(h){if(!h.avail)return;h.jails.forEach(function(j){if(j.bantime&&f2bActBtimes.indexOf(j.bantime)===-1)f2bActBtimes.push(j.bantime);});});
  return '<div class="f2b-node" style="border-color:rgba(0,255,136,.3);background:rgba(0,255,136,.04);color:rgba(0,255,136,.9);min-width:122px'+(t.totalBanned>0?';animation:f2b-glow-b 2s ease-in-out infinite':'')+'\">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">⊛</div>'
    ...
    +'<div style="font-size:var(--fs-3xl);font-weight:800;font-family:\'Courier New\';color:'+f2bBanCol+';line-height:1;text-shadow:0 0 12px '+f2bBanCol+'55">'+t.totalBanned+'</div>'
    ...
    +['iptables','ufw','nftables'].map(function(tp){return '<div style="font-size:var(--fs-4xs);padding:.03rem .14rem;background:rgba(0,255,136,.07);border:1px solid rgba(0,255,136,.2);border-radius:2px;color:rgba(0,255,136,.55)">'+tp+'</div>';}).join('')
    +'</div>'
    +allHosts.filter(function(h){return h.avail;}).map(function(h){
      var hBan=h.jails.reduce(function(a,j){return a+(j.cur_banned||0);},0);
      var pct=Math.round(hBan*100/f2bMaxBan);
      var hCol=_F2B_HOST_COL[h.label]||'rgba(0,255,136,.5)';
      return '<div style="display:flex;align-items:center;gap:.2rem;margin-bottom:.1rem">'
        +'<span style="font-size:var(--fs-3xs);color:'+hCol+';min-width:44px;font-family:\'Courier New\'">'+h.label+'</span>'
        +'<div style="flex:1;height:4px;background:rgba(255,255,255,.04);border-radius:2px;overflow:hidden">'
        +'<div style="width:'+pct+'%;height:100%;background:'+(hBan>0?'var(--red)':'rgba(0,255,136,.3)')+';border-radius:2px'+(hBan>0?';animation:f2b-shimmer .65s linear infinite':'')+'"></div>'
        +'</div>'
        +'<span style="font-size:var(--fs-3xs);color:'+(hBan>0?'var(--red)':'rgba(160,220,255,.25)')+';min-width:14px;text-align:right;font-family:\'Courier New\';font-weight:'+(hBan>0?'700':'400')+'">'+hBan+'</span>'
        +'</div>';
    }).join('')
...
function _f2bJailsDetailHtml(allHosts){
  var flat=[];
  allHosts.forEach(function(h){if(!h.avail)return;h.jails.forEach(function(j){flat.push({host:h.label,j:j});});});
  ...
  return '<div style="font-size:var(--fs-2xs);color:var(--c-label);text-transform:uppercase;letter-spacing:.8px;margin-bottom:.5rem">Jails — statut live · '+flat.length+' jails · '+allHosts.filter(function(h){return h.avail;}).length+' hôtes</div>'
    ...
    +flat.map(function(e){
      ...
      var jCol=isBan?'var(--red)':isPre?PROTO_PALETTE.WATCH:'rgba(0,255,136,.6)';
      ...
      return '<div style="padding:.35rem .5rem;background:'+bg+';border:1px solid rgba(255,255,255,.05);border-left:'+bleft+';border-radius:2px">'
        ...
        +(isPre?'<span style="color:'+_COL_F2B_PRE_HEX+'">'+j.cur_failed+' fail</span>':'')
...
function _f2bMoteurPanelHtml(allHosts, t){
  var wSz='width:38px;flex-shrink:0;align-self:center';
  return _f2bCircuitCss()
    +'<div style="font-size:var(--fs-2xs);color:var(--c-label);text-transform:uppercase;letter-spacing:.8px;margin-bottom:.8rem">Pipeline de protection par regex — Fail2ban · '+allHosts.filter(function(h){return h.avail;}).length+' hôtes actifs · '+t.totalJails+' jails</div>'
    ...
    +_f2bLogFilesHtml(allHosts, t)
    +'<div style="'+wSz+'"><div class="f2b-wire"></div></div>'
    +_f2bFiltersHtml(allHosts, t)
    +'<div style="'+wSz+'"><div class="'+(t.totalFailed>100?'f2b-wire f2b-wire-fat':'f2b-wire')+'"></div></div>'
    +_f2bJailsEngHtml(allHosts, t)
    +'<div style="'+wSz+'"><div class="'+(t.totalBanned>0?'f2b-wire f2b-wire-enforce':'f2b-wire')+'"></div></div>'
    +_f2bActionsHtml(allHosts, t)
    +'</div>'
    +'<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:.4rem;margin-bottom:.9rem">'
    +[
      {l:'IPs bannies',v:t.totalBanned,c:t.totalBanned>0?'var(--red)':'var(--green)'},
      {l:'Jails actives',v:t.activeJails+' / '+t.totalJails,c:t.activeJails>0?'var(--red)':'var(--c-muted-4)'},
      {l:'Pré-ban',v:t.prebanJails||0,c:t.prebanJails>0?'var(--amber)':'var(--c-muted-4)'},
      {l:'Échecs cumulés',v:fmt(t.totalFailed),c:t.totalFailed>0?'var(--yellow)':'var(--c-muted-4)'},
    ...
    +'</div>'
    +_f2bJailsDetailHtml(allHosts);
}
function _f2bTabsHtml(totalBanned, jailsPanel, moteurPanel){
  ...
  return '<div id="f2b-tabs" style="display:flex;align-items:flex-end;gap:0;border-bottom:1px solid rgba(255,107,53,.12);margin-bottom:1rem">'
    ...
    +'</div>'
    +'<div id="f2b-panel-jails">'+jailsPanel+'</div>'
    +'<div id="f2b-panel-moteur" style="display:none">'+moteurPanel+'</div>';
}
function openF2bModal(f2b){
  if(!f2b||_isOpen)return;
  var allHosts=_f2bBuildHosts(f2b);
  var t=_f2bBuildTotals(allHosts);
  var h=_f2bTabsHtml(
    t.totalBanned,
    _f2bJailsPanelHtml(allHosts, t),
    _f2bMoteurPanelHtml(allHosts, t)
  );
  var mc=document.getElementById('modal-card');
  mc.classList.add('modal-wide','modal-auto-height');
  var _ht=document.getElementById('modal-header-title');
  ...
  mb.querySelectorAll('[data-f2btab]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      ['jails','moteur'].forEach(function(tp){
        var p=document.getElementById('f2b-panel-'+tp);
        if(p)p.style.display=tp===name?'':'none';
      });
    });
  });
  _overlay.classList.add('open');
...
// ── openCsModal constants ────────────────────────────────────────────────────
var _CS_STAGE_COL={'RECON':'var(--cyan)','SCAN':'var(--yellow)','EXPLOIT':'var(--orange)','BRUTE':'var(--red)'};
var _CS_ORIG_COL={'cscli':'var(--amber)','crowdsec':'var(--cyan)','console':'var(--purple)'};
var _CS_SCOL2={'RECON':_COL_CYAN_HEX,'SCAN':_COL_YELLOW_HEX,'EXPLOIT':_COL_ORANGE_HEX,'BRUTE':_COL_RED_HEX};
var _CS_GEO_CTRIES=['RU','CN','KP','IR'];
var _CS_GEO_NAMES={RU:'Russie',CN:'Chine',KP:'Corée du Nord',IR:'Iran'};
var _CS_LOG_PATHS={'nginx':'/nginx/access.log','sshd':'auth · journald','auth':'auth.log','apache':'/apache2/access'};
var _CS_LOG_SRCS=['nginx','sshd','auth','apache'];
// ── openCsModal helpers ───────────────────────────────────────────────────────
function _csAlertBannerHtml(csStages){
  var hasE=(csStages.EXPLOIT||0)>0, hasB=(csStages.BRUTE||0)>0;
  if(!hasE&&!hasB) return '';
  var _aS=hasE?'EXPLOIT':'BRUTE', _aC=csStages[_aS];
  var _aCl=hasE?'var(--orange)':'var(--red)';
  return '<div style="display:flex;align-items:center;gap:.6rem;padding:.35rem .9rem;border-left:3px solid '+_aCl+';background:rgba(255,107,53,.07);border-radius:0 4px 4px 0;margin-bottom:1.2rem;animation:blink 1.5s ease-in-out infinite">'
...
function _csKpiHtml(csD, csA, csBV, csBStats, csPsr, csTrend){
  ...
  return '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:0;border:1px solid rgba(255,255,255,.07);border-radius:6px;overflow:hidden;margin-bottom:1.2rem">'
...
function _csKillChainHtml(csStages, totalStages){
  var STAGE_DEF=[{ico:'◎',k:'RECON',c:'var(--cyan)'},{ico:'◈',k:'SCAN',c:'var(--yellow)'},{ico:'◉',k:'EXPLOIT',c:'var(--orange)'},{ico:'⊗',k:'BRUTE',c:'var(--red)'}];
  return '<div style="margin-bottom:1.2rem">'
...
function _csGeoPerimetreHtml(decList){
  var cnts={RU:0,CN:0,KP:0,IR:0}, lastIp={};
  decList.forEach(function(r){
    if((r.scenario||'').indexOf('geo-block')!==-1){
      var c=r.country||'';
      if(cnts.hasOwnProperty(c)){cnts[c]++;if(!lastIp[c])lastIp[c]=r.ip||'';}
    }
  });
  var total=_CS_GEO_CTRIES.reduce(function(s,c){return s+cnts[c];},0);
  return '<div style="margin-bottom:1.2rem">'
    +'<div style="font-size:var(--fs-2xs);color:var(--c-label);text-transform:uppercase;letter-spacing:.8px;margin-bottom:.55rem">Blocage périmétrique GeoIP · '+total+' ban'+(total!==1?'s':'')+' actif'+(total!==1?'s':'')+' — RU · CN · KP · IR</div>'
    +'<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:0;border:1px solid rgba(255,255,255,.07);border-radius:6px;overflow:hidden">'
    +_CS_GEO_CTRIES.map(function(c,i){
      var n=cnts[c]||0, active=n>0;
      var col=active?'rgba(255,107,53,.85)':'rgba(160,220,255,.2)';
      var bl=i>0?'border-left:1px solid rgba(255,255,255,.07);':'';
      return '<div style="padding:.55rem .5rem;background:'+(active?'rgba(255,107,53,.05)':'rgba(0,0,0,.18)')+';'+bl+'text-align:center">'
        ...
        +'<div style="font-size:var(--fs-3xs);color:rgba(160,220,255,.25);margin-top:.1rem">'+_CS_GEO_NAMES[c]+'</div>'
        +(lastIp[c]?'<div style="font-size:var(--fs-3xs);color:rgba(255,107,107,.45);font-family:\'Courier New\',monospace;margin-top:.08rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+esc(lastIp[c])+'">'+esc(lastIp[c])+'</div>':'')
        +'</div>';
    }).join('')
    +'</div>'
    +(total===0?'<div style="font-size:var(--fs-3xs);color:rgba(0,255,136,.4);margin-top:.35rem;font-family:\'Courier New\',monospace;text-align:center">✓ Périmètre actif · scénario <GITHUB-USER>/geo-block-aggressive · aucun ban en cours</div>':'')
    +'</div>';
}
function _csIpsBanniesHtml(csIps){
  if(!csIps.length) return '';
  return '<div>'
    ...
    +csIps.map(function(e){
      var st_col=_CS_STAGE_COL[e.stage]||'var(--orange)';
...
function _csDecTableHtml(decList){
  if(!decList.length) return '<div style="color:var(--green);font-size:var(--fs-xs);font-family:\'Courier New\',monospace;padding:1.5rem;text-align:center">✓ Aucune décision active</div>';
  ...
  return '<div style="overflow-x:auto;overflow-y:auto;max-height:60vh;border:1px solid rgba(255,255,255,.07);border-radius:6px">'
    ...
    +decList.map(function(r,i){
      var sc=_CS_STAGE_COL[r.stage]||'rgba(160,220,255,.5)';
      var oc=_CS_ORIG_COL[r.origin]||'rgba(122,154,184,.5)';
...
function _csCircuitCss(){
  return '<style>'
    ...
    +'.cs-wire{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(0,217,255,0) 0,rgba(0,217,255,0) 5px,rgba(0,217,255,.65) 5px,rgba(0,217,255,.65) 9px,rgba(0,217,255,0) 9px,rgba(0,217,255,0) 24px);background-size:24px 100%;animation:cs-flow .6s linear infinite}'
    +'.cs-wire-threat{background:repeating-linear-gradient(90deg,rgba(255,107,53,0) 0,rgba(255,107,53,0) 5px,rgba(255,107,53,.85) 5px,rgba(255,107,53,.85) 9px,rgba(255,107,53,0) 9px,rgba(255,107,53,0) 24px);background-size:24px 100%;animation:cs-flow .28s linear infinite}'
    +'.cs-node{border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;font:700 9px "Courier New",monospace;letter-spacing:.06em;padding:.38rem .55rem;border:1px solid;text-align:center}'
    +'.cs-dot-on{animation:cs-pulse .9s ease-in-out infinite}'
    +'.cs-wire-fat{height:4px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(0,217,255,0) 0,rgba(0,217,255,0) 4px,rgba(0,217,255,.75) 4px,rgba(0,217,255,.75) 8px,rgba(0,217,255,0) 8px,rgba(0,217,255,0) 20px);background-size:20px 100%;animation:cs-flow .45s linear infinite}'
    +'.cs-wire-enforce{height:3px;border-radius:2px;background:repeating-linear-gradient(90deg,rgba(0,255,136,0) 0,rgba(0,255,136,0) 4px,rgba(0,255,136,.85) 4px,rgba(0,255,136,.85) 8px,rgba(0,255,136,0) 8px,rgba(0,255,136,0) 20px);background-size:20px 100%;animation:cs-flow .35s linear infinite}'
    +'</style>';
}
function _csLogSourceHtml(csSc, csPsr){
  var csActiveSrcCount=_CS_LOG_SRCS.filter(function(s){
    return csSc.some(function(sc){return (sc.name||'').indexOf(s)!==-1;});
  }).length;
  return '<div class="cs-node" style="border-color:rgba(0,217,255,.3);background:rgba(0,217,255,.05);color:rgba(0,217,255,.85);min-width:108px'+(csActiveSrcCount>0?';animation:cs-glow-c 2.2s ease-in-out infinite':'')+'\">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">◎</div>'
    ...
    +_CS_LOG_SRCS.map(function(s){
      ...
      return '<div style="display:flex;align-items:flex-start;gap:.2rem;margin-bottom:.18rem;width:100%;text-align:left">'
        ...
        +'<div style="font-size:var(--fs-4xs);color:rgba(160,220,255,.18);font-family:\'Courier New\';margin-top:.02rem">'+_CS_LOG_PATHS[s]+'</div>'
        +'</div>'
    ...
    +'<div style="display:flex;justify-content:space-between;width:100%;align-items:center">'
    +'<span style="font-size:var(--fs-3xs);color:rgba(0,255,136,.6);font-weight:700">'+csActiveSrcCount+'/'+_CS_LOG_SRCS.length+' actifs</span>'
    +((csPsr.lines_read||0)>0?'<span style="font-size:var(--fs-3xs);color:rgba(160,220,255,.3)">'+fmt(csPsr.lines_read||0)+' lignes</span>':'')
    +'</div>'
    +'</div>';
}
function _csParserHtml(csPsr){
  ...
  return '<div class="cs-node" style="border-color:rgba(0,217,255,.35);background:rgba(0,217,255,.06);color:rgba(0,217,255,.9);min-width:108px">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">⊛</div>'
...
function _csLeakyBucketHtml(csSc, csStages, totalStages, csBV){
  ...
  var csBucketFillBar='<div style="display:flex;width:100%;height:9px;background:rgba(255,255,255,.04);border-radius:3px;overflow:hidden;margin-bottom:.04rem">'
    +['RECON','SCAN','EXPLOIT','BRUTE'].map(function(s){
      var cnt=csStages[s]||0, pct=Math.round(cnt*100/csTotalForFill), col=_CS_SCOL2[s];
  ...
  return '<div class="cs-node" style="border-color:rgba(255,215,0,.3);background:rgba(255,215,0,.04);color:rgba(255,215,0,.9);min-width:148px'+(csBucketThreat?';animation:cs-glow-t 1.4s ease-in-out infinite':'')+'\">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">⊗</div>'
    +'<div style="margin-bottom:.14rem;letter-spacing:1px">LEAKY BUCKET</div>'
    +'<div style="height:1px;background:rgba(255,215,0,.12);width:100%;margin-bottom:.18rem"></div>'
    +['RECON','SCAN','EXPLOIT','BRUTE'].map(function(s){
      var cnt=csStages[s]||0, pct=Math.round(cnt*100/csStageMax), col=_CS_SCOL2[s];
      var isThreat=s==='EXPLOIT'||s==='BRUTE';
...
function _csDecNodeHtml(csD, csA, decList, csBV, csTrend){
  var csOrigCounts={};
  ...
  var csDecCol2=csD>0?'var(--red)':'var(--green)';
  var tDir=csTrend.dir||'stable', tPct=csTrend.pct||0;
  var tCol=tDir==='up'?'var(--red)':tDir==='down'?'var(--green)':'rgba(122,154,184,.5)';
  var tArrow=tDir==='up'?'↑':tDir==='down'?'↓':'→';
  return '<div class="cs-node" style="border-color:'+csDecCol2+'44;background:rgba(0,0,0,.3);color:'+csDecCol2+';min-width:118px'+(csD>0?';animation:cs-glow-t 2s ease-in-out infinite':'')+'\">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">◈</div>'
    ...
    +(csBV.last_1h!==undefined?'<div style="width:100%;margin-bottom:.08rem">'
      ...
      +'<span style="color:'+(csBV.spike?'var(--red)':'rgba(160,220,255,.6)')+';font-weight:700">'+csBV.last_1h+(csBV.spike?' ⚠':'')+'</span>'
      +'</div>'
      +(csBV.last_1h>0?'<div style="width:100%;height:3px;background:rgba(255,255,255,.04);border-radius:2px;overflow:hidden">'
        +'<div style="width:'+Math.min(csBV.last_1h*10,100)+'%;height:100%;background:'+(csBV.spike?'var(--red)':'rgba(255,107,53,.6)')+';border-radius:2px;animation:cs-shimmer .7s linear infinite;background-size:200% 100%"></div>'
        +'</div>':'')
      +'</div>':'')
...
function _csBouncerNodeHtml(csBouncers, csAppsec, csBStats){
  return '<div class="cs-node" style="border-color:rgba(0,255,136,.3);background:rgba(0,255,136,.04);color:rgba(0,255,136,.9);min-width:152px">'
    +'<div style="font-size:var(--fs-xs);margin-bottom:.12rem">⊘</div>'
...
function _csTopScenariosHtml(csSc){
  if(!csSc.length) return '';
  ...
  var CS_STCOL={recon:_COL_CYAN_HEX,scan:_COL_YELLOW_HEX,exploit:_COL_ORANGE_HEX,brute:_COL_RED_HEX};
  return '<div style="font-size:var(--fs-2xs);color:var(--c-label);text-transform:uppercase;letter-spacing:.8px;margin-bottom:.5rem">Scénarios actifs · '+csSc.length+' détectés</div>'
...
function _csMoteurPanelHtml(cs, totalStages){
  var csSc=cs.scenarios||[], csStages=cs.stage_counts||{};
  var csPsr=cs.parser_stats||{}, csBouncers=cs.bouncers||[];
  var csBV=cs.ban_velocity||{}, csBStats=cs.bouncer_stats||{}, csAppsec=cs.appsec||{};
  var decList=cs.decisions_list||[], csD=cs.active_decisions||0, csA=cs.alerts_24h||0;
  var csTrend=cs.alerts_trend||{};
  var csRead=csPsr.lines_read||0;
  var csThreatWire=(csStages.EXPLOIT||0)+(csStages.BRUTE||0)>0;
  var csEnforceWire=csD>0;
  var wSz='width:38px;flex-shrink:0;align-self:center';
  return _csCircuitCss()
    ...
    +_csLogSourceHtml(csSc, csPsr)
    +'<div style="'+wSz+'"><div class="cs-wire"></div></div>'
    +_csParserHtml(csPsr)
    +'<div style="'+wSz+'"><div class="'+(csRead>50000?'cs-wire cs-wire-fat':'cs-wire')+'"></div></div>'
    +_csLeakyBucketHtml(csSc, csStages, totalStages, csBV)
    +'<div style="'+wSz+'"><div class="'+(csThreatWire?'cs-wire cs-wire-threat':'cs-wire')+'"></div></div>'
    +_csDecNodeHtml(csD, csA, decList, csBV, csTrend)
    +'<div style="'+wSz+'"><div class="'+(csEnforceWire?'cs-wire cs-wire-enforce':'cs-wire')+'"></div></div>'
    +_csBouncerNodeHtml(csBouncers, csAppsec, csBStats)
    +'</div>'
    +_csTopScenariosHtml(csSc);
}
function _csPanelTabsHtml(csD, alertH, kpiH, kcH, geoH, ipsH, emptyH, tableH, moteurH){
  var _tbs='display:inline-flex;align-items:center;gap:.3rem;padding:.28rem .8rem;font-size:var(--fs-2xs);font-family:\'Courier New\',monospace;font-weight:700;letter-spacing:1px;text-transform:uppercase;cursor:pointer;border:none;border-bottom:2px solid transparent;background:transparent;transition:all .15s;line-height:1;';
  return '<div id="cs-tabs" style="display:flex;align-items:flex-end;gap:0;border-bottom:1px solid rgba(0,217,255,.12);margin-bottom:1rem">'
    ...
    +'</div>'
    +'<div id="cs-panel-resume">'+alertH+kpiH+kcH+geoH+ipsH+emptyH+'</div>'
    +'<div id="cs-panel-decisions" style="display:none">'+tableH+'</div>'
    +'<div id="cs-panel-moteur" style="display:none">'+moteurH+'</div>';
}
function openCsModal(cs){
  ...
  var csBV=cs.ban_velocity||{}, csBStats=cs.bouncer_stats||{}, csTrend=cs.alerts_trend||{};
  var decList=cs.decisions_list||[];
  var totalStages=(csStages.RECON||0)+(csStages.SCAN||0)+(csStages.EXPLOIT||0)+(csStages.BRUTE||0);
  var emptyH=(!csSc.length&&!csIps.length)?'<div style="color:var(--green);font-size:var(--fs-xs);font-family:\'Courier New\',monospace;padding:.5rem 0">✓ Surveillance active — aucune menace en cours</div>':'';
  var h=_csPanelTabsHtml(
    csD,
    _csAlertBannerHtml(csStages),
    _csKpiHtml(csD, csA, csBV, csBStats, csPsr, csTrend),
    _csKillChainHtml(csStages, totalStages),
    _csGeoPerimetreHtml(decList),
    _csIpsBanniesHtml(csIps),
    emptyH,
    _csDecTableHtml(decList),
    _csMoteurPanelHtml(cs, totalStages)
  );
  var mc=document.getElementById('modal-card');
...
function closeModal(){
  ...
  setTimeout(function(){
    ...
    _mc.classList.remove('modal-wide','modal-proto','modal-xl','modal-win','modal-gpu','modal-router','modal-kci','modal-geomap','modal-auto-height','theme-red','theme-green','theme-cyan','theme-purple','theme-orange','theme-yellow');
    ...
    _modalBody.classList.remove('modal-body-scroll','modal-fs-lg','xdr-modal-body');
    _isOpen=false;
