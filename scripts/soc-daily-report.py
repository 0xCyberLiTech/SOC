...
# Infra : source unique → scripts/soc_infra.yaml (Sprint 1A 2026-05-16).
...
_STAGE_CSS = {
    'PROBE':   ('#8aa6c8', '#101a25'),  # KC 7 maillons (2026-05-16) — paquets UFW droppés
    'RECON':   ('#3b82f6', '#0a1528'),
    'SCAN':    ('#f59e0b', '#2d1f00'),
    'EXPLOIT': ('#ef4444', '#2d0a0a'),
    'WAF':     ('#00bcd4', '#0a1f24'),  # KC 7 maillons — ModSec inline blocks
    'BRUTE':   ('#bf5fff', '#1a0d2e'),
}
...
def _html_f2b(f2b):
    ...
    return (
        ...
        + '<th style="color:#7a9ab8;padding:4px 8px;font-size:10px;text-align:left;font-weight:600;letter-spacing:1px">JAIL</th>'
        + '<th style="color:#7a9ab8;padding:4px 8px;font-size:10px;text-align:right;font-weight:600;letter-spacing:1px">STATUT</th>'
        + '</tr>' + jails_rows + '</table>'
    )
...
def _html_crowdsec(cs):
    ...
    for stage in _soc_infra.KC_STAGES_OFFENSIVE:
        v = sc.get(stage, 0)
    ...
    html += _html_crowdsec_ips(tips, csd)
    return html

def _html_crowdsec_ips(tips, csd):
    if not tips:
        if csd == 0:
            return '<p style="color:#22c55e;margin:8px 0 0;font-size:12px">&#10003; Surveillance active — aucune IP bannie</p>'
        return ''
...
def _html_report_footer():
    return f"""
  <!-- FOOTER DASHBOARD -->
  <tr><td bgcolor="#0a1020" style="background-color:#0a1020;border:1px solid #1e3a5f;border-top:none;
    padding:0;border-radius:0 0 6px 6px;overflow:hidden">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td bgcolor="#0f1e35" style="background-color:#0f1e35;border-top:2px solid #1e3a5f;padding:14px 20px">
          ...
          <div style="color:#2e4a6a;font-size:10px;margin-top:4px">
            Accès LAN uniquement &mdash; {_LAN_CIDR}
          </div>
        </td>
        <td align="right" bgcolor="#0f1e35" style="background-color:#0f1e35;border-top:2px solid #1e3a5f;padding:14px 20px;white-space:nowrap">
          <div style="color:#2e4a6a;font-size:9px;letter-spacing:1px">0xCyberLiTech</div>
          <div style="color:#2e4a6a;font-size:9px">Rapport automatique &mdash; Ne pas r&eacute;pondre</div>
        </td>
      </tr>
    </table>
  </td></tr>
...
<style type="text/css">
  html,body{{margin:0!important;padding:0!important;background-color:#060e1a!important}}
  .ExternalClass,.ExternalClass p,.ExternalClass span,.ExternalClass td,.ExternalClass div{{line-height:100%}}
</style>
<!--[if mso]><style type="text/css">
  body,table,td{{background-color:#060e1a!important;font-family:'Courier New',Courier,monospace!important}}
</style><![endif]-->
<title>SOC Report — 0xCyberLiTech</title></head>
<body bgcolor="#060e1a" style="margin:0;padding:0;background-color:#060e1a;font-family:'Courier New',Courier,monospace">
<table width="100%" cellpadding="0" cellspacing="0" bgcolor="#060e1a" style="background-color:#060e1a">
<tr><td align="center" bgcolor="#060e1a" style="background-color:#060e1a;padding:24px 0">
<table width="640" cellpadding="0" cellspacing="0" bgcolor="#060e1a" style="max-width:640px;width:100%;background-color:#060e1a">

  <!-- HEADER -->
  <tr><td bgcolor="#0d1525" style="background:linear-gradient(135deg,#0d1525 0%,#0f2040 100%);
  ...
  <!-- BODY -->
  <tr><td bgcolor="#0b1628" style="background-color:#0b1628;padding:18px 20px;border:1px solid #1e3a5f;border-top:none;border-bottom:none">
    ...
    {_section('Système — srv-nginx', _html_system(sys_))}
...
def build_text(d):
    ...
    active = [(s, sc.get(s,0)) for s in _soc_infra.KC_STAGES_OFFENSIVE if sc.get(s,0)>0]
